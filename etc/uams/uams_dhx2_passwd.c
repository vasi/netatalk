...
static int dh_params_generate(gcry_mpi_t *ret_p, gcry_mpi_t *ret_g,
                              unsigned int bits) {
  ...
  do {
    ...
    err =
        gcry_prime_generate(&prime, bits, qbits, &factors, NULL, NULL,
...
static int passwd_login(void *obj, struct passwd **uam_pwd, char *ibuf,
...
static int logincont1(void *obj, struct passwd **uam_pwd, char *ibuf,
                      size_t ibuflen, char *rbuf, size_t *rbuflen) {
  size_t nwritten;
  int ret;
...
static int logincont2(void *obj, struct passwd **uam_pwd, char *ibuf,
                      size_t ibuflen, char *rbuf, size_t *rbuflen) {
#ifdef SHADOWPW
  struct spwd *sp;
#endif /* SHADOWPW */
  ...
  *rbuflen = 0;
  ...
  if ((ibuflen != 2 + 16 + 256) && (ibuflen != 2 + 16 + 256 + 10)) {
    LOG(log_error, logtype_uams,
        "DHX2: Paket length not correct: %d. Should be 274 or 284.", ibuflen);
    ret = AFPERR_PARAM;
    goto error_noctx;
  }
  ...
  if (sp && sp->sp_max != -1 && sp->sp_lstchg) {
    ...
    if (expire_days < 0) {
      LOG(log_info, logtype_uams, "password for user %s expired",
          dhxpwd->pw_name);
      ret = AFPERR_PWDEXPR;
      goto exit;
    }
...
static int passwd_logincont(void *obj, struct passwd **uam_pwd, char *ibuf,
                            size_t ibuflen, char *rbuf, size_t *rbuflen) {
  uint16_t retID;
  int ret;
...
static void uam_cleanup(void) { uam_unregister(UAM_SERVER_LOGIN, "DHX2"); }
