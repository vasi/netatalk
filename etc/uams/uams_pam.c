...
static int login(void *obj, char *username, int ulen, struct passwd **uam_pwd,
  ...
  err = AFPERR_NOTAUTH;
  PAM_error = pam_start("netatalk", username, &PAM_conversation, &pamh);
...
static int pam_printer(char *start, char *stop, char *username,
  ...
  PAM_error = pam_start("netatalk", username, &PAM_conversation, &pamh);
  if (PAM_error != PAM_SUCCESS) {
    LOG(log_info, logtype_uams, "Bad Login ClearTxtUAM: %s: %s", username,
...
static int uam_setup(void *obj, const char *path) {
  if (uam_register(UAM_SERVER_LOGIN_EXT, path, "Cleartxt Passwrd", pam_login,
                   NULL, pam_logout, pam_login_ext) < 0)
    return -1;
  ...
  if (uam_register(UAM_SERVER_PRINTAUTH, path, "ClearTxtUAM", pam_printer) <
      0) {
    return -1;
  }
