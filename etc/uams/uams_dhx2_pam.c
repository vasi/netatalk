...
/* Number of bits for p which we generate. Everybode out there uses 512, so we
 * beet them */
...
static int dh_params_generate(unsigned int bits) {
  ...
  if (!gcry_check_version(GCRYPT_VERSION)) {
    LOG(log_error, logtype_uams,
        "PAM DHX2: libgcrypt versions mismatch. Need: %s", GCRYPT_VERSION);
    result = AFPERR_MISC;
    goto error;
  }
  ...
  do {
    ...
    err =
        gcry_prime_generate(&p, bits, qbits, &factors, NULL, NULL,
...
#define COPY_STRING(s) (s) ? strdup(s) : NULL
  ...
  for (count = 0; count < num_msg; count++) {
    ...
    case PAM_ERROR_MSG:
    default:
      LOG(log_info, logtype_uams, "PAM DHX2: Binary_Prompt -- %s",
          strerror(errno));
      goto pam_fail_conv;
    }
...
static int dhx2_setup(void *obj, char *ibuf, size_t ibuflen, char *rbuf,
                      size_t *rbuflen) {
  int ret;
  size_t nwritten;
  gcry_mpi_t Ma;
  ...
  *rbuflen += 2;
  ...
  /* p */
  gcry_mpi_print(GCRYMPI_FMT_USG, (unsigned char *)rbuf, PRIMEBITS / 8, NULL,
                 p);
  rbuf += PRIMEBITS / 8;
  *rbuflen += PRIMEBITS / 8;

  /* Ma */
  gcry_mpi_print(GCRYMPI_FMT_USG, (unsigned char *)rbuf, PRIMEBITS / 8,
                 &nwritten, Ma);
...
/* -------------------------------- */
static int logincont1(void *obj, char *ibuf, size_t ibuflen, char *rbuf,
                      size_t *rbuflen) {
  int ret;
  size_t nwritten;
...
static int logincont2(void *obj_in, struct passwd **uam_pwd, char *ibuf,
                      size_t ibuflen, char *rbuf, size_t *rbuflen) {
  AFPObj *obj = obj_in;
  int ret = AFPERR_MISC;
  int PAM_error;
  ...
  *rbuflen = 0;
  ...
  /* Packet size should be: Session ID + ServerNonce + Passwd buffer (evantually
   * +10 extra bytes, see Apples Docs) */
  if ((ibuflen != 2 + 16 + 256) && (ibuflen != 2 + 16 + 256 + 10)) {
    LOG(log_error, logtype_uams,
        "DHX2: Paket length not correct: %u. Should be 274 or 284.", ibuflen);
    ret = AFPERR_PARAM;
    goto error_noctx;
  }
  ...
  if (PAM_error != PAM_SUCCESS) {
    ...
    if (!obj->options.adminauthuser)
      goto error_ctx;
    if (loginasroot(obj->options.adminauthuser, &hostname, PAM_error) !=
        PAM_SUCCESS) {
      goto error_ctx;
    }
  }
...
static int changepw_1(void *obj, char *uname, char *ibuf, size_t ibuflen,
...
static int changepw_2(void *obj, char *ibuf, size_t ibuflen, char *rbuf,
                      size_t *rbuflen) {
  return (logincont1(obj, ibuf, ibuflen, rbuf, rbuflen));
}

static int changepw_3(void *obj, char *ibuf, size_t ibuflen, char *rbuf,
                      size_t *rbuflen) {
  int ret;
  int PAM_error;
  uid_t uid;
  ...
  *rbuflen = 0;
  ...
  /* Set these things up for the conv function. PAM_username was set in
   * changepw_1 */
...
static int dhx2_changepw(void *obj, char *uname, struct passwd *pwd, char *ibuf,
                         size_t ibuflen, char *rbuf, size_t *rbuflen) {
...
static int uam_setup(void *obj, const char *path) {
  if (uam_register(UAM_SERVER_LOGIN_EXT, path, "DHX2", pam_login, pam_logincont,
                   pam_logout, pam_login_ext) < 0)
    return -1;
