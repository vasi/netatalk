...
#define COPY_STRING(s) (s) ? strdup(s) : NULL
  ...
  if (num_msg < 1) {
    ...
    LOG(log_info, logtype_uams,
        "uams_dhx_pam.c :PAM DHX Conversation Err -- %s", strerror(errno));
    /* Log Entry */
    return PAM_CONV_ERR;
  }
  ...
  for (count = 0; count < num_msg; count++) {
    ...
    case PAM_PROMPT_ECHO_ON:
      if (!(string = COPY_STRING(PAM_username))) {
        /* Log Entry */
        LOG(log_info, logtype_uams,
            "uams_dhx_pam.c :PAM: username failure -- %s", strerror(errno));
        /* Log Entry */
        goto pam_fail_conv;
      }
      break;
    case PAM_PROMPT_ECHO_OFF:
      if (!(string = COPY_STRING(PAM_password))) {
        /* Log Entry */
        LOG(log_info, logtype_uams,
            "uams_dhx_pam.c :PAM: passwd failure: --: %s", strerror(errno));
        /* Log Entry */
        goto pam_fail_conv;
      }
      break;
...
static int dhx_setup(void *obj, char *ibuf, size_t ibuflen, char *rbuf,
                     size_t *rbuflen) {
  uint16_t sessid;
  size_t i;
  BIGNUM *bn, *gbn, *pbn;
  ...
  *rbuflen += KEYSIZE;
  ...
  i = sizeof(randbuf);
  if (uam_afpserver_option(obj, UAM_OPTION_RANDNUM, (void *)randbuf, &i) < 0) {
    *rbuflen = 0;
    /* Log Entry */
    LOG(log_info, logtype_uams,
        "uams_dhx_pam.c :PAM: Buffer Encryption Err. -- %s", strerror(errno));
    /* Log Entry */
    goto pam_fail;
  }
  ...
  /* encrypt using cast */
  CAST_cbc_encrypt((unsigned char *)rbuf, (unsigned char *)rbuf, CRYPTBUFLEN,
                   &castkey, msg2_iv, CAST_ENCRYPT);
  *rbuflen += CRYPTBUFLEN;
  BN_free(bn);
  DH_free(dh);
...
pam_fail:
  ...
  /* Log Entry */
  LOG(log_info, logtype_uams,
      "uams_dhx_pam.c :PAM: Fail - Cast Encryption -- %s", strerror(errno));
  /* Log Entry */
  return AFPERR_PARAM;
}
...
static int login(void *obj, char *username, int ulen, struct passwd **uam_pwd,
  ...
  if ((dhxpwd = uam_getname(obj, username, ulen)) == NULL) {
    LOG(log_info, logtype_uams, "uams_dhx_pam.c: unknown username [%s]",
        username);
...
static int pam_login(void *obj, struct passwd **uam_pwd, char *ibuf,
  ...
  *rbuflen = 0;
  ...
  if (uam_afpserver_option(obj, UAM_OPTION_USERNAME, (void *)&username, &ulen) <
      0) {
    LOG(log_info, logtype_uams,
        "uams_dhx_pam.c :PAM: uam_afpserver_option didn't meet "
        "uam_option_username  -- %s",
        strerror(errno));
    return AFPERR_PARAM;
  }
...
static int pam_logincont(void *obj, struct passwd **uam_pwd, char *ibuf,
                         size_t ibuflen, char *rbuf, size_t *rbuflen) {
  const char *hostname;
  BIGNUM *bn1, *bn2, *bn3;
  uint16_t sessid;
  ...
  *rbuflen = 0;
  ...
  if (uam_afpserver_option(obj, UAM_OPTION_CLIENTNAME, (void *)&hostname,
                           NULL) < 0) {
    LOG(log_info, logtype_uams,
  ...
  CAST_cbc_encrypt((unsigned char *)ibuf, (unsigned char *)rbuf, CRYPT2BUFLEN,
                   &castkey, msg3_iv, CAST_DECRYPT);
...
static int pam_changepw(void *obj, char *username, struct passwd *pwd,
                        char *ibuf, size_t ibuflen, char *rbuf,
                        size_t *rbuflen) {
  BIGNUM *bn1, *bn2, *bn3;
  ...
  if (uam_afpserver_option(obj, UAM_OPTION_HOSTNAME, (void *)&hostname, NULL) <
      0) {
    /* Log Entry */
  ...
  CAST_cbc_encrypt((unsigned char *)ibuf, (unsigned char *)ibuf, CHANGEPWBUFLEN,
                   &castkey, msg3_iv, CAST_DECRYPT);
  memset(&castkey, 0, sizeof(castkey));
  ...
  if (!(bn1 = BN_bin2bn((unsigned char *)ibuf, KEYSIZE, NULL))) {
    /* Log Entry */
    LOG(log_info, logtype_uams,
        "uams_dhx_pam.c :PAM: Random Number Not the same or not incremented-- "
        "%s",
        strerror(errno));
    /* Log Entry */
    return AFPERR_PARAM;
  ...
  PAM_error = pam_start("netatalk", username, &PAM_conversation, &lpamh);
  if (PAM_error != PAM_SUCCESS) {
    /* Log Entry */
    LOG(log_info, logtype_uams,
        "uams_dhx_pam.c :PAM: Needless to say, PAM_error is != to PAM_SUCCESS "
        "-- %s",
        strerror(errno));
    /* Log Entry */
    return AFPERR_PARAM;
...
static int uam_setup(void *obj, const char *path) {
  ...
  if (uam_register(UAM_SERVER_CHANGEPW, path, "DHCAST128", pam_changepw) < 0) {
