...
#define LOG_UAMS(log_level, ...) LOG(log_level, logtype_uams, __VA_ARGS__)
...
static void log_status(char *s, OM_uint32 major_status,
  ...
  while (1) {
    maj_status = gss_display_status(&min_status, major_status, GSS_C_GSS_CODE,
                                    GSS_C_NULL_OID, &maj_ctx, &msg);
    LOG_UAMS(log_error, "%s %.*s (error %s)", s, msg.length, msg.value,
             strerror(errno));
  ...
  while (1) {
    maj_status = gss_display_status(&min_status, minor_status, GSS_C_MECH_CODE,
                                    GSS_C_NULL_OID, &min_ctx, &msg);
    LOG_UAMS(log_error, "%s %.*s (error %s)", s, msg.length, msg.value,
             strerror(errno));
    gss_release_buffer(&min_status, &msg);
...
static void log_service_name(gss_ctx_id_t context) {
  ...
  major_status =
      gss_display_name(&minor_status, service_name, &service_name_buffer, NULL);
  if (major_status == GSS_S_COMPLETE) {
    LOG_LOGINCONT(log_debug, "service principal is `%s'",
...
static int get_client_username(char *username, int ulen,
  ...
  major_status =
      gss_display_name(&minor_status, *client_name, &client_name_buffer, NULL);
  if (major_status != GSS_S_COMPLETE) {
    log_status("gss_display_name", major_status, minor_status);
    return 1;
  }

  LOG_LOGINCONT(log_debug, "user principal is `%s'", client_name_buffer.value);
  ...
  if (cnblen >= ulen) {
    ...
    LOG_LOGINCONT(log_info, "username `%s' too long (%d)",
...
static int wrap_sessionkey(gss_ctx_id_t context, struct session_info *sinfo) {
  ...
  major_status = gss_wrap(&minor_status, context, true, GSS_C_QOP_DEFAULT,
                          &sesskey_buff, NULL, &wrap_buff);
...
static int accept_sec_context(gss_ctx_id_t *context,
  ...
  LOG_LOGINCONT(log_debug, "accepting context (ticketlen: %u)",
  ...
  major_status = gss_accept_sec_context(
      &minor_status, context, GSS_C_NO_CREDENTIAL, ticket_buffer,
      GSS_C_NO_CHANNEL_BINDINGS, client_name, NULL, authenticator_buff, &flags,
      NULL, NULL);
...
static int do_gss_auth(void *obj, char *ibuf, size_t ibuflen, char *rbuf,
                       int *rbuflen, char *username, size_t ulen,
  ...
  if ((ret = accept_sec_context(&context, &ticket_buffer, &client_name,
                                &authenticator_buff)))
    return ret;
 ...
 */
static int gss_login(void *obj, struct passwd **uam_pwd, char *ibuf,
...
static int gss_logincont(void *obj, struct passwd **uam_pwd, char *ibuf,
                         size_t ibuflen, char *rbuf, size_t *rbuflen) {
  ...
  /* We skip past the 'username' parameter because all that matters is the
   * ticket */
  p = ibuf;
  while (*ibuf && ibuflen) {
    ibuf++, ibuflen--;
  ...
  if (ticket_len > ibuflen) {
    LOG_LOGINCONT(log_info, "invalid ticket length (%u > %u)", ticket_len,
                  ibuflen);
    return AFPERR_PARAM;
  }
...
static int set_principal(AFPObj *obj, char *principal) {
  ...
  if (len > 255) {
    LOG(log_error, logtype_afpd,
        "set_principal: principal '%s' too long (max=255)", principal, len);
    return -1;
  }

  /* We store: 1 byte number principals (1) + 1 byte principal name length +
   * zero terminated principal */
...
static int gss_create_principal(AFPObj *obj) {
  ...
  if (obj->options.k5service && obj->options.fqdn && obj->options.k5realm) {
    ...
    if ((ret = krb5_build_principal(
             context, &service_principal, strlen(obj->options.k5realm),
             obj->options.k5realm, obj->options.k5service, obj->options.fqdn,
             NULL)))
      goto krb5_error;

    if ((ret = krb5_kt_get_entry(context, keytab, service_principal,
                                 0, // kvno - wildcard
                                 0, // enctype - wildcard
                                 &entry)) == KRB5_KT_NOTFOUND) {
      krb5_unparse_name(context, service_principal, &principal);
      LOG(log_error, logtype_afpd,
          "gss_create_principal: specified service principal '%s' not found in "
          "keytab",
          principal);
    ...
    LOG(log_debug, logtype_afpd,
        "gss_create_principal: using first entry from keytab as service "
        "principal");
...
krb5_cleanup:
  ...
  char principal[255];
  size_t len =
      snprintf(principal, sizeof(principal), "%s/%s@%s", obj->options.k5service,
               obj->options.fqdn, obj->options.k5realm);
...
static int uam_setup(void *handle, const char *path) {
  ...
  return uam_register(UAM_SERVER_LOGIN_EXT, path, "Client Krb v2", gss_login,
                      gss_logincont, gss_logout, gss_login_ext);
}
