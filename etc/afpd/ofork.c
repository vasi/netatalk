...
int of_rename(const struct vol *vol, struct ofork *s_of, struct dir *olddir,
              const char *oldpath, struct dir *newdir, const char *newpath) {
  ...
  while ((of = next)) {
    ...
    if (vol == of->of_vol && olddir->d_did == of->of_did &&
        s_of->key.dev == of->key.dev && s_of->key.inode == of->key.inode) {
      if (!done) {
        free(of_name(of));
...
struct ofork *of_alloc(struct vol *vol, struct dir *dir, char *path,
                       uint16_t *ofrefnum, const int eid, struct adouble *ad,
                       struct stat *st) {
  struct ofork *of;
  ...
  */
  ...
  if ((oforks[of_refnum] = (struct ofork *)malloc(sizeof(struct ofork))) ==
      NULL) {
...
int of_closefork(const AFPObj *obj, struct ofork *ofork) {
  ...
  /* Somone has used write_fork, we assume file was changed, register it to file
   * change event api */
   ...
   * determined.
   */
  if ((ofork->of_ad->ad_rlen == 0) && (ofork->of_flags & AFPFORK_RSRC) &&
