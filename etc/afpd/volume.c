...
static long long int get_tm_bandsize(const char *path) {
  ...
  EC_NULL_LOGSTR(file = fopen(path, "r"), "get_tm_bandsize(\"%s\"): %s", path,
                 strerror(errno));
  ...
  while (fgets(buf, sizeof(buf), file) != NULL) {
    ...
    if (fscanf(file, " <integer>%lld</integer>", &bandsize) != 1) {
      LOG(log_error, logtype_afpd,
          "get_tm_bandsize(\"%s\"): can't parse band-size", path);
      EC_FAIL;
    }
    break;
  }
...
static long long int get_tm_bands(const char *path) {
  ...
  while ((entry = readdir(dir)) != NULL)
    count++;
  count -= 2; /* All OSens I'm aware of return "." and "..", so just substract
                 them, avoiding string comparison in loop */
 ...
 * 3) parse "\1.sparsebundle/Info.plist" and read the band-size XML key integer
 * value 4) readdir "\1.sparsebundle/bands/" counting files 5) calculate used
 * size as: (file_count - 1) * band-size
 *
...
static int get_tm_used(struct vol *restrict vol) {
  ...
  while ((entry = readdir(dir)) != NULL) {
    if (((p = strstr(entry->d_name, "sparsebundle")) != NULL) &&
      ...
      EC_NULL_LOG(
          bandsdir = bformat("%s/%s/%s/", vol->v_path, entry->d_name, "bands"));
...
static int getvolspace(const AFPObj *obj, struct vol *vol, uint32_t *bfree,
                       uint32_t *btotal, VolSpace *xbfree, VolSpace *xbtotal,
                       uint32_t *bsize) {
  int spaceflag, rc;
  uint32_t maxsize;
...
getvolspace_done:
  if (vol->v_limitsize) {
    ...
    *xbfree =
        MIN(*xbfree, *xbtotal < vol->v_tm_used ? 0 : *xbtotal - vol->v_tm_used);

    LOG(log_debug, logtype_afpd,
        "volparams: total: %" PRIu64 ", used: %" PRIu64 ", free: %" PRIu64
        " bytes",
...
static int getvolparams(const AFPObj *obj, uint16_t bitmap, struct vol *vol,
  ...
  if (ad_open(&ad, vol->v_path,
    ...
    if (ad_getentryoff(&ad, ADEID_NAME)) {
      ...
      memcpy(ad_entry(&ad, ADEID_NAME), slash, ad_getentrylen(&ad, ADEID_NAME));
    }
  ...
  if ((bitmap &
       ((1 << VOLPBIT_BFREE) | (1 << VOLPBIT_BTOTAL) | (1 << VOLPBIT_XBFREE) |
        (1 << VOLPBIT_XBTOTAL) | (1 << VOLPBIT_BSIZE))) != 0) {
    if (getvolspace(obj, vol, &bfree, &btotal, &xbfree, &xbtotal, &bsize) !=
        AFP_OK) {
      if (isad) {
...
static int stat_vol(const AFPObj *obj, uint16_t bitmap, struct vol *vol,
  ...
  if ((ret = getvolparams(obj, bitmap, vol, &st, rbuf + sizeof(bitmap),
                          &buflen)) != AFP_OK) {
    *rbuflen = 0;
    return (ret);
  }
...
int afp_getsrvrparms(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  data = rbuf + 5;
  for (vcnt = 0, volume = getvolumes(); volume && vcnt < 255;
    ...
    *data++ |= 0; /* UNIX PRIVS BIT ..., OSX doesn't seem to use it, so we don't
                     either */
    *data++ = len;
...
static int volume_codepage(AFPObj *obj, struct vol *volume) {
  ...
  if ((charset_t)-1 ==
    ...
    LOG(log_error, logtype_afpd,
        "Setting codepage %s as volume codepage failed", volume->v_volcodepage);
    return -1;
  }
  ...
  if (NULL == (charset = find_charset_functions(volume->v_volcodepage)) ||
    ...
    LOG(log_warning, logtype_afpd,
        "WARNING: volume encoding %s is *not* supported by netatalk, expect "
        "problems !!!!",
...
static int volume_openDB(const AFPObj *obj, struct vol *volume) {
  ...
  if (!volume->v_cdb && !(flags & CNID_FLAG_MEMORY)) {
     ...
     */
    LOG(log_error, logtype_afpd,
        "Can't open volume \"%s\" CNID backend \"%s\" ", volume->v_path,
        volume->v_cnidscheme);
    LOG(log_error, logtype_afpd,
        "Reopen volume %s using in memory temporary CNID DB.", volume->v_path);
    flags |= CNID_FLAG_MEMORY;
    volume->v_cdb = cnid_open(volume, "tdb", flags);
#ifdef SERVERTEXT
    /* kill ourself with SIGUSR2 aka msg pending */
    if (volume->v_cdb) {
      setmessage("Something wrong with the volume's CNID DB, using temporary "
                 "CNID DB instead."
...
int afp_openvol(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  if (utf8_encoding(obj)) {
    ...
    namelen = convert_string(obj->options.maccharset, CH_UCS2, ibuf, len,
  ...
  if (volume->v_root_preexec) {
    if ((ret = afprun(1, volume->v_root_preexec, NULL)) &&
      ...
      LOG(log_error, logtype_afpd, "afp_openvol(%s): root preexec : %d",
          volume->v_path, ret);
      return AFPERR_MISC;
    }
  }
  ...
  if (volume->v_preexec) {
    if ((ret = afprun(0, volume->v_preexec, NULL)) && volume->v_preexec_close) {
      LOG(log_error, logtype_afpd, "afp_openvol(%s): preexec : %d",
          volume->v_path, ret);
      return AFPERR_MISC;
    }
  }
  ...
  if (utf8_encoding(obj)) {
    len = convert_string_allocate(CH_UCS2, CH_UTF8_MAC, volume->v_u8mname,
                                  namelen, &vol_mname);
  } else {
  ...
  if ((dir = dir_new(vol_mname, vol_uname, volume, DIRDID_ROOT_PARENT,
                     DIRDID_ROOT, bfromcstr(volume->v_path), &st)) == NULL) {
    free(vol_mname);
  ...
  if (ret == AFP_OK) {
     ...
     * FIXME (RL): should it be done inside a CNID backend ? (always returning
     * Trash DID when asked) ?
     */
    if ((volume->v_cdb->cnid_db_flags & CNID_FLAG_PERSISTENT)) {
      ...
      if (cnid_getstamp(volume->v_cdb, volume->v_stamp,
        ...
        LOG(log_error, logtype_afpd,
            "afp_openvol(%s): Fatal error: Unable to get stamp value from CNID "
            "backend",
            volume->v_path);
        ret = AFPERR_MISC;
        goto openvol_err;
    ...
    const char *msg;
    if ((msg = atalk_iniparser_getstring(obj->iniconfig, volume->v_configname,
...
void setvoltime(AFPObj *obj, struct vol *vol) {
  ...
  if (gettimeofday(&tv, NULL) < 0) {
    LOG(log_error, logtype_afpd, "setvoltime(%s): gettimeofday: %s",
