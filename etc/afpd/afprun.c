...
****************************************************************************/
static void gain_root_privilege(void) { seteuid(0); }

/****************************************************************************
 Ensure our real and effective groups are zero.
 we want to end up with rgid==egid==0
****************************************************************************/
static void gain_root_group_privilege(void) { setegid(0); }
...
int afprun(int root, char *cmd, int *outfd) {
  ...
  if (pid) {
    ...
    if (wpid != pid) {
      LOG(log_error, logtype_afpd, "waitpid(%d) : %s", (int)pid,
          strerror(errno));
      if (outfd) {
        close(*outfd);
        *outfd = -1;
  ...
  if (getuid() != uid || geteuid() != uid || getgid() != gid ||
    ...
    exit(81); /* we can't print stuff at this stage, instead use exit codes for
                 debugging */
  }
