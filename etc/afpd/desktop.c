...
int setdeskmode(const struct vol *vol, const mode_t mode) {
  ...
  if ((desk = opendir(".")) == NULL) {
    if (chdir(wd) < 0) {
      LOG(log_error, logtype_afpd, "setdeskmode: chdir %s: %s", wd,
          strerror(errno));
    }
    EC_FAIL;
  }
  for (deskp = readdir(desk); deskp != NULL; deskp = readdir(desk)) {
    if (strcmp(deskp->d_name, ".") == 0 || strcmp(deskp->d_name, "..") == 0 ||
    ...
    for (subp = readdir(sub); subp != NULL; subp = readdir(sub)) {
      if (strcmp(subp->d_name, ".") == 0 || strcmp(subp->d_name, "..") == 0) {
        continue;
      }
      *m = '\0';
    ...
    closedir(sub);
    /* XXX: need to preserve special modes */
    if (ochmod(deskp->d_name, DIRBITS | mode, NULL,
               vol_syml_opt(vol) | vol_chmod_opt(vol)) < 0 &&
        errno != EPERM) {
      LOG(log_error, logtype_afpd, "setdeskmode: chmod %s: %s",
  ...
  /* XXX: need to preserve special modes */
  if (ochmod(bdata(dtpath), DIRBITS | mode, NULL,
             vol_syml_opt(vol) | vol_chmod_opt(vol)) < 0 &&
      errno != EPERM) {
...
int setdeskowner(const struct vol *vol, uid_t uid, gid_t gid) {
  ...
  if ((desk = opendir(".")) == NULL) {
    if (chdir(wd) < 0) {
      LOG(log_error, logtype_afpd, "setdeskowner: chdir %s: %s", wd,
          strerror(errno));
    }
    EC_FAIL;
  }
  for (deskp = readdir(desk); deskp != NULL; deskp = readdir(desk)) {
    ...
    if (chown(deskp->d_name, uid, gid) < 0 && errno != EPERM) {
      LOG(log_error, logtype_afpd, "setdeskowner: chown %s: %s", deskp->d_name,
          strerror(errno));
    }
  }
  closedir(desk);
    ...
    if (lstat(cfrombstr(olddtpath), &st) == 0) {
      ...
      if (run_cmd("mv", cmd_argv) != 0) {
        LOG(log_error, logtype_afpd,
            "moving .AppleDesktop from \"%s\" to \"%s\" failed",
  ...
  if ((si.sdt_fd = open(dtf, flags, ad_mode(dtf, mode))) < 0) {
    if (errno == ENOENT && (flags & O_CREAT)) {
      ...
      *adts = '/';
      ...
      if ((si.sdt_fd = open(dtf, flags, ad_mode(dtf, mode))) < 0) {
        LOG(log_error, logtype_afpd, "iconopen(%s): open: %s", dtf,
            strerror(errno));
        return -1;
      }
    } else {
...
addicon_err:
  ...
  if ((cc == 0) && (write(si.sdt_fd, imh, sizeof(imh)) < 0)) {
    LOG(log_error, logtype_afpd, "afp_addicon(%s): write: %s",
        icon_dtfile(vol, fcreator), strerror(errno));
    dsi_writeflush(dsi);
    return AFPERR_PARAM;
  }
  ...
  *rbuflen = 0;
  ...
  if (memcmp(fcreator, ucreator, sizeof(ucreator)) == 0) {
    ...
    *(ih + sizeof(utag) + sizeof(utype) + 1) = 0;
    memcpy(ih + sizeof(utag) + sizeof(utype) + 2, &usize, sizeof(usize));
  ...
  for (;;) {
    ...
    if (lseek(si.sdt_fd, (off_t)bsize, SEEK_CUR) < 0) {
      LOG(log_error, logtype_afpd, "afp_iconinfo(%s): lseek: %s",
          icon_dtfile(vol, fcreator), strerror(errno));
      return (AFPERR_PARAM);
    }
...
#define min(a, b) ((a) < (b) ? (a) : (b))
  ...
  if (buflen < rc) {
    ...
    *rbuflen = buflen;
    ...
    while (*rbuflen > 0) {
#ifdef WITH_SENDFILE
      if (dsi_stream_read_file(dsi, si.sdt_fd, offset, dsi->datasize, AFP_OK) <
          0) {
        switch (errno) {
        case ENOSYS:
...
char *mtoupath(const struct vol *vol, char *mpath, cnid_t did, int utf8) {
  ...
  if ((size_t)-1 ==
      (outlen = convert_charset((utf8) ? CH_UTF8_MAC : vol->v_maccharset,
                                vol->v_volcharset, vol->v_maccharset, m, inplen,
                                u, outlen, &flags))) {
...
char *utompath(const struct vol *vol, char *upath, cnid_t id, int utf8) {
  ...
  /* convert charsets */
  if ((size_t)-1 ==
      (outlen = convert_charset(
           vol->v_volcharset, (utf8) ? CH_UTF8_MAC : vol->v_maccharset,
           vol->v_maccharset, u, outlen, mpath, MAXPATHLEN, &flags))) {
...
static int ad_addcomment(const AFPObj *obj, struct vol *vol, struct path *path,
  ...
  if (ad_open(adp, upath,
              ADFLAGS_HF | ((isadir) ? ADFLAGS_DIR : 0) | ADFLAGS_CREATE |
                  ADFLAGS_RDWR,
              0666) < 0) {
    return (AFPERR_ACCESS);
  }
...
static int ad_rmvcomment(const AFPObj *obj, struct vol *vol,
  ...
  if (ad_open(adp, upath,
              ADFLAGS_HF | ADFLAGS_RDWR | ((isadir) ? ADFLAGS_DIR : 0)) < 0) {
    switch (errno) {
    case ENOENT:
      return (AFPERR_NOITEM);
    case EACCES:
