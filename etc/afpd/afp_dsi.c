...
/*
 * We generally pass this from afp_over_dsi to all afp_* funcs, so it should
 * already be available everywhere. Unfortunately some funcs (eg acltoownermode)
 * need acces to it but are deeply nested in the function chain with the caller
 * already without acces to it. Changing this would require adding a reference
 * to the caller which itself might be called in many places (eg acltoownermode
 * is called from accessmode). The only sane way out is providing a copy of it
 * here:
 */
AFPObj *AFPobj = NULL;
...
static void afp_dsi_close(AFPObj *obj) {
  ...
  if (geteuid() != obj->uid) {
    if (seteuid(obj->uid) < 0) {
      LOG(log_error, logtype_afpd,
          "can't seteuid(%u) back %s: uid: %u, euid: %u", obj->uid,
  ...
  close_all_vol(obj);
  if (obj->logout) {
    /* Block sigs, PAM/systemd/whoever might send us a SIG??? in
     * (*obj->logout)() -> pam_close_session() */
    sigfillset(&sigs);
...
static void afp_dsi_transfer_session(int sig) {
  ...
  /*
   * The session transfer happens in the middle of FPDisconnect old session,
   * thus we have to send the reply now.
   */
  ...
  /*
   * Now returning from this signal handler return to dsi_receive which should
   * start reading/continuing from the connected socket that was passed via the
   * parent from another session. The parent will terminate that session.
   */
...
static void afp_dsi_reload(int sig) { reload_request = 1; }
...
static void afp_dsi_debug(int sig) { debug_request = 1; }
...
static void alarm_handler(int sig) {
  ...
  LOG(log_maxdebug, logtype_afpd,
      "alarm: tickles: %u, flags: %s|%s|%s|%s|%s|%s|%s|%s|%s", dsi->tickle,
...
static void child_handler(int sig) { wait(NULL); }
...
#ifdef SA_NOCLDWAIT
  /* this enhancement simplifies things for Solaris, it also improves
   * performance */
...
void afp_over_dsi(AFPObj *obj) {
  ...
  if (obj->options.tcp_rcvbuf) {
    if (setsockopt(dsi->socket, SOL_SOCKET, SO_RCVBUF, &obj->options.tcp_rcvbuf,
  ...
  if (obj->options.tcp_sndbuf) {
    if (setsockopt(dsi->socket, SOL_SOCKET, SO_SNDBUF, &obj->options.tcp_sndbuf,
  ...
  while (1) {
    ...
    if (cmd == 0) {
      ...
      if (dsi->flags & DSI_RECONSOCKET) {
        /* we just got a reconnect so we immediately try again to receive on the
         * new fd */
        dsi->flags &= ~DSI_RECONSOCKET;
        continue;
      }

      /* the client sometimes logs out (afp_logout) but doesn't close the DSI
       * session */
    ...
    case DSIFUNC_TICKLE:
      ...
      /* timer is not every 30 seconds anymore, so we don't get killed on the
       * client side. */
    ...
    case DSIFUNC_CMD:
      ...
      if (replaycache[rc_idx].DSIreqID == dsi->clientID &&
        ...
        if (afp_switch[function]) {
          ...
          err = (*afp_switch[function])(obj, (char *)dsi->commands, dsi->cmdlen,
