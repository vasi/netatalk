 ...
 * - empty the trash
 * afp_enumerate return an empty listing but offspring count != 0 in
 * afp_getdirparams and the Mac doesn't try to call afp_delete!
 *
...
/* ----------------------------- */
int for_each_dirent(const struct vol *vol, char *name, dir_loop fn,
                    void *data) {
  DIR *dp;
  struct dirent *de;
  char *m_name;
...
#define REPLY_PARAM_MAXLEN                                                     \
  (4 + 104 + 1 + MACFILELEN + 4 + 2 + UTF8FILELEN_EARLY + 1)

/* ----------------------------- */
static int enumerate(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  if (sd.sd_buflen == 0) {
    if ((sd.sd_buf = (char *)malloc(SDBUFBRK)) == NULL) {
      LOG(log_error, logtype_afpd, "afp_enumerate: malloc: %s",
          strerror(errno));
      *rbuflen = 0;
      return AFPERR_MISC;
    }
  ...
  *rbuflen = 0;
  ...
  LOG(log_debug, logtype_afpd,
      "enumerate(\"%s/%s\", f/d:%04x/%04x, rc:%u, i:%u, max:%u)", getcwdpath(),
      o_path->u_name, fbitmap, dbitmap, reqcnt, sindex, maxsz);
  ...
  if (sindex == 1 || curdir->d_did != sd.sd_did || vid != sd.sd_vid) {
    ...
    if ((!o_path->st_valid && ostat(".", &o_path->st, vol_syml_opt(vol)) < 0) ||
      ...
      LOG(log_error, logtype_afpd, "enumerate: loop error: %s (%d)",
  ...
  while ((len = (unsigned char)*(sd.sd_last)) != 0) {
    ...
    if (ad_convert(sd.sd_last, &s_path.st, vol, &convname) == 0) {
      if (convname) {
        ...
        s_path.id = cnid_lookup(vol->v_cdb, &s_path.st, curdir->d_did,
                                sd.sd_last, strlen(sd.sd_last));
        AFP_CNID_DONE();
        if (s_path.id != CNID_INVALID) {
          AFP_CNID_START("cnid_update");
          int cnid_up_ret =
              cnid_update(vol->v_cdb, s_path.id, &s_path.st, curdir->d_did,
                          (char *)convname, strlen(convname));
          AFP_CNID_DONE();
          if (cnid_up_ret != 0)
            LOG(log_error, logtype_afpd,
                "enumerate: error updating CNID of \"%s\"",
    ...
    if (S_ISDIR(s_path.st.st_mode)) {
      ...
      if ((dir = dircache_search_by_name(vol, curdir, s_path.u_name, len)) ==
          NULL) {
    ...
    *data++ =
        S_ISDIR(s_path.st.st_mode) ? (char)FILDIRBIT_ISDIR : FILDIRBIT_ISFILE;
    if (ext) {
      *data++ = 0;
    }
    data += esz;
