...
/****
 * With coalesce we try to reduce the events over UDP, the eventlistener would
 *throw these events away anyway. This works only, if the connected listener
 *uses the events on a "per directory" base It is a very simple aproach, but
 *saves a lot of events sent to listeners. Every "child element" event is
 *ignored as long as its parent event is not older than MAX_COALESCE_TIME_MS ms.
 *If large directory trees or large files are created or deleted, this probably
 *will not work recursive, because the time to copy data will exceed this event
 *timeout.
 *
 ****/
