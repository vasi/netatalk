...
static int getforkparams(const AFPObj *obj, struct ofork *ofork,
  ...
  st = &path.st;
  if (bitmap &
      ((1 << FILPBIT_DFLEN) | (1 << FILPBIT_EXTDFLEN) | (1 << FILPBIT_FNUM) |
       (1 << FILPBIT_CDATE) | (1 << FILPBIT_MDATE) | (1 << FILPBIT_BDATE))) {
...
static int fork_setmode(const AFPObj *obj, struct adouble *adp, int eid,
  ...
  if (!(access & (OPENACC_WR | OPENACC_RD | OPENACC_DWR | OPENACC_DRD))) {
    return ad_lock(adp, eid, ADLOCK_RD | ADLOCK_FILELOCK, AD_FILELOCK_OPEN_NONE,
                   1, ofrefnum);
  }
  ...
  if ((access & (OPENACC_RD | OPENACC_DRD))) {
    ...
    /* boolean logic is not enough, because getforkmode is not always telling
     * the true
     */
    if ((access & OPENACC_RD)) {
      ret = ad_lock(adp, eid, ADLOCK_RD | ADLOCK_FILELOCK, AD_FILELOCK_OPEN_RD,
                    1, ofrefnum);
      if (ret)
        return ret;
    }
    if ((access & OPENACC_DRD)) {
      ret = ad_lock(adp, eid, ADLOCK_RD | ADLOCK_FILELOCK, AD_FILELOCK_DENY_RD,
                    1, ofrefnum);
      if (ret)
        return ret;
    }
  ...
  if ((access & (OPENACC_WR | OPENACC_DWR))) {
    ...
    if ((access & OPENACC_WR)) {
      ret = ad_lock(adp, eid, ADLOCK_RD | ADLOCK_FILELOCK, AD_FILELOCK_OPEN_WR,
                    1, ofrefnum);
      if (ret)
        return ret;
    }
    if ((access & OPENACC_DWR)) {
      ret = ad_lock(adp, eid, ADLOCK_RD | ADLOCK_FILELOCK, AD_FILELOCK_DENY_WR,
                    1, ofrefnum);
      if (ret)
        return ret;
    }
   ...
   * Note that this still leaves room for a race condition where between placing
   * our own locks above and putting the Solaris share move below another client
   * puts a lock. We then end up with set locks from above and return with an
   * error code, the proper fix requires a sane cleanup function for the error
  ...
  if (obj->options.flags & OPTION_SHARE_RESERV) {
    shmd.f_access = (access & OPENACC_RD ? F_RDACC : 0) |
                    (access & OPENACC_WR ? F_WRACC : 0);
    if (shmd.f_access == 0)
      /* we must give an access mode, otherwise fcntl will complain */
      shmd.f_access = F_RDACC;
    shmd.f_deny =
        (access & OPENACC_DRD ? F_RDDNY : F_NODNY) | (access & OPENACC_DWR)
            ? F_WRDNY
            : 0;
    shmd.f_id = ofrefnum;
...
int afp_openfork(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  LOG(log_debug, logtype_afpd, "afp_openfork(\"%s\", %s, %s)",
      fullpathname(s_path->u_name), (fork == OPENFORK_DATA) ? "data" : "reso",
  ...
  if (ad_open(ofork->of_ad, upath, adflags, 0666) < 0) {
    ...
    case EISDIR:
      ...
      goto openfork_err;
    default:
      LOG(log_error, logtype_afpd, "afp_openfork(%s): ad_open: %s",
  ...
  if (ad_open(ofork->of_ad, upath, adflags, 0666) == 0) {
    ...
    if (ad_get_MD_flags(ofork->of_ad) & O_CREAT) {
      ...
      if ((id = get_id(vol, ofork->of_ad, st, dir->d_did, upath,
        ...
        LOG(log_error, logtype_afpd, "afp_createfile(\"%s\"): CNID error",
            upath);
        goto openfork_err;
      }
...
int afp_setforkparams(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  if (size < 0)
    return AFPERR_PARAM; /* Some MacOS don't return an error they just don't
                            change the size! */

  if (bitmap == (1 << FILPBIT_DFLEN) || bitmap == (1 << FILPBIT_EXTDFLEN)) {
    st_size = ad_size(ofork->of_ad, eid);
    err = -2;
    if (st_size > size && ad_tmplock(ofork->of_ad, eid, ADLOCK_WR, size,
    ...
    err = ad_rtruncate(ofork->of_ad,
                       mtoupath(ofork->of_vol, of_name(ofork), ofork->of_did,
                                utf8_encoding(obj)),
                       size);
    if (st_size > size)
      ad_tmplock(ofork->of_ad, eid, ADLOCK_CLR, size, st_size - size,
...
/* ---------------------- */
static int byte_lock(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  if (ad_lock(ofork->of_ad, eid, lockop, offset, length, ofork->of_refnum) <
      0) {
    switch (errno) {
    case EACCES:
    case EAGAIN:
...
static int read_fork(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
                     size_t *rbuflen, int is64) {
  DSI *dsi = obj->dsi;
  struct ofork *ofork;
  ...
  LOG(log_debug, logtype_afpd,
      ...
      ", size: %" PRIu64 ")",
      ofork->of_refnum, (ofork->of_flags & AFPFORK_DATA) ? "data" : "reso",
      offset, reqcount, size);
  ...
  *rbuflen = MIN(reqcount, dsi->server_quantum);
  ...
  LOG(log_debug, logtype_afpd,
      "afp_read(name: \"%s\", offset: %jd, reqcount: %jd): got %jd bytes from "
      "file",
...
int afp_flushfork(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  if (NULL == (ofork = of_find(ofrefnum))) {
    LOG(log_error, logtype_afpd,
        "afp_flushfork: of_find(%d) could not locate fork", ofrefnum);
    return (AFPERR_PARAM);
  }
...
int afp_syncfork(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  if (NULL == (ofork = of_find(ofrefnum))) {
    LOG(log_error, logtype_afpd,
        "afpd_syncfork: of_find(%d) could not locate fork", ofrefnum);
    return (AFPERR_PARAM);
  }
...
int flushfork(struct ofork *ofork) {
  ...
  if (ad_data_fileno(ofork->of_ad) != -1 &&
    ...
    LOG(log_error, logtype_afpd, "flushfork(%s): dfile(%d) %s", of_name(ofork),
        ad_data_fileno(ofork->of_ad), strerror(errno));
    err = -1;
  }
...
int afp_closefork(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  if (NULL == (ofork = of_find(ofrefnum))) {
    LOG(log_error, logtype_afpd,
        "afp_closefork: of_find(%d) could not locate fork", ofrefnum);
    return (AFPERR_PARAM);
  }
...
static ssize_t write_file(struct ofork *ofork, int eid, off_t offset,
                          char *rbuf, size_t rbuflen) {
  ssize_t cc;
  ...
  if ((cc = ad_write(ofork->of_ad, eid, offset, 0, rbuf, rbuflen)) < 0) {
    switch (errno) {
    case EDQUOT:
    case EFBIG:
    case ENOSPC:
    ...
    case EACCES:
      return AFPERR_ACCESS;
    default:
      LOG(log_error, logtype_afpd, "afp_write(%s): ad_write: %s",
          of_name(ofork), strerror(errno));
      return (AFPERR_PARAM);
    }
  }
 ...
 * in reqcount et al.
 */
static int write_fork(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
                      size_t *rbuflen, int is64) {
  struct ofork *ofork;
  ...
  if (obj->options.flags & OPTION_RECVFILE) {
    ...
    if ((cc = ad_recvfile(ofork->of_ad, eid, dsi->socket, offset, dsi->datasize,
      ...
      default:
        ...
        cc = AFPERR_MISC;
        LOG(log_error, logtype_afpd, "afp_write: ad_writefile: %s",
            strerror(errno));
      }
      *rbuflen = 0;
      if (obj->options.flags & OPTION_AFP_READ_LOCK)
