...
struct uam_mod *uam_load(AFPObj *obj, const char *path, const char *name) {
  ...
  if (mod->uam_fcn->uam_type != UAM_MODULE_SERVER) {
    LOG(log_error, logtype_afpd,
        "uam_load(%s): attempted to load a non-server module", name);
    goto uam_load_err;
  }

  /* version check would go here */

  if (!mod->uam_fcn->uam_setup || ((*mod->uam_fcn->uam_setup)(obj, name) < 0)) {
...
struct passwd *uam_getname(void *private, char *name, const int len) {
  ...
  if (obj->options.addomain ||
      (obj->options.ntdomain && obj->options.ntseparator)) {
    ...
    if (pwent) {
      ...
      if (len < MAXUSERLEN) {
        ...
        LOG(log_error, logtype_uams, "The name '%s' is longer than %d",
...
int uam_afpserver_option(void *private, const int what, void *option,
  ...
  case UAM_OPTION_CLIENTNAME: {
    ...
    if (getnameinfo(sa, sizeof(dsi->client), hbuf, sizeof(hbuf), NULL, 0, 0) ==
        0)
      *buf = hbuf;
    else
