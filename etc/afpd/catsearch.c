...
/* Removes checked items from top of directory stack. Returns index of the first
 * unchecked elements or -1. */
...
/* Looks up for an opened adouble structure, opens resource fork of selected
 * file.
...
/* -------------------- */
static struct finderinfo *unpack_finderinfo(struct vol *vol, struct path *path,
                                            struct adouble **adp,
...
static int crit_check(struct vol *vol, struct path *path) {
  ...
  if (S_ISDIR(path->st.st_mode)) {
     ...
     */
    if (!(path->m_name =
              utompath(vol, path->u_name, 0, utf8_encoding(vol->v_obj)))) {
      ...
      path->id = id;
      if (!(path->m_name =
                utompath(vol, path->u_name, id, utf8_encoding(vol->v_obj)))) {
        return 0;
      }
    }
  }
  ...
  if ((c1.rbitmap & (1 << FILPBIT_PDINFO))) {
    if ((size_t)(-1) ==
        (len = convert_charset(CH_UTF8_MAC, CH_UCS2, CH_UTF8, path->m_name,
                               strlen(path->m_name), convbuf, 512, &flags))) {
      goto crit_check_ret;
    }
...
#define VETO_STR                                                               \
  "./../.AppleDouble/.AppleDB/Network Trash "                                  \
  "Folder/TheVolumeSettingsFolder/TheFindByContentFolder/.AppleDesktop/"       \
  ".Parent/"
...
static int catsearch(const AFPObj *obj, struct vol *vol, struct dir *dir,
                     int rmatches, uint32_t *pos, char *rbuf, uint32_t *nrecs,
                     int *rsize, int ext) {
  static uint32_t cur_pos; /* Saved position index (ID) - used to remember
                              "position" across FPCatSearch calls */
  static DIR
  ...
  /* We need to initialize all mandatory structures/variables and change working
    ...
    while ((entry = readdir(dirpos)) != NULL) {
      ...
      case S_IFDIR:
        ...
        */
        ...
        path.d_dir =
            dircache_search_by_name(vol, currentdir, path.u_name, unlen);
        if (path.d_dir == NULL) {
          /* path.m_name is set by adddir */
          if ((path.d_dir = dir_add(vol, currentdir, &path, unlen)) == NULL) {
            result = AFPERR_MISC;
            goto catsearch_end;
          }
        }
 ...
 * @param ext       (r)  extended search flag
 */
static int catsearch_db(const AFPObj *obj, struct vol *vol, struct dir *dir,
                        const char *uname, int rmatches, uint32_t *pos,
  ...
  if (cur_pos == 0 || *pos == 0) {
    if (convert_charset(vol->v_volcharset, vol->v_volcharset, vol->v_maccharset,
    ...
    num_matches =
        cnid_find(vol->v_cdb, buffer, strlen(uname), resbuf, sizeof(resbuf));
    AFP_CNID_DONE();
    if (num_matches == -1) {
      result = AFPERR_MISC;
      goto catsearch_end;
  ...
  while (cur_pos < num_matches) {
    ...
    ccr = crit_check(vol, &path);
    if ((ccr & 1)) {
      LOG(log_debug, logtype_afpd, "catsearch_db: match: %s/%s", getcwdpath(),
          path.u_name);
...
/* -------------------------- */
static int catsearch_afp(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
                         size_t *rbuflen, int ext) {
  struct vol *vol;
  uint16_t vid;
  uint16_t spec_len;
  ...
  *rbuflen = 24;
  if ((c1.rbitmap & (1 << FILPBIT_PDINFO)) &&
    ...
    ret = catsearch(obj, vol, vol->v_root, rmatches, &catpos[0], rbuf + 24,
                    &nrecs, &rsize, ext);
