...
static int uam_gss_enabled() {
  ...
  return auth_uamfind(UAM_SERVER_LOGIN_EXT, "Client Krb v2",
...
static void status_flags(char *data, const int notif, const int ipok,
  ...
  status = AFPSRVRINFO_COPY | AFPSRVRINFO_SRVSIGNATURE | AFPSRVRINFO_SRVMSGS |
           AFPSRVRINFO_FASTBOZO | AFPSRVRINFO_SRVUTF8 | AFPSRVRINFO_EXTSLEEP;
...
static int status_server(char *data, const char *server,
  ...
  Obj = (char *)server;
  if ((size_t)-1 ==
      (len = convert_string(options->unixcharset, options->maccharset, Obj, -1,
                            buf, sizeof(buf)))) {
...
static size_t status_netaddress(char *data, int *servoffset, const DSI *dsi,
                                const struct afp_options *options) {
  char *begin;
  uint16_t offset;
 ...
 */
static size_t status_directorynames(char *data, int *diroffset, const DSI *dsi,
                                    const struct afp_options *options) {
  char *begin = data;
  uint16_t offset;
...
static size_t status_utf8servername(char *data, int *nameoffset, const DSI *dsi,
                                    const struct afp_options *options) {
  uint16_t namelen;
  size_t len;
  char *begin = data;
  ...
  if ((len = convert_string(options->unixcharset, CH_UTF8_MAC,
                            options->hostname, -1, data + sizeof(namelen),
...
void status_init(AFPObj *obj, DSI *dsi) {
  ...
  status_flags(status, options->flags & OPTION_SERVERNOTIF,
               (options->fqdn || ipok), options->passwdbits, 1, options->flags);
...
void set_signature(struct afp_options *options) {
  ...
  if (len == 0) {
    ...
    LOG(log_warning, logtype_afpd,
        "WARNING: signature string %s is very short !", options->signatureopt);
    goto server_signature_user;
  } else if (len > 16) {
    LOG(log_warning, logtype_afpd,
        "WARNING: signature string %s is very long !", options->signatureopt);
    len = 16;
    goto server_signature_user;
  } else {
    LOG(log_info, logtype_afpd, "signature string is %s.",
...
server_signature_auto:
  ...
  if (!stat(options->sigconffile, &tmpstat)) { /* conf file exists? */
    if ((fp = fopen(options->sigconffile, "r")) != NULL) { /* read open? */
      ...
      if ((fp = freopen(options->sigconffile, "a+", fp)) !=
          NULL) { /* append because not found */
...
server_signature_random:
  ...
  if (fp && header) { /* conf file is created or size=0 */
    ...
    fprintf(fp, "# \n");
    fprintf(fp, "# ServerSignature is unique identifier used to prevent "
                "logging on to\n");
    fprintf(fp, "# the same server twice.\n");
    fprintf(fp, "# \n");
    fprintf(fp, "# If setting \"signature = xxxxx\" in afp.conf, this file is "
                "not used.\n\n");
  }
...
server_signature_done:
  ...
  LOG(log_info, logtype_afpd,
      ...
      (options->signature)[0], (options->signature)[1], (options->signature)[2],
      (options->signature)[3], (options->signature)[4], (options->signature)[5],
      (options->signature)[6], (options->signature)[7], (options->signature)[8],
      (options->signature)[9], (options->signature)[10],
      (options->signature)[11], (options->signature)[12],
      (options->signature)[13], (options->signature)[14],
