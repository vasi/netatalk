...
static size_t mangle_extension(const struct vol *vol, const char *uname,
  ...
  if (p && p != uname) {
    ...
    size_t len =
        convert_charset(vol->v_volcharset, charset, vol->v_maccharset, p,
                        strlen(p), extension, MAX_EXT_LENGTH, &flags);
...
static char *demangle_checks(const struct vol *vol, char *uname,
  ...
  uint16_t flags;
  static char
      buffer[MAXPATHLEN + 2]; /* for convert_charset dest_len parameter +2 */
  size_t len;
  size_t mfilenamelen;
  ...
  if ((size_t)-1 ==
      (len = convert_charset(vol->v_volcharset, vol->v_maccharset, 0, uname,
                             strlen(uname), buffer, MAXPATHLEN, &flags))) {
    return mfilename;
  }
  ...
  if ((flags & CONV_REQMANGLE)) {
    if (len) {
      ...
      if ((size_t)-1 ==
          (len = convert_charset(vol->v_maccharset, CH_UTF8_MAC, 0, buffer, len,
                                 buffer, MAXPATHLEN, &flags))) {
        return mfilename;
      }
      /* Now compare the two names, they have to match the number of characters
       * in buffer */
      /* prefix can be longer than len, OSX might send us the first character(s)
       * of a     */
      /* decomposed char as the *last* character(s) before the #, so our match
       * below will */
      ...
      /* We couldn't convert the name to maccharset at all, so we'd expect a
       * name */
      /* in the "???#ID" form ... */
      if (!strncmp("???", mfilename, prefix)) {
        return uname;
      }
      /* ..but OSX might send us only the first characters of a decomposed
       * character. */
      ...
      if ((size_t)-1 ==
          (len = convert_charset(vol->v_volcharset, CH_UTF8_MAC, 0, uname,
                                 strlen(uname), buffer, MAXPATHLEN, &flags))) {
        return mfilename;
      }
...
static char *private_demangle(const struct vol *vol, char *mfilename,
  ...
  if ((dir = dirlookup(vol, id))) {
    ...
    if (!osx) {
      ...
      return demangle_checks(vol, cfrombstr(dir->d_u_name), mfilename, prefix,
                             t);
    }
...
char *mangle(const struct vol *vol, char *filename, size_t filenamelen,
  ...
  maxlen =
      (flags & 2) ? UTF8FILELEN_EARLY : MACFILELEN; /* was vol->max_filename */
  ...
  if (filenamelen + k + ext_len > maxlen) {
    ...
    size_t n = convert_charset(
        vol->v_volcharset, (flags & 2) ? CH_UTF8_MAC : vol->v_maccharset,
        vol->v_maccharset, uname, strlen(uname), m, maxlen - k - ext_len, &opt);
    m[n != (size_t)-1 ? n : 0] = 0;
  } else {
