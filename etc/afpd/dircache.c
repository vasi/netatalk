 ...
 *
 * The directory cache caches directories and files(!). The main reason for
 * having the cache is avoiding recursive walks up the path, querying the CNID
 * database each time, when we have to calculate the location of eg directory
 * with CNID 30, which is located in a dir with CNID 25, next CNID 20 and then
 * CNID 2 (the volume root as per AFP spec). If all these dirs where in the
 * cache, each database look up can be avoided. Additionally there's the element
 * "fullpath" in struct dir, which is used to avoid the recursion in any case.
 * Wheneveer a struct dir is initialized, the fullpath to the directory is
 * stored there.
 *
 * In order to speed up the CNID query for files too, which eg happens when a
 * directory is enumerated, files are stored too in the dircache. In order to
 * differentiate between files and dirs, we set the flag DIRF_ISFILE in struct
 * dir.d_flags for files.
 *
 * The most frequent codepatch that leads to caching is directory enumeration
 * (cf enumerate.c):
 * - if a element is a directory:
 *   (1) the cache is searched by dircache_search_by_name()
 *   (2) if it wasn't found a new struct dir is created and cached both from
 * within dir_add()
 * - for files the caching happens a little bit down the call chain:
 *   (3) first getfilparams() is called, which calls
 *   (4) getmetadata() where the cache is searched with
 * dircache_search_by_name() (5) if the element is not found (6) get_id()
 * queries the CNID from the database (7) then a struct dir is initialized via
 * dir_new() (note the fullpath arg is NULL) (8) finally added to the cache with
 * dircache_add() (2) of course does contain the steps 6,7 and 8.
 *
 * The dircache is a LRU cache, whenever it fills up we call dircache_evict
 * internally which removes DIRCACHE_FREE_QUANTUM elements from the cache.
 *
 * There is only one cache for all volumes, so of course we use the volume id in
 * hashing calculations.
 *
 * In order to avoid cache poisoning, we store the cached entries st_ctime from
 * stat in struct dir.ctime_dircache. Later when we search the cache we compare
 * the stored value with the result of a fresh stat. If the times differ, we
 * remove the cached entry and return "no entry found in cache". A elements
 * ctime changes when 1) the element is renamed (we loose the cached entry here,
 * but it will expire when the cache fills) 2) its a directory and an object has
 * been created therein 3) the element is deleted and recreated under the same
 * name Using ctime leads to cache eviction in case 2) where it wouldn't be
 * necessary, because the dir itself (name, CNID, ...) hasn't changed, but
 ...
 * It is a hashtable which we use to store "struct dir"s in. If the cache get
 * full, oldest entries are evicted in chunks of DIRCACHE_FREE.
 *
 ...
 * =========
 *
 * Sending SIGINT to a afpd child causes it to dump the dircache to a file
 * "/tmp/dircache.PID".
 */
...
static int hash_comp_didname(const void *k1, const void *k2) {
  ...
  return !(key1->d_vid == key2->d_vid && key1->d_pdid == key2->d_pdid &&
 ...
 *
 * Found cache entries are expunged if both the parent directory st_ctime and
 * the objects st_ctime are modified. This func builds on the fact, that all our
 * code only ever needs to and does search the dircache by CNID expecting
 * directories to be returned, but not files. Thus (1) if we find a file for a
 * given CNID we (1a) remove it from the cache (1b) return NULL indicating
 * nothing found (2) we can then use d_fullpath to stat the directory
 *
  ...
  if (cdir) {
    if (cdir->d_flags & DIRF_ISFILE) { /* (1) */
      LOG(log_debug, logtype_afpd,
          "dircache(cnid:%u): {not a directory:\"%s\"}", ntohl(cnid),
    ...
    LOG(log_debug, logtype_afpd, "dircache(cnid:%u): {not in cache}",
        ntohl(cnid));
 ...
 *
 * Found cache entries are expunged if both the parent directory st_ctime and
 * the objects st_ctime are modified.
 *
 * @param vol      (r) volume
...
struct dir *dircache_search_by_name(const struct vol *vol,
                                    const struct dir *dir, char *name,
                                    int len) {
  struct dir *cdir = NULL;
  struct dir key;
  struct stat st;
...
void dircache_remove(const struct vol *vol, struct dir *dir, int flags) {
  ...
  if (flags & QUEUE_INDEX) {
    ...
    dequeue(
        dir->qidx_node->prev); /* this effectively deletes the dequeued node */
    queue_count--;
  }
  ...
  if (flags & DIDNAME_INDEX) {
    if ((hn = hash_lookup(index_didname, dir)) == NULL) {
      LOG(log_error, logtype_afpd,
          "dircache_remove(%u,\"%s\"): not in didname index", ntohl(dir->d_did),
  ...
  if (flags & DIRCACHE) {
    if ((hn = hash_lookup(dircache, dir)) == NULL) {
      LOG(log_error, logtype_afpd,
          "dircache_remove(%u,\"%s\"): not in dircache", ntohl(dir->d_did),
          cfrombstr(dir->d_u_name));
      dircache_dump();
      AFP_PANIC("dircache_remove");
    }
...
void log_dircache_stat(void) {
  LOG(log_info, logtype_afpd,
      "dircache statistics: "
      "entries: %lu, lookups: %llu, hits: %llu, misses: %llu, added: %llu, "
      "removed: %llu, expunged: %llu, evicted: %llu",
      queue_count, dircache_stat.lookups, dircache_stat.hits,
      dircache_stat.misses, dircache_stat.added, dircache_stat.removed,
...
void dircache_dump(void) {
  ...
  while ((hn = hash_scan_next(&hs))) {
    ...
    fprintf(dump, "%05u: %3u  %6u  %6u %s    %s\n", i++, ntohs(dir->d_vid),
            ntohl(dir->d_pdid), ntohl(dir->d_did),
            dir->d_flags & DIRF_ISFILE ? "f" : "d", cfrombstr(dir->d_fullpath));
  }
