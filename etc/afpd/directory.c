...
int afp_errno;
/* As long as directory.c hasn't got its own init call, this get initialized in
 * dircache_init */
...
/*
 * dir_remove queues struct dirs to be freed here. We can't just delete them
 * immeidately eg in dircache_search_by_id, because a caller somewhere up the
 * stack might be referencing it. So instead:
...
static int netatalk_mkdir(const struct vol *vol, const char *name) {
  ...
  if (vol->v_flags & AFPVOL_UNIX_PRIV) {
    ...
    LOG(log_maxdebug, logtype_afpd,
        "netatalk_mkdir(\"%s\") {parent mode: %04o, vol umask: %04o}", name,
        st.st_mode, vol->v_umask);
    ...
    if (ostatat(dirfd, spath, &st, vol_syml_opt(vol)) == 0) {
      ...
      if (S_ISDIR(st.st_mode)) {
        if (AFP_OK != (err = copydir(vol, ddir, dirfd, spath, dpath)))
          goto copydir_done;
      } else if (AFP_OK != (err = copyfile(vol, vol, ddir, dirfd, spath, dpath,
                                           NULL, NULL))) {
        goto copydir_done;
  ...
  if (vol->v_obj->afp_version >= 30) {
    if (toUTF8) {
      if (dir->d_did == DIRDID_ROOT_PARENT) {
        /*
         * With uft8 volume name is utf8-mac, but requested path may be a
         * mangled longname. See #2611981. So we compare it with the longname
         * from the current volume and if they match we overwrite the requested
         * path with the utf8 volume name so that the following strcmp can
         * match.
         */
        ucs2_to_charset(vol->v_maccharset, vol->v_macname, temp,
                        AFPVOL_MACNAMELEN + 1);
        if (strcasecmp(ret->m_name, temp) == 0)
          ucs2_to_charset(CH_UTF8_MAC, vol->v_u8mname, ret->m_name,
                          AFPVOL_U8MNAMELEN);
      } else {
        /* toUTF8 */
        if (mtoUTF8(vol, ret->m_name, strlen(ret->m_name), temp, MAXPATHLEN) ==
            (size_t)-1) {
    ...
    LOG(log_maxdebug, logtype_afpd,
        "cname_mtouname('%s',did:%u) {demangled:'%s', fileid:%u}", ret->m_name,
        ntohl(dir->d_did), t, ntohl(fileid));

    if (t != ret->m_name) {
      ret->u_name = t;
      /* duplicate work but we can't reuse all convert_char we did in
       * demangle_osx flags weren't the same
       */
 ...
 *    AFPERR_NOOBJ: the dir was there when we stated it in cname, so we have a
 * race
...
static struct path *path_from_dir(struct vol *vol, struct dir *dir,
  ...
  case AFPERR_ACCESS:
    ...
    LOG(log_debug, logtype_afpd,
        ...
        cfrombstr(dir->d_fullpath), cfrombstr(curdir->d_fullpath), ret->u_name);
 ...
 *
 * Given a path like "/Volumes/volume/dir/subdir" in a volume "/Volumes/volume"
 * return a pointer to struct dir of "subdir".
...
struct dir *dirlookup_bypath(const struct vol *vol, const char *path) {
  ...
  for (int i = 0; i < l->qty; i++) { /* 3. */
    ...
    LOG(log_debug, logtype_afpd, "dirlookup_bypath: statpath: \"%s\"",
        cfrombstr(statpath));
    ...
    if ((dir = dircache_search_by_name(vol, /* 5. */
                                       dir, cfrombstr(l->entry[i]),
                                       blength(l->entry[i]))) == NULL) {

      AFP_CNID_START("cnid_add");
      cnid =
          cnid_add(vol->v_cdb, /* 6. */
                   &st, did, cfrombstr(l->entry[i]), blength(l->entry[i]), 0);
      AFP_CNID_DONE();
...
struct dir *dirlookup(const struct vol *vol, cnid_t did) {
  ...
  if ((ret = dircache_search_by_did(vol, did)) != NULL) { /* 2a */
    ...
    if (lstat(cfrombstr(ret->d_fullpath), &st) != 0) {
      LOG(log_debug, logtype_afpd,
          "dirlookup(did: %u, path: \"%s\"): lstat: %s", ntohl(did),
          cfrombstr(ret->d_fullpath), strerror(errno));
      switch (errno) {
      case ENOENT:
      case ENOTDIR:
  ...
  LOG(log_debug, logtype_afpd, "dirlookup(did: %u): stating \"%s\"", ntohl(did),
      cfrombstr(fullpath));
  ...
  /* Create struct dir */
  if ((ret = dir_new(mpath, upath, vol, pdid, did, fullpath, &st)) ==
      NULL) { /* 6 */
...
exit:
  ...
  if (ret)
    LOG(log_debug, logtype_afpd,
        "dirlookup(did: %u): RESULT: pdid: %u, path: \"%s\"", ntohl(ret->d_did),
        ntohl(ret->d_pdid), cfrombstr(ret->d_fullpath));
...
#define ENUMVETO                                                               \
  "./../Network Trash "                                                        \
  "Folder/TheVolumeSettingsFolder/TheFindByContentFolder/:2eDS_Store/"         \
  "Contents/Desktop Folder/Trash/Benutzer/"
 ...
 */
struct dir *dir_new(const char *m_name, const char *u_name,
                    const struct vol *vol, cnid_t pdid, cnid_t did,
                    bstring path, struct stat *st) {
  struct dir *dir;
  ...
  if (convert_string_allocate(
          ...
          CH_UCS2, m_name, -1, (char **)&dir->d_m_name_ucs2) == (size_t)-1) {
...
struct dir *dir_add(struct vol *vol, const struct dir *dir, struct path *path,
  ...
  ad_init(&ad, vol);
  if ((ad_open(&ad, path->u_name, ADFLAGS_HF | ADFLAGS_DIR | ADFLAGS_RDONLY)) ==
      0) /* 1 */
    adp = &ad;

  /* Get CNID */
  if ((id = get_id(vol, adp, &path->st, dir->d_did, path->u_name, len)) ==
      0) { /* 2 */
    err = 1;
    goto exit;
  }
  ...
  if ((cdir = dir_new(path->m_name, path->u_name, vol, dir->d_did, id, fullpath,
                      &path->st)) == NULL) { /* 3 */
    err = 4;
    goto exit;
  }
 ...
 * 11. If it's there and it's a file, it must should be the last element of the
 * requested path. Return with curdir = cnode name parent dir, struct path =
 * filename
 * 12. Treat symlinks like files, don't follow them
 * 13. If it's a dir:
 * 14. Search the dircache for it
 * 15. If it's not in the cache, create a struct dir for it and add it to the
 * cache
  ...
  *cpath += len + size;
  ...
  while (len) {       /* 3 */
    ...
    for (p = path; *data != 0 && len > 0; len--) {
      *p++ = *data++;
      if (p > &path[UTF8FILELEN_EARLY]) { /* FIXME safeguard, limit of early Mac
                                             OS X */
        afp_errno = AFPERR_PARAM;
        return NULL;
      }
    ...
    *p = 0; /* Terminate string */
    ...
    if (cname_mtouname(vol, dir, &ret, toUTF8) != 0) { /* 7 */
      LOG(log_error, logtype_afpd, "cname('%s'): error from cname_mtouname",
          path);
      return NULL;
    }
    ...
    if (check_name(vol, ret.u_name)) {
      ...
      if (vol->v_obj->options.flags & OPTION_VETOMSG) {
        bstring message = bformat("Attempt to access vetoed file or directory "
                                  "\"%s\" in directory \"%s\"",
                                  ret.u_name, bdata(dir->d_u_name));
        if (setmessage(bdata(message)) == 0)
          /* Client may make multiple attempts, only send the message the first
           * time */
          kill(getpid(), SIGUSR2);
        bdestroy(message);
      }
    ...
    if (movecwd(vol, cdir) < 0) { /* 16 */
      LOG(log_debug, logtype_afpd,
          ...
          cfrombstr(curdir->d_fullpath), cfrombstr(cdir->d_fullpath),
          strerror(errno));
      if (len == 0)
 ...
 * If we aren't the file's owner we can't change its perms when moving it and
 * smb nfs,... don't even try.
 */
...
int getdirparams(const AFPObj *obj, const struct vol *vol, uint16_t bitmap,
                 struct path *s_path, struct dir *dir, char *buf,
                 size_t *buflen) {
  struct maccess ma;
  struct adouble ad;
  ...
  if ((bitmap &
       ((1 << DIRPBIT_ATTR) | (1 << DIRPBIT_CDATE) | (1 << DIRPBIT_MDATE) |
    ...
    if (!ad_metadata(upath, ADFLAGS_DIR, &ad)) {
      ...
      if (ad.ad_mdp->adf_flags & O_CREAT) {
        ...
        if (s_path->m_name == NULL) {
          if ((s_path->m_name = utompath(vol, upath, dir->d_did,
  ...
  if (utf_nameoff) {
    ...
    data =
        set_name(vol, data, pdid, cfrombstr(dir->d_m_name), dir->d_did, utf8);
  }
  if (isad) {
    ad_close(&ad, ADFLAGS_HF);
  }
...
int setdirparams(struct vol *vol, struct path *path, uint16_t d_bitmap,
  ...
  LOG(log_debug, logtype_afpd, "setdirparams(\"%s\", bitmap: %02x)",
      path->u_name, bitmap);
  ...
  while (bitmap != 0) {
    ...
    case DIRPBIT_UNIXPR:
      if (vol_unix_priv(vol)) {
        set_upriv = true;
        memcpy(&owner, buf,
               sizeof(owner)); /* FIXME need to change owner too? */
        buf += sizeof(owner);
        memcpy(&group, buf, sizeof(group));
        buf += sizeof(group);
  ...
  if (d_bitmap & ((1 << DIRPBIT_ATTR) | (1 << DIRPBIT_CDATE) |
    ...
    ad_init(&ad, vol);
    if (ad_open(&ad, upath,
                ADFLAGS_HF | ADFLAGS_DIR | ADFLAGS_CREATE | ADFLAGS_RDWR,
                0777) != 0) {
      LOG(log_debug, logtype_afpd,
          "setdirparams(\"%s\", bitmap: %02x): need adouble", path->u_name,
          d_bitmap);
      return AFPERR_ACCESS;
    }
...
setdirparam_done:
  ...
  if (isad) {
    if (path->st_valid && !path->st_errno) {
      ...
      if (dir && dir->d_pdid) {
        ad_setid(&ad, st->st_dev, st->st_ino, dir->d_did, dir->d_pdid,
                 vol->v_stamp);
      }
    }
    if (ad_flush(&ad) != 0) {
  ...
  *rbuflen = 0;
  ...
  if (ntohl(did) == 2) {
    ...
    if (movecwd(vol, dir) < 0)
      ...
      /*
       * Assuming only OSens that have dirfd also may require fsyncing
       * directories in order to flush metadata e.g. Linux.
       */
    ...
    dfd = dirfd(dp);
    if (fsync(dfd) < 0)
      LOG(log_error, logtype_afpd, "afp_syncdir(%s):  %s", dir->d_u_name,
          strerror(errno));
 ...
 * newparent curdir
 * dirfd     -1 means ignore dirfd (or use AT_FDCWD), otherwise src is relative
 * to dirfd
 */
int renamedir(struct vol *vol, int dirfd, char *src, char *dst, struct dir *dir,
              struct dir *newparent, char *newname) {
  struct adouble ad;
  int err;
  ...
  *rbuflen = 0;
  ...
  case 3: /* unicode */
    ...
    if (id != 0) {
      ...
      len = convert_string_allocate(
          obj->options.unixcharset,
          ((!utf8) ? obj->options.maccharset : CH_UTF8_MAC), pw->pw_name, -1,
          &name);
    } else {
      len = 0;
      name = NULL;
  ...
  case 6: /* UUID -> groupname */
    ...
    case UUID_USER:
      if ((pw = getpwnam(name)) == NULL)
        return (AFPERR_NOITEM);
      LOG(log_debug, logtype_afpd, "afp_mapid: name:%s -> uid:%d", name,
          pw->pw_uid);
      id = htonl(UUID_USER);
    ...
    case UUID_GROUP:
      if ((gr = getgrnam(name)) == NULL)
        return (AFPERR_NOITEM);
      LOG(log_debug, logtype_afpd, "afp_mapid: group:%s -> gid:%d", name,
          gr->gr_gid);
      id = htonl(UUID_GROUP);
      memcpy(rbuf, &id, sizeof(id));
      rbuf += sizeof(id);
  ...
  *rbuflen = 0;
  ...
  if (len == 0)
    ...
    case 4:
      ...
      id = gr->gr_gid;
      LOG(log_debug, logtype_afpd,
          "afp_mapname: getgrnam for name: %s -> id: %d", ibuf, id);
      id = htonl(id);
      memcpy(rbuf, &id, sizeof(id));
      *rbuflen = sizeof(id);
      break;
