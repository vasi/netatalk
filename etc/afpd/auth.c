...
static struct uam_obj uam_login = {
    "", "", 0, {{NULL, NULL, NULL, NULL}}, &uam_login, &uam_login};
static struct uam_obj uam_changepw = {
    "", "", 0, {{NULL, NULL, NULL, NULL}}, &uam_changepw, &uam_changepw};
...
static int afp_null_nolog(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
                          size_t *rbuflen) {
  *rbuflen = 0;
  return (AFPERR_NOOP);
}
...
static int set_auth_switch(const AFPObj *obj, int expired) {
  ...
  if (expired) {
    ...
    uam_afpserver_action(AFP_CHANGEPW, UAM_AFPSERVER_PREAUTH, afp_changepw,
                         NULL);
  } else {
    ...
    case 33:
    case 32:
      uam_afpserver_action(AFP_GETEXTATTR, UAM_AFPSERVER_POSTAUTH,
                           afp_getextattr, NULL);
      uam_afpserver_action(AFP_SETEXTATTR, UAM_AFPSERVER_POSTAUTH,
                           afp_setextattr, NULL);
      uam_afpserver_action(AFP_REMOVEATTR, UAM_AFPSERVER_POSTAUTH,
                           afp_remextattr, NULL);
      uam_afpserver_action(AFP_LISTEXTATTR, UAM_AFPSERVER_POSTAUTH,
                           afp_listextattr, NULL);

    case 31:
      uam_afpserver_action(AFP_SYNCDIR, UAM_AFPSERVER_POSTAUTH, afp_syncdir,
                           NULL);
      uam_afpserver_action(AFP_SYNCFORK, UAM_AFPSERVER_POSTAUTH, afp_syncfork,
                           NULL);
    ...
    case 30:
      ...
      /* catsearch_ext uses the same packet as catsearch FIXME double check
         this, it wasn't true for enue enumerate_ext */
      uam_afpserver_action(AFP_CATSEARCH_EXT, UAM_AFPSERVER_POSTAUTH,
                           afp_catsearch_ext, NULL);
      uam_afpserver_action(AFP_GETSESSTOKEN, UAM_AFPSERVER_POSTAUTH,
                           afp_getsession, NULL);
      ...
      uam_afpserver_action(AFP_WRITE_EXT, UAM_AFPSERVER_POSTAUTH, afp_write_ext,
                           NULL);
      uam_afpserver_action(AFP_DISCTOLDSESS, UAM_AFPSERVER_POSTAUTH,
                           afp_disconnect, NULL);

    case 22:
      /*
       * If first connection to a server is done in classic AFP2.2 version is
       * used but OSX uses AFP3.x FPzzz command !
       */
...
static int login(AFPObj *obj, struct passwd *pwd, void (*logout)(void),
  ...
  LOG(log_note, logtype_afpd, "Login by %s (%s)", pwd->pw_name,
      afp_versions[afp_version_index].av_name);
  ...
  if (admin) {
    ...
    if (setegid(pwd->pw_gid) < 0 || seteuid(pwd->pw_uid) < 0) {
      LOG(log_error, logtype_afpd, "login: %s %s", pwd->pw_name,
          strerror(errno));
      return AFPERR_BADUAM;
    }
  }
  ...
  /* Some PAM module might have reset our signal handlers and timer, so we need
   * to reestablish them */
...
int afp_disconnect(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  if (send_fd(obj->ipc_fd, dsi->socket) != 0)
    goto exit;
  /* Now see what happens: either afpd master sends us SIGTERM because our
   * session */
...
int afp_logincont(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  ibuf += 2;
  ibuflen -= 2;
  err = afp_uam->u.uam_login.logincont(obj, &pwd, ibuf, ibuflen, rbuf, rbuflen);
...
int afp_changepw(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  ibuflen -= (ibuf - start);
  ret = uam->u.uam_changepw(obj, username, pwd, ibuf, ibuflen, rbuf, rbuflen);
  LOG(log_info, logtype_afpd, "password change %s.",
      (ret == AFPERR_AUTHCONT) ? "continued" : (ret ? "failed" : "succeeded"));
  if (ret == AFP_OK)
...
#define UAM_LIST(type)                                                         \
  (((type) == UAM_SERVER_LOGIN || (type) == UAM_SERVER_LOGIN_EXT)              \
       ? &uam_login                                                            \
       : (((type) == UAM_SERVER_CHANGEPW) ? &uam_changepw : NULL))
  ...
  while (p) {
    ...
    */
    if (stat(name, &st) == 0) {
      ...
      LOG(log_info, logtype_afpd, "uam: uam not found (status=%d)",
          stat(name, &st));
    }
