 ...
 *
 * "dsi" can be NULL in which case all DSI objects and the config object is
 * freed, otherwise its an afpd session child and only any unneeded DSI objects
 * are freed
 */
...
int configinit(AFPObj *obj) {
  ...
  LOG(log_debug, logtype_afpd,
      ...
      obj->options.hostname, obj->options.listen ? obj->options.listen : "-",
  ...
  if (obj->options.listen) {
    ...
    while (p) {
      if ((dsi = dsi_init(obj, obj->options.hostname, p, obj->options.port)) ==
          NULL)
        break;
   ...
   * We use getifaddrs() instead of if_nameindex() because the latter appears
   * still to be unable to return ipv4 addresses
   */
      ...
      for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if (family == AF_INET || family == AF_INET6) {
          if (getnameinfo(ifa->ifa_addr,
                          ...
                          interfaddr, NI_MAXHOST, NULL, 0,
                          NI_NUMERICHOST) != 0) {
            LOG(log_error, logtype_afpd,
                "getinterfaddr: getnameinfo() failed %s", gai_strerror(errno));
            continue;
          }

          if ((dsi = dsi_init(obj, obj->options.hostname, interfaddr,
                              obj->options.port)) == NULL)
            continue;
          ...
          *next = dsi;
          ...
          LOG(log_note, logtype_afpd,
              ...
              p, getip_string((struct sockaddr *)&dsi->server),
              getip_port((struct sockaddr *)&dsi->server));
        } /* if (family == AF_INET || family == AF_INET6) */
      }   /* for (ifa != NULL) */
      p = strtok_r(NULL, ", ", &savep);
   ...
   */
  if (dsi == NULL) {
    if ((dsi = dsi_init(obj, obj->options.hostname, NULL, obj->options.port)) ==
        NULL)
      EC_FAIL_LOG("no suitable network address found, use \"afp listen\" or "
                  "\"afp interfaces\"",
                  0);
    status_init(obj, dsi);
    *next = dsi;
  ...
  if ((r = atalk_iniparser_getstring(obj->iniconfig, INISEC_GLOBAL,
                                     "fce listener", NULL))) {
  ...
  r = atalk_iniparser_getstring(obj->iniconfig, INISEC_GLOBAL, "fce version",
                                "1");
