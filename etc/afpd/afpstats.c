...
static gpointer afpstats_thread(gpointer _data)
{
    ...
    if (!(bus = dbus_g_bus_get_private(DBUS_BUS_SESSION, ctxt, &error))) {
        LOG(log_error, logtype_afpd,"Couldn't connect to session bus: %s", error->message);
        return NULL;
    }
...
int afpstats_init(server_child_t *childs_in)
{
    ...
    thread = g_thread_new("afpstats_thread", afpstats_thread, NULL);
