 ...
 *
 * for every detected filesystem change a UDP packet is sent to an arbitrary
 * list of listeners. Each packet contains unix path of modified filesystem
 * element, event reason, and a consecutive event id (32 bit). Technically we
 * are UDP client and are sending out packets synchronuosly as they are created
 * by the afp functions. This should not affect performance measurably. The only
 * delaying calls occur during initialization, if we have to resolve non-IP
 * hostnames to IP. All numeric data inside the packet is network byte order, so
 * use ntohs / ntohl to resolve length and event id. Ideally a listener receives
 * every packet with no gaps in event ids, starting with event id 1 and mode
 * FCE_CONN_START followed by data events from id 2 up to 0xFFFFFFFF, followed
 * by 0 to 0xFFFFFFFF and so on.
 *
 * A gap or not starting with 1 mode FCE_CONN_START or receiving mode
 * FCE_CONN_BROKEN means that the listener has lost at least one filesystem
 * event
 *
...
static unsigned long fce_ev_enabled =
    (1 << FCE_FILE_MODIFY) | (1 << FCE_FILE_DELETE) | (1 << FCE_DIR_DELETE) |
    (1 << FCE_FILE_CREATE) | (1 << FCE_DIR_CREATE) | (1 << FCE_FILE_MOVE) |
    (1 << FCE_DIR_MOVE) | (1 << FCE_LOGIN) | (1 << FCE_LOGOUT);
...
static char *fce_event_names[] = {[FCE_FILE_MODIFY] = "FCE_FILE_MODIFY",
 ...
 * We don't give return code because all errors are handled internally (I
 * hope..)
 *
 * */
void fce_init_udp() {
 ...
 * */
static ssize_t build_fce_packet(const AFPObj *obj, char *iobuf, fce_ev_t event,
                                const char *path, const char *oldpath,
                                pid_t pid, const char *user,
                                uint32_t event_id) {
  char *p = iobuf;
  size_t pathlen;
  ssize_t datalen = 0;
...
static void send_fce_event(const AFPObj *obj, int event, const char *path,
  ...
  static uint32_t event_id =
      0; /* the unique packet couter to detect packet/data loss. Going from
            0xFFFFFFFF to 0x0 is a valid increment */
  static char *user;
  ...
  if (obj->fce_notify_script) {
    ...
    bstring cmd =
        bformat("%s -v %d -e %s -i %" PRIu32 "", obj->fce_notify_script,
                FCE_PACKET_VERSION, fce_event_names[event], event_id);
  ...
  for (int i = 0; i < udp_sockets; i++) {
    ...
    if (udp_entry->sock == -1) {
      ...
      /* Reopen socket */
      udp_entry->sock =
          socket(udp_entry->addrinfo.ai_family, udp_entry->addrinfo.ai_socktype,
      ...
      /* Okay, we have a running socket again, send server that we had a problem
       * on our side*/
      data_len = build_fce_packet(obj, (char *)iobuf, FCE_CONN_BROKEN, "", NULL,
                                  getpid(), user, 0);

      sendto(udp_entry->sock, iobuf, data_len, 0,
             (struct sockaddr *)&udp_entry->sockaddr,
             udp_entry->addrinfo.ai_addrlen);
    }

    /* build our data packet */
    data_len = build_fce_packet(obj, (char *)iobuf, event, path, oldpath,
                                getpid(), user, event_id);

    sent_data = sendto(udp_entry->sock, iobuf, data_len, 0,
                       (struct sockaddr *)&udp_entry->sockaddr,
                       udp_entry->addrinfo.ai_addrlen);
    ...
    if (sent_data != data_len) {
      ...
      LOG(log_error, logtype_fce,
          "send_fce_event: error sending packet to %s:%s, transferred %d of "
          "%d: %s",
          udp_entry->addr, udp_entry->port, sent_data, data_len,
          strerror(errno));
...
int fce_register(const AFPObj *obj, fce_ev_t event, const char *path,
  ...
  LOG(log_debug, logtype_fce, "register_fce(path: %s, event: %s)", path,
  ...
  if (fce_handle_coalescation(event, path)) {
    LOG(log_debug9, logtype_fce, "Coalesced fc event <%d> for <%s>", event,
        path);
    return AFP_OK;
  }
...
/*
 * API-Calls for file change api, called form outside (file.c directory.c
 * ofork.c filedir.c)
 * */
...
/*
 *
 * Extern connect to afpd parameter, can be called multiple times for multiple
 * listeners (up to MAX_UDP_SOCKS times)
 *
 * */
