...
static void afp_exit(int ret) { exit(ret); }
...
static bool init_listening_sockets(const AFPObj *config) {
  ...
  asev =
      asev_init(config->options.connections + numlisteners + ASEV_THRESHHOLD);
  if (asev == NULL) {
    return false;
  }
  ...
  while ((pid = waitpid(WAIT_ANY, &status, WNOHANG)) > 0) {
    if (WIFEXITED(status)) {
      if (WEXITSTATUS(status))
        LOG(log_info, logtype_afpd, "child[%d]: exited %d", pid,
            WEXITSTATUS(status));
      else
...
static int setlimits(void) {
  ...
  if (getrlimit(RLIMIT_NOFILE, &rlim) != 0) {
    LOG(log_warning, logtype_afpd,
        "setlimits: reading current limits failed: %s", strerror(errno));
    return -1;
  }
...
int main(int ac, char **av) {
  ...
  if (!(server_children = server_child_alloc(obj.options.connections))) {
    LOG(log_error, logtype_afpd, "main: server_child alloc: %s",
        strerror(errno));
  ...
  while (1) {
    ...
    for (int i = 0; i < asev->used; i++) {
      if (asev->fdset[i].revents & (POLLIN | POLLERR | POLLHUP | POLLNVAL)) {
        ...
        case LISTEN_FD:
          if ((child = dsi_start(&obj, (DSI *)(asev->data[i].private),
                                 server_children))) {
...
static afp_child_t *dsi_start(AFPObj *obj, DSI *dsi,
  ...
  if (dsi_getsession(dsi, server_children, obj->options.tickleval, &child) !=
      0) {
    LOG(log_error, logtype_afpd, "dsi_start: session error: %s",
        strerror(errno));
    return NULL;
  }
