...
int afp_getfildirparams(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  LOG(log_debug, logtype_afpd,
      "getfildirparams(vid:%u, did:%u, f/d:%04x/%04x) {cwdid:%u, cwd: %s, "
      "name:'%s'}",
      ntohs(vid), ntohl(dir->d_did), fbitmap, dbitmap, ntohl(curdir->d_did),
      cfrombstr(curdir->d_fullpath), s_path->u_name);
  ...
  if (S_ISDIR(st->st_mode)) {
    ...
    *(rbuf + 2 * sizeof(uint16_t)) = (char)FILDIRBIT_ISDIR;
    ...
    if (fbitmap && AFP_OK != (ret = getfilparams(
                                  obj, vol, fbitmap, s_path, curdir,
...
*/
static int moveandrename(const AFPObj *obj, struct vol *vol, struct dir *sdir,
                         int sdir_fd, char *oldname, char *newname, int isdir) {
  ...
  if (!isdir) {
    ...
    if ((oldunixname = strdup(
             ctoupath(vol, dirlookup(vol, sdir->d_pdid), oldname))) == NULL)
      return AFPERR_PARAM;
    adflags = ADFLAGS_DIR;
  }
   ...
   *   b) fchdir sdir_fd before eg ad_open or use *at functions where
   * appropriate
   */
  ...
  if (isdir)
    LOG(log_debug, logtype_afpd, "moveandrename(\"%s\" -> \"%s/%s\")",
        oldunixname, bdata(curdir->d_fullpath), upath);
  else
    LOG(log_debug, logtype_afpd, "moveandrename(\"%s/%s\" -> \"%s/%s\")",
  ...
  if ((!isdir && curdir == sdir) || (isdir && curdir->d_did == sdir->d_pdid)) {
    ...
    if (stat(upath, st) == 0) {
      if (!stat(oldunixname, &nst) &&
          !(nst.st_dev == st->st_dev && nst.st_ino == st->st_ino)) {
        /* not the same file */
        rc = AFPERR_EXIST;
        goto exit;
      }
  ...
  if (rc == AFP_OK && id) {
    ...
    if (cacheddir) {
      LOG(log_warning, logtype_afpd, "Still cached: \"%s/%s\"", getcwdpath(),
          upath);
 ...
 *
 * If the volume option delete veto files is set, this function recursively
 * scans the directory "upath" for vetoed files and tries deletes these, the it
 * will try to delete the directory. That may fail if the directory contains
 * normal files that aren't vetoed.
 *
 * @returns 0 if the directory upath and all of its contents were deleted,
 * otherwise -1. If the volume option is not set it returns -1.
 */
int delete_vetoed_files(struct vol *vol, const char *upath, bool in_vetodir) {
  ...
  while ((de = readdir(dp))) {
    ...
    if (stat(de->d_name, &sb) != 0) {
      LOG(log_error, logtype_afpd, "delete_vetoed_files(\"%s/%s\"): %s", upath,
    ...
    if (vetoed) {
      LOG(log_debug, logtype_afpd,
          "delete_vetoed_files(\"%s/%s\"): deleting vetoed file", upath,
          de->d_name);
  ...
  *rbuflen = 0;
  ...
  if (path_isadir(s_path)) {
    ...
    if (s_path->st_valid && s_path->st_errno == ENOENT) {
      ...
      if ((cachedfile =
               dircache_search_by_name(vol, dir, upath, strlen(upath)))) {
...
char *ctoupath(const struct vol *vol, struct dir *dir, char *name) {
  ...
  return absupath(vol, dir,
                  mtoupath(vol, name, dir->d_did, utf8_encoding(vol->v_obj)));
}
