 ...
 * @returns 0 if this message is being set the first time, return 1 if the
 * preceeding message was the same
 */
...
void readmessage(AFPObj *obj) {
  ...
  if (message != NULL) {
    ...
    if (rc < 0) {
      LOG(log_error, logtype_afpd, "Error deleting %s: %s", filename,
          strerror(rc));
    }
#ifdef DEBUG
    else {
...
int afp_getsrvrmesg(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen += sizeof(bitmap);
  ...
  if (msglen) {
    if ((size_t)-1 == (outlen = convert_string(
