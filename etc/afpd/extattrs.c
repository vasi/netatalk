  ...
  if (ad_metadata(uname, adflags, adp) != 0) {
    ...
    case EACCES:
      LOG(log_error, logtype_afpd,
          "afp_listextattr(%s): %s: check resource fork permission?", uname,
          strerror(errno));
      return AFPERR_ACCESS;
    default:
      LOG(log_error, logtype_afpd,
          "afp_listextattr(%s): error getting metadata: %s", uname,
          strerror(errno));
      return AFPERR_MISC;
    }
  } else {
    ...
    /* Now check for Ressource fork and add virtual EA "com.apple.ResourceFork"
     * if size > 0 */
    LOG(log_debug7, logtype_afpd,
        "afp_listextattr(%s): Ressourcefork size: %llu", uname, adp->ad_rlen);

    if (adp->ad_rlen > 0) {
      LOG(log_debug7, logtype_afpd,
          "afp_listextattr(%s): sending com.apple.RessourceFork.", uname);
...
static char *to_stringz(char *ibuf, uint16_t len) {
  ...
  /* we must copy the name as its not 0-terminated and I DONT WANT TO WRITE to
   * ibuf */
...
int afp_getextattr(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen += sizeof(bitmap);
  ...
  if (path_isadir(s_path)) {
    LOG(log_debug, logtype_afpd, "afp_getextattr(%s): is a dir",
        s_path->u_name);
  } else {
    LOG(log_debug, logtype_afpd, "afp_getextattr(%s): is a file",
        s_path->u_name);
    opened = of_findname(vol, s_path);
    if (opened) {
      adp = opened->of_ad;
...
int afp_setextattr(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  if (path_isadir(s_path)) {
    LOG(log_debug, logtype_afpd, "afp_setextattr(%s): is a dir",
        s_path->u_name);
  } else {
    LOG(log_debug, logtype_afpd, "afp_setextattr(%s): is a file",
        s_path->u_name);
    opened = of_findname(vol, s_path);
    if (opened) {
      adp = opened->of_ad;
...
int afp_remextattr(AFPObj *obj, char *ibuf, size_t ibuflen, char *rbuf,
  ...
  *rbuflen = 0;
  ...
  if (path_isadir(s_path)) {
    LOG(log_debug, logtype_afpd, "afp_remextattr(%s): is a dir",
        s_path->u_name);
  } else {
    LOG(log_debug, logtype_afpd, "afp_remextattr(%s): is a file",
        s_path->u_name);
    opened = of_findname(vol, s_path);
    if (opened) {
      adp = opened->of_ad;
