...
int dbd_delete(DBD *dbd, struct cnid_dbd_rqst *rqst, struct cnid_dbd_rply *rply,
  ...
  case DBIF_IDX_DIDNAME:
    ...
    if (rc) {
      ...
      LOG(log_debug, logtype_cnid,
          "cnid_delete: DID: %lu, name: %s not in database", ntohl(rqst->did),
          rqst->name);
      rply->result = CNID_DBD_RES_NOTFOUND;
    }
    break;
  default:
    ...
    if ((rc = dbif_del(dbd, DBIF_CNID, &key, 0)) < 0) {
      LOG(log_error, logtype_cnid,
          "dbd_delete: Unable to delete entry for CNID %u", ntohl(rqst->cnid));
      rply->result = CNID_DBD_RES_ERR_DB;
      return -1;
    }
    if (rc) {
      LOG(log_debug, logtype_cnid, "cnid_delete: CNID %u deleted",
          ntohl(rqst->cnid));
      rply->result = CNID_DBD_RES_OK;
    } else {
