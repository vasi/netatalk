...
int main(int argc, char **argv) {
  ...
  if (vol->v_cdb == NULL) {
    dbd_log(LOGSTD, "Cant initialize CNID database connection for %s",
            vol->v_path);
    exit(EXIT_FAILURE);
  }
  ...
  if ((flags & DBD_FLAGS_V2TOEA) && (vol->v_adouble != AD_VERSION_EA)) {
    dbd_log(LOGSTD, "Can't run adouble:v2 to adouble:ea conversion because not "
                    "an adouble:ea volume");
    exit(EXIT_FAILURE);
  }
