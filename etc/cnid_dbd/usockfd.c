...
int usockfd_create(char *usock_fn, mode_t mode, int backlog) {
  ...
  if (bind(sockfd, (struct sockaddr *)&addr, sizeof(struct sockaddr_un)) < 0) {
    LOG(log_error, logtype_cnid, "error binding to socket for %s: %s", usock_fn,
        strerror(errno));
    return -1;
  }

  if (listen(sockfd, backlog) < 0) {
    LOG(log_error, logtype_cnid, "error in listen for %s: %s", usock_fn,
        strerror(errno));
    return -1;
  }
...
int tsockfd_create(char *host, char *port, int backlog) {
  ...
  for (p = servinfo; p != NULL; p = p->ai_next) {
    if ((sockfd = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1) {
      LOG(log_info, logtype_cnid, "tsockfd_create: socket: %s",
          strerror(errno));
      continue;
    }
...
#define SOL_TCP IPPROTO_TCP
  ...
  if (p == NULL) {
    LOG(log_error, logtype_cnid,
        "tsockfd_create: no suitable network config %s:%s", host, port);
...
int usockfd_check(int sockfd, const sigset_t *sigset) {
  ...
  if (ret) {
    ...
    if ((fd = accept(sockfd, NULL, &size)) < 0) {
      if (errno == EINTR)
        return 0;
      LOG(log_error, logtype_cnid, "error in accept: %s", strerror(errno));
      return -1;
    }
    return fd;
  } else
