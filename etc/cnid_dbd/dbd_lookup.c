...
...are the expected results of certain operations. (f) is the speced CNID, in
some cases it's only intermediate as described in the text and is overridden by
another spec.
...
int dbd_lookup(DBD *dbd, struct cnid_dbd_rqst *rqst,
  ...
  LOG(log_maxdebug, logtype_cnid,
      "dbd_lookup(name:'%s', did:%u, dev/ino:0x%llx/0x%llx) {devino: %u, "
      "didname: %u}",
  ...
  if (!devino && !didname) {
    ...
    LOG(log_debug, logtype_cnid,
        "dbd_lookup: name: '%s', did: %u, dev/ino: 0x%llx/0x%llx is not in the "
        "CNID database",
        rqst->name, ntohl(rqst->did), (unsigned long long)rqst->dev,
  ...
  if ((devino && (type_devino != rqst->type)) ||
    ...
    if (devino && (type_devino != rqst->type)) {
      ...
      LOG(log_debug, logtype_cnid,
          "dbd_lookup(name:'%s', did:%u, dev/ino:0x%llx/0x%llx): type mismatch "
          "for devino",
          rqst->name, ntohl(rqst->did), (unsigned long long)rqst->dev,
          (unsigned long long)rqst->ino);
      ...
      if (rc < 0) {
        LOG(log_error, logtype_cnid,
            "dbd_lookup(name:'%s', did:%u, dev/ino:0x%llx/0x%llx): error "
            "deleting type mismatch for devino",
            rqst->name, ntohl(rqst->did), (unsigned long long)rqst->dev,
            (unsigned long long)rqst->ino);
        return -1;
      }
    ...
    if (didname && (type_didname != rqst->type)) {
      ...
      if (rc < 0) {
        LOG(log_error, logtype_cnid,
            "dbd_lookup(name:'%s', did:%u, dev/ino:0x%llx/0x%llx): error "
            "deleting type mismatch for didname",
            rqst->name, ntohl(rqst->did), (unsigned long long)rqst->dev,
            (unsigned long long)rqst->ino);
        return -1;
      }
  ...
  if (devino && didname && id_devino == id_didname) {
    /* everything is fine */
    LOG(log_debug, logtype_cnid,
        "dbd_lookup(DID:%u/'%s',0x%llx/0x%llx): Got CNID: %u", ntohl(rqst->did),
        rqst->name, (unsigned long long)rqst->dev,
  ...
  if (devino && didname && id_devino != id_didname) {
    ...
    LOG(log_debug, logtype_cnid,
        "dbd_lookup: CNID mismatch: (DID:%u/'%s') --> %u , (0x%llx/0x%llx) --> "
        "%u",
  ...
  if (!didname) {
    LOG(log_debug, logtype_cnid,
        "dbd_lookup(CNID hint: %u, DID:%u, \"%s\", 0x%llx/0x%llx): CNID "
        "resolve problem: server side rename oder reused inode",
  ...
  if (!devino) {
    LOG(log_debug, logtype_cnid,
        "dbd_lookup(DID:%u/'%s',0x%llx/0x%llx): CNID resolve problem: changed "
        "dev/ino",
  ...
  /* This is also a catch all if we've forgot to catch some possibility with the
   * preceding ifs*/
  if (!update) {
    rply->result = CNID_DBD_RES_NOTFOUND;
    return 1;
