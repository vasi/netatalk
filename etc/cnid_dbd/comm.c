...
/* ------------
   nbe of clients
*/
int comm_nbe(void) { return fds_in_use; }
...
int comm_rcv(struct cnid_dbd_rqst *rqst, time_t timeout,
  ...
  if ((b = readt(cur_fd, rqst, sizeof(struct cnid_dbd_rqst), 1,
                 CNID_DBD_TIMEOUT)) != sizeof(struct cnid_dbd_rqst)) {
    if (b)
  ...
  rqst->name = nametmp;
  if (rqst->namelen && readt(cur_fd, (char *)rqst->name, rqst->namelen, 1,
                             CNID_DBD_TIMEOUT) != rqst->namelen) {
    LOG(log_error, logtype_cnid, "error reading message name: %s",
        strerror(errno));
    invalidate_fd(cur_fd);
    return 0;
  }
...
int comm_snd(struct cnid_dbd_rply *rply) {
  ...
  if (write(cur_fd, rply->name, rply->namelen) != rply->namelen) {
    LOG(log_error, logtype_cnid, "error writing message name: %s",
        strerror(errno));
    invalidate_fd(cur_fd);
    return 0;
  }
