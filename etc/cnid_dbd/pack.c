...
int idxname(DB *dbp, const DBT *pkey, const DBT *pdata, DBT *skey) {
  ...
  if (convert_charset(volume->v_volcharset, volume->v_volcharset,
                      volume->v_maccharset, (char *)pdata->data + CNID_NAME_OFS,
                      strlen((char *)pdata->data + CNID_NAME_OFS), buffer,
