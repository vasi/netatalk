...
static char *netatalk_dirs[] = {".AppleDB", ".AppleDesktop", NULL};
static char *special_dirs[] = {".zfs", NULL};
...
static char *utompath(char *upath) {
  ...
  /* convert charsets */
  if ((size_t)-1 == (outlen = convert_charset(vol->v_volcharset, CH_UTF8_MAC,
                                              vol->v_maccharset, u, outlen,
                                              mpath, MAXPATHLEN, &flags))) {
...
static int update_cnid(cnid_t did, const struct stat *sp, const char *oldname,
  ...
  /* Update the database */
  if (cnid_update(vol->v_cdb, id, sp, did, (char *)newname, strlen(newname)) <
      0)
    return -1;
...
static int check_adfile(const char *fname, const struct stat *st,
  ...
  *newname = NULL;
  ...
  if ((ret = access(adname, F_OK)) != 0) {
    if (errno != ENOENT) {
      dbd_log(LOGSTD, "Access error for ad-file '%s/%s': %s", cwdbuf, adname,
              strerror(errno));
      return -1;
    }
    ...
    if ((ret = ad_open(&ad, fname, adflags | ADFLAGS_CREATE | ADFLAGS_RDWR,
                       0666)) != 0) {
      dbd_log(LOGSTD, "Error creating AppleDouble file '%s/%s': %s", cwdbuf,
    ...
    if (ad_open(&ad, fname, adflags | ADFLAGS_RDONLY) != 0) {
      dbd_log(LOGSTD, "Error opening AppleDouble file for '%s/%s'", cwdbuf,
              fname);
      return -1;
    }
    ad_close(&ad, ADFLAGS_HF);
...
static void remove_eafiles(const char *name, struct ea *ea) {
  ...
  if ((chdir(ADv2_DIRNAME)) != 0) {
    dbd_log(LOGSTD, "Couldn't chdir to '%s/%s': %s", cwdbuf, ADv2_DIRNAME,
            strerror(errno));
    return;
  }

  if ((dp = opendir(".")) == NULL) {
    dbd_log(LOGSTD, "Couldn't open the directory '%s/%s': %s", cwdbuf,
  ...
  while ((ep = readdir(dp))) {
    if (strstr(ep->d_name, eaname) != NULL) {
      dbd_log(LOGSTD, "Removing EA file: '%s/%s/%s'", cwdbuf, ADv2_DIRNAME,
              ep->d_name);
      if ((unlink(ep->d_name)) != 0) {
        dbd_log(LOGSTD, "Error unlinking EA file '%s/%s/%s': %s", cwdbuf,
                ADv2_DIRNAME, ep->d_name, strerror(errno));
      }
    } /* if */
  }   /* while */
...
static int check_eafiles(const char *fname) {
  ...
  while (count < ea.ea_count) {
    ...
    if (lstat(eaname, &st) != 0) {
      ...
      remove = 1;
      if ((unlink(eaname)) != 0)
        dbd_log(LOGSTD, "Error removing EA file '%s/%s': %s", cwdbuf, eaname,
                strerror(errno));
    }

    if (remove) {
      /* Be CAREFUL here! This should do what ea_delentry does. ea_close relies
       * on it !*/
...
static int check_addir(int volroot) {
  ...
  if ((addir_ok = access(ADv2_DIRNAME, F_OK)) != 0) {
    if (errno != ENOENT) {
      dbd_log(LOGSTD, "Access error in directory %s: %s", cwdbuf,
              strerror(errno));
      return -1;
    }
  ...
  if ((adpar_ok = access(vol->ad_path(".", ADFLAGS_DIR), F_OK)) != 0) {
    if (errno != ENOENT) {
      dbd_log(LOGSTD, "Access error on '%s/%s': %s", cwdbuf,
              vol->ad_path(".", ADFLAGS_DIR), strerror(errno));
      return -1;
    }
  ...
  if ((addir_ok != 0) || (adpar_ok != 0)) {
    ...
    if (dbd_flags & DBD_FLAGS_SCAN) {
      /* Yes:  missing .Parent is not a problem, but missing ad-dir
         causes later checking of ad-files to fail. So we have to return
         appropriately */
      if (addir_ok != 0)
        return -1;
      else /* (adpar_ok != 0) */
...
/*
  Check if file cotains "::EA" and if it does check if its correspondig data
  fork exists. Returns: 0 = name is not an EA file 1 = name is an EA file and no
  problem was found -1 = name is an EA file and data fork is gone
 */
  ...
  if (vol->v_vfs_ea == AFPVOL_EA_AD) {
    ...
    if ((namep = strstr(namedup, "::EA")) == NULL) {
      ...
      if ((access(pname, F_OK)) == 0) {
        ...
        ret = -1;
        if (errno != ENOENT) {
          dbd_log(LOGSTD, "Access error for file '%s/%s': %s", cwdbuf, name,
                  strerror(errno));
          goto ea_check_done;
        }

        /* Orphaned EA file*/
        dbd_log(LOGSTD, "Orphaned Extended Attribute file '%s/%s/%s'", cwdbuf,
                ADv2_DIRNAME, name);
...
static int read_addir(void) {
  ...
  while ((ep = readdir(dp))) {
    ...
    if ((lstat(ep->d_name, &st)) < 0) {
      dbd_log(LOGSTD,
              "Lost file or dir while enumeratin dir '%s/%s/%s', probably "
              "removed: %s",
    ...
    if ((access(pname, F_OK)) != 0) {
      if (errno != ENOENT) {
        dbd_log(LOGSTD, "Access error for file '%s/%s': %s", cwdbuf, pname,
                strerror(errno));
        continue;
      }
      /* Orphaned ad-file*/
      dbd_log(LOGSTD, "Orphaned AppleDoube file '%s/%s/%s'", cwdbuf,
              ADv2_DIRNAME, ep->d_name);
...
static cnid_t check_cnid(const char *name, cnid_t did, struct stat *st,
  ...
  if (ADFILE_OK) {
    ...
    if (ad_open(&ad, name, adflags | ADFLAGS_RDWR) != 0) {
      ...
      if (ad_cnid == CNID_INVALID)
        ...
        dbd_log(LOGDEBUG, "CNID from .AppleDouble file for '%s/%s': %u", cwdbuf,
                name, ntohl(ad_cnid));
      ad_close(&ad, ADFLAGS_HF);
    }
  }
  ...
  if (ad_cnid != db_cnid && adfile_ok == 0) {
    ...
    if (ad_open(&ad, name, adflags | ADFLAGS_HF | ADFLAGS_RDWR) != 0) {
      dbd_log(LOGSTD, "Error opening AppleDouble file for '%s/%s': %s", cwdbuf,
              name, strerror(errno));
      return CNID_INVALID;
    }
...
static int dbd_readdir(int volroot, cnid_t did) {
  ...
  /* Check AppleDouble files in AppleDouble folder, but only if it exists or
   * could be created */
  if (ADDIR_OK)
    if ((read_addir()) != 0)
      if (!(dbd_flags & DBD_FLAGS_SCAN))
  ...
  while ((ep = readdir(dp))) {
    ...
    if ((statcount % 10000) == 0) {
      if (dbd_flags & DBD_FLAGS_STATS)
        dbd_log(LOGSTD, "Scanned: %10llu, time: %10llu s", statcount,
                (unsigned long long)(time(NULL) - t));
    }
    ...
    **************************************************************************/
    if (S_ISDIR(st.st_mode) &&
        cnid) { /* If we have no cnid for it we cant enter recursion */
      strcat(cwdbuf, "/");
      strcat(cwdbuf, name);
      dbd_log(LOGDEBUG, "Entering directory: %s", cwdbuf);
      if (-1 == (cwd = open(".", O_RDONLY))) {
        dbd_log(LOGSTD, "Cant open directory '%s': %s", cwdbuf,
                strerror(errno));
        continue;
      }
      if (0 != chdir(name)) {
        dbd_log(LOGSTD, "Cant chdir to directory '%s': %s", cwdbuf,
                strerror(errno));
        close(cwd);
        continue;
      }
