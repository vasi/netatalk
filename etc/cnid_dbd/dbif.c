...
/*!
 * Get the db stamp which is the st_ctime of the file "cnid2.db" and store it in
 * buffer
 */
 ...
 *
 * @returns -1 on error, 0 if theres no rootinfo key yet, 1 if *version is
 * returned
 */
...
/*!
 * Upgrade CNID database versions, initialize rootinfo key as as necessary in
 * dbif_setversion()
 *
...
static int dbif_logautorem(DBD *dbd) {
  ...
  if (logfiles != NULL) {
    for (file = logfiles; *file != NULL; file++) {
      if (unlink(*file) < 0)
        LOG(log_warning, logtype_cnid, "Error removing stale logfile %s: %s",
            *file, strerror(errno));
    }
    free(logfiles);
  }
...
/*
   We must open the db_env with an absolute pathname, as `dbd` keeps chdir'ing,
   which breaks e.g. bdb logfile-rotation with relative pathnames. But still we
   use relative paths with DB_ERRLOGFILE in order to avoid creating absolute
   paths by copying. Both have no problem with a relative path.
*/
int dbif_env_open(DBD *dbd, struct db_param *dbp, uint32_t dbenv_oflags) {
  ...
  if (dbenv_oflags & DB_RECOVER) {
    ...
    if ((ret = dbd->db_env->close(dbd->db_env, 0))) {
      LOG(log_error, logtype_cnid,
          "error closing DB environment after recovery: %s", db_strerror(ret));
      dbd->db_env = NULL;
      return -1;
    }
    dbd->db_errlog = NULL;
    ...
    if ((ret = db_env_create(&dbd->db_env, 0))) {
      LOG(log_error, logtype_cnid,
          "error creating DB environment after recovery: %s", db_strerror(ret));
      dbd->db_env = NULL;
      return -1;
    }
  ...
  if ((ret = dbd->db_env->set_cachesize(dbd->db_env, 0, 1024 * dbp->cachesize,
                                        0))) {
    LOG(log_error, logtype_cnid,
        "error setting DB environment cachesize to %i: %s", dbp->cachesize,
        db_strerror(ret));
    dbd->db_env->close(dbd->db_env, 0);
    dbd->db_env = NULL;
    return -1;
  ...
  if ((ret = dbd->db_env->set_lk_max_locks(dbd->db_env, dbp->maxlocks))) {
    LOG(log_error, logtype_cnid,
        "error setting DB environment maxlocks to %i: %s", 10000,
        db_strerror(ret));
    dbd->db_env->close(dbd->db_env, 0);
    dbd->db_env = NULL;
    return -1;
  ...
  if ((ret = dbd->db_env->set_lk_max_objects(dbd->db_env, dbp->maxlockobjs))) {
    LOG(log_error, logtype_cnid,
        "error setting DB environment max lockobjects to %i: %s", 10000,
        db_strerror(ret));
    dbd->db_env->close(dbd->db_env, 0);
    dbd->db_env = NULL;
    return -1;
  ...
  if ((ret =
           dbd->db_env->open(dbd->db_env, dbd->db_envhome, dbenv_oflags, 0))) {
    LOG(log_error, logtype_cnid,
        "error opening DB environment after recovery: %s", db_strerror(ret));
    dbd->db_env->close(dbd->db_env, 0);
    dbd->db_env = NULL;
    return -1;
  }
  ...
  if (dbp->logfile_autoremove) {
    ...
    if ((ret =
             dbd->db_env->log_set_config(dbd->db_env, DB_LOG_AUTO_REMOVE, 1))) {
...
int dbif_open(DBD *dbd, struct db_param *dbp, int reindex) {
  ...
  if (dbd->db_envhome) {
    ...
    /* chdir to db_envhome. makes it easier checking for old db files and
     * creating db_errlog file  */
    if ((chdir(dbd->db_envhome)) != 0) {
      LOG(log_error, logtype_cnid, "error chdiring to db_env '%s': %s",
          dbd->db_envhome, strerror(errno));
      return -1;
    ...
    if ((stat(dbd->db_filename, &st)) == 0) {
      ...
      if ((ret = upgrade_db->upgrade(upgrade_db, dbd->db_filename, 0))) {
        LOG(log_error, logtype_cnid, "error upgarding database: %s",
            db_strerror(ret));
        return -1;
      }
      if ((ret = upgrade_db->close(upgrade_db, 0))) {
        LOG(log_error, logtype_cnid, "error closing database: %s",
            db_strerror(ret));
        return -1;
      }
      if ((ret = dbd->db_env->txn_checkpoint(dbd->db_env, 0, 0, DB_FORCE))) {
        LOG(log_error, logtype_cnid, "error forcing checkpoint: %s",
            db_strerror(ret));
        return -1;
      }
  ...
  for (i = 0; i != DBIF_DB_CNT; i++) {
    ...
    if (!dbd->db_env) { /* In memory db */
      if ((ret = dbd->db_table[i].db->set_cachesize(
               dbd->db_table[i].db, 0, dbp->cachesize,
    ...
    if (dbd->db_table[i].db->open(dbd->db_table[i].db, dbd->db_txn,
                                  dbd->db_filename, dbd->db_table[i].name,
                                  dbd->db_table[i].type,
                                  dbd->db_table[i].openflags, 0664) < 0) {
    ...
    if (reindex && i > 0) {
      ...
      if ((ret = dbd->db_table[i].db->truncate(dbd->db_table[i].db, NULL,
                                               &count, 0))) {
  ...
  if ((ret = dbd->db_table[0].db->associate(
           dbd->db_table[DBIF_CNID].db, dbd->db_txn,
           dbd->db_table[DBIF_IDX_DIDNAME].db, didname,
           (reindex) ? DB_CREATE : 0)) != 0) {
  ...
  if ((ret = dbd->db_table[0].db->associate(
           dbd->db_table[0].db, dbd->db_txn, dbd->db_table[DBIF_IDX_DEVINO].db,
           devino, (reindex) ? DB_CREATE : 0)) != 0) {
  ...
  if ((ret = dbd->db_table[0].db->associate(
           dbd->db_table[0].db, dbd->db_txn, dbd->db_table[DBIF_IDX_NAME].db,
           idxname,
           (reindex ||
...
int dbif_close(DBD *dbd) {
  ...
  if (dbd->db_errlog != NULL && fclose(dbd->db_errlog) == EOF) {
    LOG(log_error, logtype_cnid, "error closing DB logfile: %s",
        strerror(errno));
    err++;
  }
...
int dbif_env_remove(const char *path) {
  ...
  if ((ret = dbd->db_env->open(dbd->db_env, dbd->db_envhome,
                               DB_CREATE | DB_INIT_LOG | DB_INIT_MPOOL |
                                   DB_INIT_LOCK | DB_INIT_TXN | DB_RECOVER |
                                   DB_PRIVATE,
                               0))) {
    LOG(log_error, logtype_cnid, "error opening DB environment: %s",
        db_strerror(ret));
 ...
 * DB->del(). All three return -1 on error. dbif_get()/dbif_del return 1 if the
 * key was found and 0 otherwise. dbif_put() returns 0 if key/val was
 * successfully updated and 1 if the DB_NOOVERWRITE flag was specified and the
 * key already exists.
 *
...
int dbif_get(DBD *dbd, const int dbi, DBT *key, DBT *val, u_int32_t flags) {
  ...
  ret = dbd->db_table[dbi].db->get(dbd->db_table[dbi].db, dbd->db_txn, key, val,
                                   flags);
...
int dbif_pget(DBD *dbd, const int dbi, DBT *key, DBT *pkey, DBT *val,
  ...
  ret = dbd->db_table[dbi].db->pget(dbd->db_table[dbi].db, dbd->db_txn, key,
                                    pkey, val, flags);
...
int dbif_put(DBD *dbd, const int dbi, DBT *key, DBT *val, u_int32_t flags) {
  ...
  ret = dbd->db_table[dbi].db->put(dbd->db_table[dbi].db, dbd->db_txn, key, val,
                                   flags);
...
int dbif_del(DBD *dbd, const int dbi, DBT *key, u_int32_t flags) {
  ...
  ret = dbd->db_table[dbi].db->del(dbd->db_table[dbi].db, dbd->db_txn, key,
                                   flags);
 ...
 *
 * @param resbuf    (w) buffer for search results CNIDs, maxsize is assumed to
 * be DBD_MAX_SRCH_RSLTS * sizefof(cnid_t)
 *
...
int dbif_count(DBD *dbd, const int dbi, u_int32_t *count) {
  ...
  if (ret) {
    LOG(log_error, logtype_cnid,
        "error getting stat infotmation on database: %s", db_strerror(ret));
    return -1;
  }
...
int dbif_dump(DBD *dbd, int dumpindexes) {
  ...
  while (rc == 0) {
    ...
    if (cnid == 0) {
      ...
      strftime(timebuf, sizeof(timebuf), "%b %d %Y %H:%M:%S",
               localtime(&stamp));
      printf(
      ...
      count++;
      printf("id: %10u, did: %10u, type: %s, dev: 0x%llx, ino: 0x%016llx, "
             "name: %s\n",
             cnid, did, typestring[type], (long long unsigned int)dev,
             (long long unsigned int)ino, (char *)data.data + CNID_NAME_OFS);
    }
  ...
  if (rc != DB_NOTFOUND) {
    LOG(log_error, logtype_cnid, "Error iterating over btree: %s",
        db_strerror(rc));
    return -1;
  }
    ...
    while (rc == 0) {
      ...
      if (cnid == 0) {
        ...
        printf("id: %10u <== dev: 0x%llx, ino: 0x%016llx\n", cnid,
               (unsigned long long int)dev, (unsigned long long int)ino);
        count++;
      }
      rc = cur->c_get(cur, &key, &data, DB_NEXT);
    }
...
/*
   ...
   Uses in-value of cnid to seek to that cnid, then gets next and return that in
   cnid. If close=1, close cursor. Return -1 on error, 0 on EOD
   (end-of-database), 1 if returning cnid.
*/
int dbif_idwalk(DBD *dbd, cnid_t *cnid, int close) {
  ...
  if (!dbd->db_cur) {
    ...
    flag = DB_SET_RANGE; /* This will seek to next cnid after the one just
                            deleted */
    id = htonl(*cnid);
    key.data = &id;
