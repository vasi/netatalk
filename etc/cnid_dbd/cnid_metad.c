...
#define SWITCH_TO_GID(gid)                                                     \
  ((setresgid(gid, gid, gid) < 0 || setgid(gid) < 0) ? -1 : 0)
#define SWITCH_TO_UID(uid)                                                     \
  ((setresuid(uid, uid, uid) < 0 || setuid(uid) < 0) ? -1 : 0)
#endif /* USE_SETRESUID */
#else  /* ! linux */
#ifndef USE_SETEUID
...
#define TESTTIME                                                               \
  10 /* this much seconds apfd client tries to  *                              \
...
static void daemon_exit(int i) { exit(i); }
...
int maybe_start_dbd(const AFPObj *obj, char *dbdpn, const char *volpath,
  ...
  if (up && up->pid) {
    ...
    LOG(log_debug, logtype_cnid,
        "maybe_start_dbd: cnid_dbd[%d] already serving", up->pid);
  ...
  if (!up) {
    ...
    if (!up) {
      LOG(log_error, logtype_cnid,
          "no free slot for cnid_dbd child. Configured maximum: %d. Do you "
          "have so many volumes?",
          MAXVOLS);
      return -1;
    }
  } else {
    ...
    if (up->count > 0) {
      ...
      if (t < (up->tm + TESTTIME)) {
        ...
        if (up->count > MAXSPAWN) {
          ...
          return -1; /* just exit, don't sleep, because we might have work to do
                        for another client  */
        }
      } else {
    ...
    LOG(log_maxdebug, logtype_cnid, "maybe_start_dbd: respawn count: %u",
        up->count);
    if (up->count > MAXSPAWN) {
      /* We spawned too fast. From now until the first time we tried + TESTTIME
         seconds we will just return -1 above */
      LOG(log_info, logtype_cnid,
          "maybe_start_dbd: reached MAXSPAWN threshold");
    }
  }
  ...
  if (pid == 0) {
    ...
    if (up->count == MAXSPAWN) {
      ...
      LOG(log_warning, logtype_cnid,
          "Multiple attempts to start CNID db daemon for \"%s\" failed, wiping "
          "the slate clean...",
          up->v_path);
      ret = execlp(dbdpn, dbdpn, "-F", obj->options.configfile, "-p", volpath,
                   "-t", buf1, "-l", buf2, "-u", username, NULL);
    } else {
      ret = execlp(dbdpn, dbdpn, "-F", obj->options.configfile, "-p", volpath,
                   "-t", buf1, "-l", buf2, "-u", username, NULL);
    }
...
static int set_dbdir(const char *dbdir, const char *vpath) {
  ...
  if (lstat(cfrombstr(oldpath), &st) == 0 &&
      lstat(cfrombstr(newpath), &st) != 0 && errno == ENOENT) {
  ...
  if (lstat(cfrombstr(newpath), &st) < 0 &&
      mkdir(cfrombstr(newpath), 0755) < 0) {
    LOG(log_error, logtype_cnid, "set_dbdir: mkdir failed for %s",
        bdata(newpath));
    EC_FAIL;
  }
...
/* ------------------ */
static void catch_child(int sig) { sigchild = 1; }
...
int main(int argc, char *argv[]) {
  ...
  host = atalk_iniparser_getstrdup(obj.iniconfig, INISEC_GLOBAL, "cnid listen",
                                   "localhost:4700");
  ...
  while (1) {
    ...
    if (sigchild)
      while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        ...
        if (WIFEXITED(status)) {
          ...
          /* cnid_dbd did a clean exit probably on idle timeout, reset
           * bookkeeping */
          srv[i].tm = 0;
          srv[i].count = 0;
        }
        if (WIFSIGNALED(status)) {
          LOG(log_info, logtype_cnid, "cnid_dbd[%i] got signal %i", pid,
              WTERMSIG(status));
        }
        sigchild = 0;
      }
    ...
    if (!len[0] || !len[1]) {
      LOG(log_error, logtype_cnid,
          "wrong len parameter: len[0]: %d, len[1]: %d", len[0], len[1]);
      goto loop_end;
    }
