...
static void default_params(struct db_param *dbp, char *dir) {
  ...
  if (make_pathname(dbp->usock_file, dir, DEFAULT_USOCK_FILE, usock_maxlen()) <
      0) {
...
struct db_param *db_param_read(char *dir) {
  ...
  if ((fp = fopen(pfn, "r")) == NULL) {
    if (errno == ENOENT) {
      ...
      LOG(log_error, logtype_cnid, "error opening %s: %s", pfn,
          strerror(errno));
      return NULL;
    }
  }
  ...
  while ((items = fscanf(fp, " %s %s", key, val)) != EOF) {
    ...
    if (!strcmp(key, "fd_table_size")) {
      ...
      LOG(log_info, logtype_cnid,
          "db_param: setting max number of concurrent afpd connections per "
          "volume (fd_table_size) to %d",
