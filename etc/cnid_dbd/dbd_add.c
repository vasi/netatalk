...
int get_cnid(DBD *dbd, struct cnid_dbd_rply *rply) {
  ...
  if (id == 0) {
    if ((rc = dbif_get(dbd, DBIF_CNID, &rootinfo_key, &rootinfo_data, 0)) !=
        1) {
      rply->result = CNID_DBD_RES_ERR_DB;
      return -1;
    }
...
int dbd_add(DBD *dbd, struct cnid_dbd_rqst *rqst, struct cnid_dbd_rply *rply) {
  ...
  LOG(log_debug, logtype_cnid,
      "dbd_add(did:%u, '%s', dev/ino:0x%llx/0x%llx) {start}", ntohl(rqst->did),
  ...
  if (get_cnid(dbd, rply) < 0) {
    if (rply->result == CNID_DBD_RES_ERR_MAX) {
      ...
      LOG(log_error, logtype_cnid,
          "dbd_add: Failed to compute CNID for %s, error reading/updating "
          "Rootkey",
          rqst->name);
      return -1;
    }
  ...
  if (add_cnid(dbd, rqst, rply) < 0) {
    if (rply->result == CNID_DBD_RES_ERR_DUPLCNID) {
      ...
      LOG(log_error, logtype_cnid,
          "dbd_add: Failed to add CNID for %s to database", rqst->name);
      return -1;
    }
  }
  LOG(log_debug, logtype_cnid,
