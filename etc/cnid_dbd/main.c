...
/*
   Note: DB_INIT_LOCK is here so we can run the db_* utilities while netatalk is
   running. It's a likey performance hit, but it might we worth it.
 */
#define DBOPTIONS                                                              \
...
/*
  The dbd_XXX and comm_XXX functions all obey the same protocol for return
  values:

  1: Success, if transactions are used commit.
  0: Failure, but we continue to serve requests. If transactions are used
  abort/rollback. -1: Fatal error, either from t he database or from the socket.
 ...
 *                     LOCK_EXCL/LOCK_SHRD return LOCK_EXCL or LOCK_SHRD
 * respectively on success, 0 if the lock couldn't be acquired, -1 on other
 * errors
 */
  ...
  case LOCK_SHRD:
    if (lockfd == -1) {
      ...
      if ((lockfd = open(lockpath, O_RDWR | O_CREAT, 0644)) < 0) {
        LOG(log_error, logtype_cnid, "Error opening lockfile: %s",
            strerror(errno));
        return -1;
      }

      if ((stat(dbpath, &st)) != 0) {
        LOG(log_error, logtype_cnid, "Error statting lockfile: %s",
            strerror(errno));
        return -1;
      }
      ...
      if ((chown(lockpath, st.st_uid, st.st_gid)) != 0) {
        LOG(log_error, logtype_cnid,
            "Error inheriting lockfile permissions: %s", strerror(errno));
        return -1;
      }
    }
 ...
 *
 * Also tries to copy the rootinfo key, that would allow for keeping the db
 *stamp and last used CNID
 **/
    ...
    */
    if (count > dbp->flush_frequency) {
      LOG(log_info, logtype_cnid,
          "Checkpointing BerkeleyDB after %d writes for volume '%s'", count,
          dbp->dir);
      if (dbif_txn_checkpoint(dbd, 0, 0, 0) < 0)
        return -1;
      count = 0;
...
static void switch_to_user(char *dir) {
  ...
  if (stat(".", &st) < 0) {
    LOG(log_error, logtype_cnid, "error in stat for %s: %s", dir,
        strerror(errno));
    exit(1);
  }
  if (!getuid()) {
    LOG(log_debug, logtype_cnid, "Setting uid/gid to %i/%i", st.st_uid,
        st.st_gid);
...
int main(int argc, char *argv[]) {
  ...
  LOG(log_debug, logtype_cnid, "user: %s, path %s", username ? username : "-",
      volpath);
  ...
  if (open_db() != 0) {
    LOG(log_error, logtype_cnid,
        "Failed to open CNID database for volume \"%s\"", vol->v_localname);
