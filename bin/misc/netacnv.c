...
struct flag_map flag_map[] = {flag(CONV_ESCAPEHEX),  flag(CONV_UNESCAPEHEX),
                              flag(CONV_ESCAPEDOTS), flag(CONV_IGNORE),
                              flag(CONV_FORCE),      flag(CONV__EILSEQ),
                              flag(CONV_TOUPPER),    flag(CONV_TOLOWER),
...
int main(int argc, char **argv) {
  ...
  if ((optind + 1) != argc) {
    printf("Usage: test [-o <conversion option> [...]] [-f <from charset>] [-t "
           "<to charset>] [-m legacy Mac charset] <string>\n");
