...
static void usage_find(void) {
  printf("Usage: ad find [-v VOLUME_PATH] NAME\n");
}
...
int ad_find(int argc, char **argv, AFPObj *obj) {
  ...
  if (convert_charset(vol.vol->v_volcharset, vol.vol->v_volcharset,
                      vol.vol->v_maccharset, argv[optind], strlen(argv[optind]),
                      namebuf, MAXPATHLEN, &flags) == (size_t)-1) {
    ERROR("conversion error");
  }

  int count;
  char resbuf[DBD_MAX_SRCH_RSLTS * sizeof(cnid_t)];
  if ((count = cnid_find(vol.vol->v_cdb, namebuf, strlen(namebuf), resbuf,
                         sizeof(resbuf))) < 1) {
    ret = 1;
  } else {
    ret = 0;
