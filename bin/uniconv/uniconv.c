...
static char *convert_name(char *name, struct stat *st, cnid_t cur_did) {
  ...
  outlen = convert_charset(ch_from, ch_to, ch_mac, (char *)p, strlen((char *)p),
                           (char *)q, sizeof(buffer) - 2, &flags);
  if ((size_t)-1 == outlen) {
    if (ch_to == CH_UTF8) {
      ...
      p = (unsigned char *)name;
      outlen =
          convert_charset(ch_to, ch_to, ch_mac, (char *)p, strlen((char *)p),
                          (char *)q, sizeof(buffer) - 2, &flags);
      if ((size_t)-1 == outlen) {
        /* it's not UTF8... */
        fprintf(stderr,
                "ERROR: conversion from '%s' to '%s' for '%s' in DID %u "
                "failed!!!\n",
  ...
  if (strcmp(name, buffer)) {
    if (dry_run) {
      ...
      if (CNID_INVALID !=
          (id = cnid_add(cdb, st, cur_did, buffer, strlen(buffer), 0)))
        fprintf(stdout, "converted '%s' to '%s' (ID %u, DID %u).\n", name,
                buffer, ntohl(id), ntohl(cur_did));
    }
  } else if (verbose > 1)
    fprintf(stdout, "no conversion required\n");
...
static int init(char *path) {
  ...
  if (NULL == (cdb = cnid_open(path, 0, cnid_type, 0, "localhost", "4700", NULL,
                               NULL))) {
...
static void help(void) {
  fprintf(stdout, "\nuniconv, a tool to convert between various Netatalk "
                  "volume encodings\n");
  fprintf(stdout, "\nUsage:  uniconv [-ndv] -c cnid -f fromcode -t tocode [-m "
                  "maccode] path\n\n");
  fprintf(stdout, "Examples:\n");
  fprintf(stdout,
          "     uniconv -c dbd -f ASCII -t UTF8 -m MAC_ROMAN /path/to/share\n");
  fprintf(stdout, "     uniconv -c cdb -f ISO-8859-1 -t UTF8 -m MAC_ROMAN "
                  "/path/to/share\n");
  fprintf(stdout, "     uniconv -c cdb -f ISO-8859-ADAPTED -t ASCII -m "
                  "MAC_ROMAN /path/to/share\n");
  fprintf(stdout,
  ...
  fprintf(stdout, "     Make sure you know what you are doing. Always backup "
                  "your data first.\n\n");
  fprintf(stdout, "     It is *strongly* recommended to do a `dry run' first "
                  "and to check the\n");
...
int main(int argc, char *argv[]) {
  ...
  if ((charset_t)-1 == (ch_from = add_charset(from_charset))) {
    fprintf(stderr, "Setting codepage %s as source codepage failed",
            from_charset);
    exit(-1);
  }

  if ((charset_t)-1 == (ch_to = add_charset(to_charset))) {
    fprintf(stderr, "Setting codepage %s as destination codepage failed",
            to_charset);
    exit(-1);
  }
