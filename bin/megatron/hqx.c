...
ssize_t hqx_read(int fork, char *buffer, size_t length) {
  ...
  if (cc > 0) {
    hqx.forkcrc[fork] = updcrc(hqx.forkcrc[fork], (u_char *)buffer, cc);
    hqx.forklen[fork] -= cc;
  }
#if DEBUG >= 3
...
int hqx_header_read(struct FHeader *fh) {
  ...
  hqx.headercrc = updcrc(hqx.headercrc, (u_char *)&namelen, sizeof(namelen));

#if HEXOUTPUT
  write(headerfork, &namelen, sizeof(namelen));
#endif /* HEXOUTPUT */

  if ((headerbuf = (char *)malloc((unsigned int)(namelen + BHH_HEADSIZ))) ==
      NULL) {
    return (-1);
  }
  ...
  headerptr = headerbuf;
  hqx.headercrc = updcrc(hqx.headercrc, (u_char *)headerbuf,
                         (namelen + BHH_HEADSIZ - BHH_CRCSIZ));
...
int hqx_header_write(struct FHeader *fh) { return (-1); }
...
static const u_char hqxlookup[] = {
    ...
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFC, 0x00, 0x01, 0x02,
    0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0xFF, 0xFF,
    0x0D, 0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0xFF, 0x14, 0x15, 0xFD, 0xFF,
    0xFF, 0xFF, 0xFF, 0xFF, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B, 0x1C, 0x1D,
    0x1E, 0x1F, 0x20, 0x21, 0x22, 0x23, 0x24, 0xFF, 0x25, 0x26, 0x27, 0x28,
    0x29, 0x2A, 0x2B, 0xFF, 0x2C, 0x2D, 0x2E, 0x2F, 0xFF, 0xFF, 0xFF, 0xFF,
    0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0xFF, 0x37, 0x38, 0x39, 0x3A,
    0x3B, 0x3C, 0xFF, 0xFF, 0x3D, 0x3E, 0x3F, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
  ...
  while (found == NOWAY) {
    if (line == FIRST) {
      if (*hqx7_first == ':') {
        ...
        stopflag = NOWAY;
        hqx7_first++;
        while ((stopflag == NOWAY) && (nc < (hqx7_last - hqx7_first))) {
