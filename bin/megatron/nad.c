  ...
  if (*in) {
    ...
    while (out < maxout) {
      ...
      if (((0x81 <= p) && (p <= 0x9F)) || ((0xE0 <= p) && (p <= 0xEF))) {
        /* JIS X 0208 */
        p2 = *in++;
        if (((0x40 <= p2) && (p2 <= 0x7E)) || ((0x80 <= p2) && (p2 <= 0xFC)))
          sjis2euc(&p, &p2);
        *out++ = p;
        p = p2;
...
static char *utompathiconv(char *upath) {
  ...
  /* convert charsets */
  if ((size_t)-1 == (outlen = convert_charset(
                         vol.v_volcharset, vol.v_maccharset, vol.v_maccharset,
                         u, outlen, mpath, MAXPATHLEN, &flags))) {
    fprintf(stderr, "Conversion from %s to %s for %s failed.",
...
static char *mtoupathiconv(char *mpath) {
  ...
  if ((size_t)-1 == (outlen = convert_charset(
                         vol.v_maccharset, vol.v_volcharset, vol.v_maccharset,
                         m, inplen, u, outlen, &flags))) {
    fprintf(stderr, "conversion from %s to %s for %s failed.",
            vol.v_maccodepage, vol.v_volcodepage, mpath);
...
int nad_open(char *path, int openflags, struct FHeader *fh, int options) {
  ...
  if (openflags == O_RDONLY) {
    ...
    strcpy(nad.adpath[1], nad.ad.ad_ops->ad_path(nad.adpath[0], ADFLAGS_HF));
...
#endif /* DEBUG */
    if (ad_open(nad.adpath[0], ADFLAGS_DF | ADFLAGS_HF, openflags,
                (int)(st.st_mode & 0666), &nad.ad) < 0) {
      perror(nad.adpath[0]);
      return (-1);
    }
...
int nad_header_read(struct FHeader *fh) {
...
#if DEBUG
  {
    ...
    fprintf(stderr, "finder_info.fdCreator\t\t%.*s\n",
            sizeof(fh->finder_info.fdCreator), &fh->finder_info.fdCreator);
    fprintf(stderr, "nad type and creator\t\t%.*s\n\n",
            sizeof(fh->finder_info.fdType) + sizeof(fh->finder_info.fdCreator),
...
int nad_header_write(struct FHeader *fh) {
  ...
  fprintf(stderr, "ad_entry of ADEID_NAME\t\t%.*s\n",
          ad_getentrylen(&nad.ad, ADEID_NAME), ad_entry(&nad.ad, ADEID_NAME));
...
ssize_t nad_read(int fork, char *forkbuf, size_t bufc) {
  ...
  if ((cc = ad_read(&nad.ad, forkeid[fork], nad.offset[fork], forkbuf, bufc)) <
      0) {
...
ssize_t nad_write(int fork, char *forkbuf, size_t bufc) {
  ...
  while ((writelen > 0) && (cc >= 0)) {
    cc = ad_write(&nad.ad, forkeid[fork], nad.offset[fork], 0, buf_ptr,
                  writelen);
    nad.offset[fork] += cc;
    buf_ptr += cc;
    writelen -= cc;
