...
static char *name[] = {"unhex",     "unbin",     "unsingle",
                       "macbinary", "hqx2bin",   "single2bin",
                       "nadheader", "binheader", "megatron"};
  ...
  *newname = '\0';
  for (c = 1; c < argc; ++c) {
    ...
    if (strcmp(argv[c], "--filename") == 0) {
      if (++c < argc)
        strncpy(newname, argv[c], ADEDLEN_NAME);
      continue;
    }
