...
#define PTR_DIFF(p1, p2)                                                       \
  ((ptrdiff_t)(((const char *)(p1)) - (const char *)(p2)))
...
#define TALLOC_MAGIC                                                           \
  (TALLOC_MAGIC_BASE + (TALLOC_VERSION_MAJOR << 12) +                          \
...
_PUBLIC_ int talloc_version_major(void) { return TALLOC_VERSION_MAJOR; }

_PUBLIC_ int talloc_version_minor(void) { return TALLOC_VERSION_MINOR; }
...
static void talloc_abort_magic(unsigned magic) {
  ...
  talloc_log("Bad talloc magic[0x%08X/%u/%u] expected[0x%08X/%u/%u]\n", magic,
             major, minor, TALLOC_MAGIC, TALLOC_VERSION_MAJOR,
...
#if defined(DEVELOPER) && defined(VALGRIND_MAKE_MEM_NOACCESS)
/* Mark the whole remaining pool as not accessable */
#define TC_INVALIDATE_VALGRIND_POOL(_pool_tc)                                  \
  do {                                                                         \
    size_t _flen = TC_POOL_SPACE_LEFT(_pool_tc);                               \
    char *_fptr = (char *)(_pool_tc)->pool;                                    \
    VALGRIND_MAKE_MEM_NOACCESS(_fptr, _flen);                                  \
...
#define TC_INVALIDATE_POOL(_pool_tc)                                           \
  do {                                                                         \
...
static void talloc_abort_type_missmatch(const char *location, const char *name,
                                        const char *expected) {
  const char *reason;

  reason = talloc_asprintf(NULL, "%s: Type mismatch: name[%s] expected[%s]",
...
static void talloc_report_depth_FILE_helper(const void *ptr, int depth,
  ...
  fprintf(f, "%*s%-30s contains %6lu bytes in %3lu blocks (ref %d) %p\n",
          depth * 4, "", name, (unsigned long)talloc_total_size(ptr),
          (unsigned long)talloc_total_blocks(ptr),
...
*/
_PUBLIC_ void *_talloc_realloc_array(const void *ctx, void *ptr, size_t el_size,
                                     unsigned count, const char *name) {
  if (count >= MAX_TALLOC_SIZE / el_size) {
    return NULL;
  }
...
static void talloc_autofree(void) { talloc_free(autofree_context); }
