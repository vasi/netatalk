...
static struct adouble_fops ad_adouble = {
    &ad_path,        &ad_mkrf,           &ad_rebuild_adouble_header_v2,
...
static int new_ad_header(struct adouble *ad, const char *path, struct stat *stp,
  ...
  if ((ad->ad_options & ADVOL_INVDOTS) && (*path == '.') &&
    ...
    memcpy(ad_entry(ad, ADEID_FINDERI) + FINDERINFO_FRFLAGOFF, &ashort,
           sizeof(ashort));
  }
...
/**
 * Read an AppleDouble buffer, returns 0 on success, -1 if an entry was
 *malformatted
 **/
  ...
  for (; nentries > 0; nentries--) {
    ...
    if (!eid || eid > ADEID_MAX || off >= sizeof(ad->ad_data) ||
      ...
      LOG(log_warning, logtype_ad,
          "parse_entries: bogus eid: %u, off: %u, len: %u", (uint)eid,
          (uint)off, (uint)len);
    }
  }
...
int ad_valid_header_osx(const char *path) {
  ...
  if ((adosx.ad_magic != AD_MAGIC) || (adosx.ad_version != AD_VERSION2)) {
    LOG(log_warning, logtype_ad,
        "ad_valid_header_osx(\"%s\"): not an adouble:osx file",
        fullpathname(path));
    EC_FAIL;
  }

  if (strncmp(buf + ADEDOFF_FILLER, AD_FILLER_NETATALK,
...
static int ad_convert_osx(const char *path, struct adouble *ad) {
  ...
  EC_ZERO_LOG(
      ftruncate(ad_reso_fileno(ad), ad_getentryoff(ad, ADEID_RFORK) +
                                        ad_getentrylen(ad, ADEID_RFORK)));
  ...
  if (adea.ad_mdp->adf_flags & O_CREAT) {
    memcpy(ad_entry(&adea, ADEID_FINDERI), ad_entry(ad, ADEID_FINDERI),
           ADEDLEN_FINDERI);
    ad_flush(&adea);
  }
...
static int ad_header_read_ea(const char *path, struct adouble *ad,
  ...
  if ((ad->ad_magic != AD_MAGIC) || (ad->ad_version != AD_VERSION2)) {
    LOG(log_error, logtype_ad,
        "ad_header_read_ea(\"%s\"): wrong magic or version",
        fullpathname(path));
    errno = EINVAL;
    EC_FAIL;
  }
  ...
  if (nentries != ADEID_NUM_EA) {
    LOG(log_error, logtype_ad,
        "ad_header_read_ea(\"%s\"): invalid number of entries: %d",
        fullpathname(path), nentries);
    errno = EINVAL;
    EC_FAIL;
  }
  ...
  if (nentries != ADEID_NUM_EA || !ad_entry(ad, ADEID_FINDERI) ||
      !ad_entry(ad, ADEID_COMMENT) || !ad_entry(ad, ADEID_FILEDATESI) ||
      !ad_entry(ad, ADEID_AFPFILEI) || !ad_entry(ad, ADEID_PRIVDEV) ||
      !ad_entry(ad, ADEID_PRIVINO) || !ad_entry(ad, ADEID_PRIVSYN) ||
      !ad_entry(ad, ADEID_PRIVID)) {
    LOG(log_error, logtype_ad, "ad_header_read_ea(\"%s\"): invalid metadata EA",
        fullpathname(path));
    errno = EINVAL;
    EC_FAIL;
  }
...
EC_CLEANUP:
  if (ret != 0 && errno == EINVAL) {
    ...
    unbecome_root();
    LOG(log_error, logtype_ad,
        "ad_header_read_ea(\"%s\"): deleted invalid metadata EA",
        fullpathname(path), nentries);
    errno = ENOENT;
  }
  EC_EXIT;
...
/*!
 * Takes a path to an AppleDouble file and creates the parrent .AppleDouble
 * directory
 *
 * Example:
...
static int ad_mkrf_osx(const char *path) { return 0; }
...
/* --------------------------- */
static int ad_header_upgrade(struct adouble *ad, const char *name) { return 0; }
 ...
 * 2. Open non-existent ressource fork, this will just result in first read
 * return EOF
 * 3. If ad_open was called with ADFLAGS_DF we may have opened the datafork and
 * thus ought to close it before returning with an error condition.
 */
 ...
 *
 * @param adfile   (r) the file you really want to open: ADFLAGS_DF or
 * ADFLAGS_HF
...
static int ad_open_df(const char *path, int adflags, mode_t mode,
  ...
  LOG(log_debug, logtype_ad,
      "ad_open_df(\"%s\", %s): BEGIN [dfd: %d (ref: %d), mfd: %d (ref: %d), "
      "rfd: %d (ref: %d)]",
      fullpathname(path), adflags2logstr(adflags), ad_data_fileno(ad),
      ad->ad_data_fork.adf_refcount, ad_meta_fileno(ad),
      ad->ad_mdp->adf_refcount, ad_reso_fileno(ad), ad->ad_rfp->adf_refcount);
...
EC_CLEANUP:
  LOG(log_debug, logtype_ad,
      "ad_open_df(\"%s\", %s): END: %d [dfd: %d (ref: %d), mfd: %d (ref: %d), "
      "rfd: %d (ref: %d)]",
      fullpathname(path), adflags2logstr(adflags), ret, ad_data_fileno(ad),
      ad->ad_data_fork.adf_refcount, ad_meta_fileno(ad),
      ad->ad_mdp->adf_refcount, ad_reso_fileno(ad), ad->ad_rfp->adf_refcount);
  EC_EXIT;
}
...
static int ad_open_hf_v2(const char *path, int adflags, mode_t mode,
  ...
  LOG(log_debug, logtype_ad,
      "ad_open_hf_v2(\"%s\", %s): BEGIN [dfd: %d (ref: %d), mfd: %d (ref: %d), "
      "rfd: %d (ref: %d)]",
      fullpathname(path), adflags2logstr(adflags), ad_data_fileno(ad),
      ad->ad_data_fork.adf_refcount, ad_meta_fileno(ad),
      ad->ad_mdp->adf_refcount, ad_reso_fileno(ad), ad->ad_rfp->adf_refcount);
  ...
  if (!(ad->ad_mdp->adf_flags & O_CREAT)) {
    ...
    if (fstat(ad->ad_mdp->adf_fd, &st_meta) == 0) {
      if (st_meta.st_size == 0)
        ...
        /* we have valid data in st_meta stat structure, reused it in
         * ad_header_read */
        pst = &st_meta;
    }
  }
...
EC_CLEANUP:
  ...
  LOG(log_debug, logtype_ad,
      "ad_open_hf_v2(\"%s\", %s): END: %d [dfd: %d (ref: %d), mfd: %d (ref: "
      "%d), rfd: %d (ref: %d)]",
      fullpathname(path), adflags2logstr(adflags), ret, ad_data_fileno(ad),
      ad->ad_data_fork.adf_refcount, ad_meta_fileno(ad),
      ad->ad_mdp->adf_refcount, ad_reso_fileno(ad), ad->ad_rfp->adf_refcount);
...
static int ad_open_hf_ea(const char *path, int adflags, int mode,
                         struct adouble *ad) {
  EC_INIT;
  int oflags;
  int opened = 0;
  ...
  LOG(log_debug, logtype_ad,
      "ad_open_hf_ea(\"%s\", %s): BEGIN [dfd: %d (ref: %d), mfd: %d (ref: %d), "
      "rfd: %d (ref: %d)]",
      fullpathname(path), adflags2logstr(adflags), ad_data_fileno(ad),
      ad->ad_data_fork.adf_refcount, ad_meta_fileno(ad),
      ad->ad_mdp->adf_refcount, ad_reso_fileno(ad), ad->ad_rfp->adf_refcount);
  ...
  if (ad_meta_fileno(ad) != -1) {
    ...
    if ((oflags & O_RDWR) &&
        ...
        (ad->ad_mdp->adf_flags & O_RDONLY)) {
      LOG(log_error, logtype_ad,
          "ad_open_hf_ea(%s): rw request for ro file: %s", fullpathname(path),
          strerror(errno));
      errno = EACCES;
      EC_FAIL;
    }
  ...
  if (ad->ad_ops->ad_header_read(path, ad, NULL) != 0) {
    if (!(adflags & ADFLAGS_CREATE)) {
      LOG(log_debug, logtype_ad,
          "ad_open_hf_ea(\"%s\"): can't read metadata EA", path);
      errno = ENOENT;
      EC_FAIL;
    }
    ...
    LOG(log_debug, logtype_ad, "ad_open_hf_ea(\"%s\"): creating metadata EA",
        path);

    /* It doesnt exist, EPERM or another error */
    if (!(errno == ENOATTR || errno == ENOENT)) {
      LOG(log_error, logtype_ad, "ad_open_hf_ea: unexpected: %s",
          strerror(errno));
      EC_FAIL;
    }
    ...
    ad_flush(ad);
    LOG(log_debug, logtype_ad, "ad_open_hf_ea(\"%s\"): created metadata EA",
        path);
  }
...
EC_CLEANUP:
  ...
  LOG(log_debug, logtype_ad,
      "ad_open_hf_ea(\"%s\", %s): END: %d [dfd: %d (ref: %d), mfd: %d (ref: "
      "%d), rfd: %d (ref: %d)]",
      fullpathname(path), adflags2logstr(adflags), ret, ad_data_fileno(ad),
      ad->ad_data_fork.adf_refcount, ad_meta_fileno(ad),
      ad->ad_mdp->adf_refcount, ad_reso_fileno(ad), ad->ad_rfp->adf_refcount);
...
static int ad_open_rf_v2(const char *path, int adflags, int mode,
                         struct adouble *ad) {
  EC_INIT;

  /*
   * ad_open_hf_v2() does the work, but if it failed and adflags are
   * ADFLAGS_NOHF | ADFLAGS_RF ad_open_hf_v2() didn't give an error, but we're
   * supposed to return a reso fork fd
   */
...
EC_CLEANUP:
  LOG(log_debug, logtype_ad, "ad_open_rf_v2(\"%s\"): END: %d",
      fullpathname(path), ret);
  EC_EXIT;
}
...
static int ad_open_rf_ea(const char *path, int adflags, int mode,
  ...
  if (ad_meta_fileno(ad) < 0)
    EC_FAIL;
  if ((ad_reso_fileno(ad) =
           sys_getxattrfd(ad_meta_fileno(ad), AD_EA_RESO, oflags)) == -1) {
    if (!(adflags & ADFLAGS_CREATE)) {
      switch (errno) {
      case EACCES:
      case EPERM:
      ...
      oflags |= O_CREAT;
      EC_NEG1_LOG(ad_reso_fileno(ad) = sys_getxattrfd(
                      ad_meta_fileno(ad), AD_EA_RESO, oflags, 0666));
    }
  }
#else
  ...
  if ((ad_reso_fileno(ad) = open(rfpath, oflags)) == -1) {
    if (!(adflags & ADFLAGS_CREATE)) {
      ...
      LOG(log_debug, logtype_ad,
          "ad_open_rf(\"%s\"): created adouble rfork: \"%s\"", path, rfpath);
    }
  }
#endif
  opened = 1;
  ...
  if (ad->ad_rfp->adf_flags & O_CREAT) {
    /* This is a new adouble header file, create it */
    LOG(log_debug, logtype_ad,
        "ad_open_rf(\"%s\"): created adouble rfork, initializing: \"%s\"", path,
        rfpath);
    EC_NEG1_LOG(new_ad_header(ad, path, NULL, adflags));
    LOG(log_debug, logtype_ad,
        "ad_open_rf(\"%s\"): created adouble rfork, flushing: \"%s\"", path,
        rfpath);
    ad_flush(ad);
  } else {
    /* Read the adouble header */
    LOG(log_debug, logtype_ad,
        "ad_open_rf(\"%s\"): reading adouble rfork: \"%s\"", path, rfpath);
...
const char *ad_path_ea(const char *path, int adflags) { return path; }
...
/* ---------------- */
uid_t ad_getfuid(void) { return default_uid; }
...
int ad_mkdir(const char *path, mode_t mode) {
  ...
  LOG(log_debug, logtype_ad, "ad_mkdir(\"%s\", %04o) {cwd: \"%s\"}", path, mode,
      getcwdpath());
 ...
 * ad_open(struct adouble *ad, const char *path, int adflags, int flags, mode_t
 * mode)
 *
 * You must call ad_init() before ad_open, usually you'll just call it like
 * this: \n
 * @code
 *      struct adoube ad;
 ...
 *                         ADFLAGS_NOHF:      it's not an error if header file
 * couldn't be opened ADFLAGS_NORF:      it's not an error if reso fork couldn't
 * be opened ADFLAGS_DIR:       if path is a directory you MUST or ADFLAGS_DIR
 * to adflags
 *
 ...
 *
 *                       Special flags:
 *                         ADFLAGS_CHECK_OF:  check for open forks from us and
 * other afpd's ADFLAGS_SETSHRMD:  this adouble struct will be used to set
 * sharemode locks. This basically results in the files being opened RW instead
 * of RDONLY.
 * @param mode      (r)  mode used with O_CREATE
 *
 * The open mode flags (rw vs ro) have to take into account all the following
 * requirements:
 * - we remember open fds for files because me must avoid a single close
 * releasing fcntl locks for other fds of the same file
 *
 * BUGS:
 *
...
int ad_open(struct adouble *ad, const char *path, int adflags, ...) {
  ...
  LOG(log_debug, logtype_ad,
      ...
      fullpathname(path), adflags2logstr(adflags), ad->ad_data_refcount,
      ad->ad_meta_refcount, ad->ad_reso_refcount, ad_data_fileno(ad),
      ad->ad_data_fork.adf_refcount, ad_meta_fileno(ad),
      ad->ad_mdp->adf_refcount, ad_reso_fileno(ad), ad->ad_rfp->adf_refcount);

  if (adflags & ADFLAGS_CHECK_OF)
    /* Checking for open forks requires sharemode lock support (ie RDWR instead
     * of RDONLY) */
    adflags |= ADFLAGS_SETSHRMD;
  ...
  if (ad->ad_inited != AD_INITED) {
    ...
    ad->ad_open_forks =
        ((ad->ad_data_fork.adf_refcount > 0) ? ATTRBIT_DOPEN : 0);
  ...
  if (adflags & ADFLAGS_CREATE)
    mode = (sizeof(mode_t) < sizeof(int) ? va_arg(args, int)
                                         : va_arg(args, mode_t));
  va_end(args);
...
EC_CLEANUP:
  LOG(log_debug, logtype_ad,
      ...
      "[dfd: %d (ref: %d), mfd: %d (ref: %d), rfd: %d (ref: %d)]",
      fullpathname(path), ret, ad->ad_data_refcount, ad->ad_meta_refcount,
      ad->ad_reso_refcount, ad_data_fileno(ad), ad->ad_data_fork.adf_refcount,
      ad_meta_fileno(ad), ad->ad_mdp->adf_refcount, ad_reso_fileno(ad),
      ad->ad_rfp->adf_refcount);
 ...
 *
 * Return only metadata but try very hard ie at first try as user, then try as
 * root.
 *
 * @param name  name of file/dir
 * @param flags ADFLAGS_DIR: name is a directory \n
 *              ADFLAGS_CHECK_OF: test if name is open by us or another afpd
 * process
 *
...
int ad_metadata(const char *name, int flags, struct adouble *adp) {
  ...
  /* Sanitize flags */
  oflags =
      (flags & (ADFLAGS_CHECK_OF | ADFLAGS_DIR)) | ADFLAGS_HF | ADFLAGS_RDONLY;
...
int ad_openat(struct adouble *ad, int dirfd, /* dir fd openat like */
              const char *path, int adflags, ...) {
  EC_INIT;
  int cwdfd = -1;
  va_list args;
  mode_t mode = 0;
