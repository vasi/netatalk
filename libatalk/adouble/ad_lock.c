...
static int set_lock(int fd, int cmd, struct flock *lock) {
  ...
  LOG(log_debug, logtype_ad,
      "set_lock(fd: %d, %s, %s, off: %jd (%s), len: %jd): BEGIN", fd,
      cmd == F_SETLK ? "F_SETLK" : "F_GETLK",
      lock->l_type == F_RDLCK   ? "F_RDLCK"
      : lock->l_type == F_WRLCK ? "F_WRLCK"
                                : "F_UNLCK",
      (intmax_t)lock->l_start, shmdstrfromoff(lock->l_start),
      (intmax_t)lock->l_len);
...
static void adf_freelock(struct ad_fd *ad, const int i) {
  ...
  if ((ad->adf_lockmax > ARRAY_FREE_DELTA) &&
    ...
    tmp = (struct adf_lock_t *)realloc(
        ad->adf_lock,
        sizeof(adf_lock_t) * (ad->adf_lockcount + ARRAY_FREE_DELTA));
    if (tmp) {
      ad->adf_lock = tmp;
...
static int adf_findlock(struct ad_fd *ad, const int fork, const int type,
...
static int adf_findxlock(struct ad_fd *ad, const int fork, const int type,
                         const off_t off, const off_t len) {
  adf_lock_t *lock = ad->adf_lock;
  int i;
...
int ad_lock(struct adouble *ad, uint32_t eid, int locktype, off_t off,
  ...
  LOG(log_debug, logtype_ad, "ad_lock(%s, %s, off: %jd (%s), len: %jd): BEGIN",
      eid == ADEID_DFORK ? "data" : "reso", locktypetostr(locktype),
  ...
  if (adf_findxlock(adf, fork, ADLOCK_WR | ((type & ADLOCK_WR) ? ADLOCK_RD : 0),
  ...
  if (adf->adf_lockmax == adf->adf_lockcount) {
    adf_lock_t *tmp = (adf_lock_t *)realloc(
        adf->adf_lock,
        sizeof(adf_lock_t) * (adf->adf_lockmax + ARRAY_BLOCK_SIZE));
    if (!tmp) {
...
int ad_tmplock(struct adouble *ad, uint32_t eid, int locktype, off_t off,
  ...
  LOG(log_debug, logtype_ad,
      "ad_tmplock(%s, %s, off: %jd (%s), len: %jd): BEGIN",
      eid == ADEID_DFORK ? "data" : "reso", locktypetostr(locktype),
      (intmax_t)off, shmdstrfromoff(off), (intmax_t)len);
  ...
  /* see if it's locked by another fork. */
  if (fork &&
      adf_findxlock(adf, fork, ADLOCK_WR | ((type & ADLOCK_WR) ? ADLOCK_RD : 0),
                    lock.l_start, lock.l_len) > -1) {
    errno = EACCES;
    err = -1;
    goto exit;
