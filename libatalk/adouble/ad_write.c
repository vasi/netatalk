...
ssize_t ad_write(struct adouble *ad, uint32_t eid, off_t off, int end,
  ...
  if (eid == ADEID_DFORK) {
    ...
    return -1; /* we don't know how to write if it's not a ressource or data
                  fork */
  }
...
EC_CLEANUP:
  if (ret != 0)
    LOG(log_error, logtype_ad, "ad_rtruncate(\"%s\"): %s", fullpathname(uname),
        strerror(errno));
  EC_EXIT;
}
...
int ad_dtruncate(struct adouble *ad, const off_t size) {
  if (sys_ftruncate(ad_data_fileno(ad), size) < 0) {
    LOG(log_error, logtype_ad, "sys_ftruncate(fd: %d): %s", ad_data_fileno(ad),
        strerror(errno));
    return -1;
  }
 ...
 */
int copy_fork(int eid, struct adouble *add, struct adouble *ads, char *buf,
              size_t buflen) {
  ssize_t cc;
  int err = 0;
  char fixedbuf[8192];
