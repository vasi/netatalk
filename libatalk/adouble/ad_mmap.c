...
static void *ad_mmap(const size_t length, const int prot, const int flags,
...
void *ad_mmapread(struct adouble *ad, const u_int32_t eid, const off_t off,
                  const size_t buflen) {
  /* data fork */
  if (eid == ADEID_DFORK) {
  ...
  ad->ad_hf.adf_off = 0;
  return ad_mmap(buflen, PROT_READ | PROT_WRITE, MAP_PRIVATE, ad->ad_hf.adf_fd,
 ...
 * mess is for. */
void *ad_mmapwrite(struct adouble *ad, const u_int32_t eid, off_t off,
                   const int end, const size_t buflen) {
  struct stat st;
  ...
  if (eid == ADEID_DFORK) {
    ...
    ad->ad_df.adf_off = 0;
    return ad_mmap(buflen, PROT_READ | PROT_WRITE, MAP_SHARED, ad->ad_df.adf_fd,
                   off);
  }
  ...
  ad->ad_hf.adf_off = 0;
  return ad_mmap(buflen, PROT_READ | PROT_WRITE, MAP_SHARED, ad->ad_hf.adf_fd,
                 off);
}
