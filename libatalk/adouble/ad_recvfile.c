...
/*
 * If tofd is -1, drain the incoming socket of count bytes without writing to
 * the outgoing fd, if a write fails we do the same.
 *
...
static ssize_t default_sys_recvfile(int fromfd, int tofd, off_t offset,
                                    size_t count) {
  int saved_errno = 0;
  size_t total = 0;
  ...
  while (total < count) {
    ...
    while (num_written < read_ret) {
      ...
      if (tofd == -1) {
        ...
        write_ret =
            pwrite(tofd, buffer + num_written, read_ret - num_written, offset);
        if (write_ret <= 0) {
  ...
  while (1) {
    ...
    if ((ret = select(maxfd, &readfds, NULL, NULL, NULL)) <= 0) {
      ...
      LOG(log_error, logtype_dsi,
          "waitfordata: unexpected select return: %d %s", ret,
...
static ssize_t sys_recvfile(int fromfd, int tofd, off_t offset, size_t count,
  ...
  LOG(log_debug, logtype_dsi,
      "sys_recvfile: from = %d, to = %d, offset = %.0f, count = %lu", fromfd,
      tofd, (double)offset, (unsigned long)count);
