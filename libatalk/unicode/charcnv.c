...
charset_t add_charset(const char *name) {
  ...
  if ((int)cur_charset_t >= MAX_CHARSETS) {
    LOG(log_debug, logtype_default,
        "Adding charset %s failed, too many charsets (max. %u allowed)", name,
        MAX_CHARSETS);
    return (charset_t)-1;
  }
  ...
  if (conv_handles[cur_charset_t][CH_UCS2] == (atalk_iconv_t)-1) {
    LOG(log_error, logtype_default,
        "Required conversion from %s to %s not supported", name,
...
size_t convert_string(charset_t from, charset_t to, void const *src,
  ...
  if ((size_t)-1 ==
      (o_len = convert_string_internal(from, CH_UCS2, src, srclen,
...
convert:
  ...
  inbuf = src; /* this restarts the whole conversion if buffer needed to be
                  increased */
  i_len = srclen;
  o_len = destlen;
  retval = atalk_iconv(descriptor, &inbuf, &i_len, &outbuf, &o_len);
...
size_t convert_string_allocate(charset_t from, charset_t to, void const *src,
  ...
  *dest = NULL;
  ...
  /* convert from_set to UCS2 */
  if ((size_t)(-1) ==
      (o_len = convert_string_internal(from, CH_UCS2, src, srclen, buffer,
                                       sizeof(buffer)))) {
    LOG(log_error, logtype_default, "Conversion failed ( %s to CH_UCS2 )",
        charset_name(from));
    return (size_t)-1;
  ...
  /* Convert UCS2 to to_set */
  if ((size_t)-1 == (o_len = convert_string_allocate_internal(
                         CH_UCS2, to, (char *)u, i_len, dest)))
    LOG(log_error, logtype_default, "Conversion failed (CH_UCS2 to %s):%s",
        charset_name(to), strerror(errno));
...
/**
 * Copy a string from a charset_t char* src to a UCS2 destination, allocating a
 *buffer
 *
 * @param dest always set at least to NULL
 *
...
/** -----------------------------------
 * Copy a string from a charset_t char* src to a UTF-8 destination, allocating a
 *buffer
 *
 * @param dest always set at least to NULL
 *
...
/** ---------------------------------
 * Copy a string from a UTF-8 src to a unix char * destination, allocating a
 *buffer
 *
 * @param dest always set at least to NULL
 *
...
size_t charset_precompose(charset_t ch, char *src, size_t inlen, char *dst,
  ...
  if ((size_t)(-1) == (len = convert_string_internal(CH_UCS2, ch, (char *)u,
                                                     ilen, dst, outlen))) {
    free(buffer);
    return (size_t)(-1);
  }
 ...
 * FIXME:
 *      This will *not* work if the destination charset is not multibyte, i.e.
 * UCS2->UCS2 will fail The (un)escape scheme is not compatible to the old cap
 * style escape. This is bad, we need it for e.g. HFS cdroms.
 */

static size_t pull_charset_flags(charset_t from_set, charset_t to_set,
                                 charset_t cap_set, const char *src,
  ...
  while (i_len > 0) {
    ...
    if (i_len > 0 && atalk_iconv(descriptor, &inbuf, &i_len, &outbuf, &o_len) ==
                         (size_t)-1) {
    ...
    if (j) {
      ...
      if (escch == ':') {
        if ((option & CONV_UNESCAPEHEX)) {
          ...
          while (i_len >= 3 && inbuf[0] == ':' && isxdigit(inbuf[1]) &&
                 isxdigit(inbuf[2])) {
          ...
          if (hlen) {
            const char *h_buf = h;
            if (atalk_iconv(descriptor_cap, &h_buf, &hlen, &outbuf, &o_len) ==
                (size_t)-1) {
              i_len += hlen * 3;
              inbuf -= hlen * 3;
  ...
  return (i_len + j == 0 || (option & CONV_FORCE)) ? destlen - o_len
                                                   : (size_t)-1;
}
 ...
 * ? This will *not* work if the destination charset is not multibyte, i.e.
 * UCS2->UCS2 will fail The escape scheme is not compatible to the old cap style
 * escape. This is bad, we need it for e.g. HFS cdroms.
 */
...
static size_t push_charset_flags(charset_t to_set, charset_t cap_set, char *src,
  ...
  while (i_len >= 2) {
    while (i_len > 0 && atalk_iconv(descriptor, &inbuf, &i_len, &outbuf,
                                    &o_len) == (size_t)-1) {
      if (errno == EILSEQ) {
        if ((option & CONV_IGNORE)) {
          *flags |= CONV_REQMANGLE;
...
size_t convert_charset(charset_t from_set, charset_t to_set,
                       charset_t cap_charset, const char *src, size_t src_len,
                       char *dest, size_t dest_len, uint16_t *flags) {
  size_t i_len, o_len;
  ucs2_t *u;
  ...
  /* convert from_set to UCS2 */
  if ((size_t)(-1) ==
      (o_len = pull_charset_flags(from_set, to_set, cap_charset, src, src_len,
  ...
  /* Convert UCS2 to to_set */
  if ((size_t)(-1) ==
      (o_len = push_charset_flags(to_set, cap_charset, (char *)u, i_len, dest,
                                  dest_len, flags))) {
    LOG(log_error, logtype_default, "Conversion failed (CH_UCS2 to %s):%s",
