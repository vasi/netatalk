...
static size_t sys_iconv(void *cd, char **inbuf, size_t *inbytesleft,
                        char **outbuf, size_t *outbytesleft) {
#ifdef HAVE_USABLE_ICONV
  size_t ret = iconv((iconv_t)cd, (ICONV_CONST char **)inbuf, inbytesleft,
                     outbuf, outbytesleft);
  if (ret == (size_t)-1)
 ...
 **/
size_t atalk_iconv(atalk_iconv_t cd, const char **inbuf, size_t *inbytesleft,
                   char **outbuf, size_t *outbytesleft) {
  char cvtbuf[2048];
  char *bufp = cvtbuf;
  size_t bufsize;
  ...
  if (cd->direct) {
    return cd->direct(cd->cd_direct, (char **)inbuf, inbytesleft, outbuf,
                      outbytesleft);
  }
  ...
  while (*inbytesleft > 0) {
    ...
    if (cd->pull(cd->cd_pull, (char **)inbuf, inbytesleft, &bufp, &bufsize) ==
            (size_t)-1 &&
        errno != E2BIG) {
      return -1;
    }
    ...
    if (cd->push(cd->cd_push, &bufp, &bufsize, outbuf, outbytesleft) ==
        (size_t)-1) {
      return -1;
    }
  }
...
atalk_iconv_t atalk_iconv_open(const char *tocode, const char *fromcode) {
  ...
  if (!from || (from->flags & CHARSET_ICONV)) {
    ret->cd_pull =
        iconv_open(UCS2ICONV, from && from->iname ? from->iname : fromcode);
  ...
  if (ret->pull) {
    if (!to || (to->flags & CHARSET_ICONV)) {
      ret->cd_push =
          iconv_open(to && to->iname ? to->iname : tocode, UCS2ICONV);
