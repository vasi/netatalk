...
int islower_w(ucs2_t c) { return (c == tolower_w(c)); }

int islower_sp(uint32_t c_sp) { return (c_sp == tolower_sp(c_sp)); }
...
int isupper_w(ucs2_t c) { return (c == toupper_w(c)); }

int isupper_sp(uint32_t c_sp) { return (c_sp == toupper_sp(c_sp)); }
...
int strcasecmp_w(const ucs2_t *a, const ucs2_t *b) {
  ...
  while (*a && *b) {
    if ((0xD800 <= *a) && (*a < 0xDC00)) {
      if ((ret = tolower_sp((uint32_t)*a << 16 | (uint32_t)a[1]) -
                 tolower_sp((uint32_t)*b << 16 | (uint32_t)b[1])))
        return ret;
      a++;
      b++;
      if (!(*a && *b))
...
ucs2_t *strdup_w(const ucs2_t *src) { return strndup_w(src, 0); }
