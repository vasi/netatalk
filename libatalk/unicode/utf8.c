...
/* Given a unicode scalar, get a trail UTF-8 byte for a particular bit shift
 * amount */
...
struct charset_functions charset_utf8 = {"UTF8",
                                         0x08000103,
                                         utf8_pull,
                                         utf8_push,
                                         CHARSET_VOLUME | CHARSET_MULTIBYTE |
                                             CHARSET_PRECOMPOSED,
                                         NULL,
                                         NULL,
                                         NULL};
...
static size_t utf8_push(void *cd, char **inbuf, size_t *inbytesleft,
  ...
  while (*inbytesleft >= 2 && *outbytesleft >= 1) {
     ...
     * If  0xd800 <= uc <= 0xdfff, uc itself does not represent a Unicode
     * character. Rather, it is just part of a surrogate pair.  A surrogate pair
     * consists of a high surrogate in the range [0xd800 ... 0xdbff] and a low
     * surrogate in the range [0xdc00 ... 0xdfff].  Together the pair maps to a
     * single Unicode character whose scalar value is 64K or larger.  It is this
     * scalar value that is transformed to UTF-8, not the individual surrogates.
     *
