...
static size_t mac_chinese_simp_push(void *cd, char **inbuf, size_t *inbytesleft,
  ...
  return cjk_generic_push(mac_chinese_simp_char_push, cd, inbuf, inbytesleft,
                          outbuf, outbytesleft);
}
...
static size_t mac_chinese_simp_char_pull(ucs2_t *out, const uint8_t *in,
  ...
  return cjk_char_pull(
      cjk_lookup(c, mac_chinese_simp_2uni_index, mac_chinese_simp_2uni_charset),
      out, mac_chinese_simp_compose);
}

static size_t mac_chinese_simp_pull(void *cd, char **inbuf, size_t *inbytesleft,
                                    char **outbuf, size_t *outbytesleft) {
  return cjk_generic_pull(mac_chinese_simp_char_pull, cd, inbuf, inbytesleft,
                          outbuf, outbytesleft);
}
#endif
