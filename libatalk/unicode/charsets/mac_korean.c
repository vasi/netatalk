...
static size_t mac_korean_char_push(uint8_t *out, const ucs2_t *in,
  ...
  if ((wc & ~7) == 0xf860) {
    ...
    *size = 1;
    ...
    while ((comb & ~15) == 0xf870 || (comb >= 0x0300 && comb <= 0x036f) ||
  ...
  return cjk_char_push(
      cjk_lookup(wc, mac_korean_uni2_index, mac_korean_uni2_charset), out);
}

static size_t mac_korean_push(void *cd, char **inbuf, size_t *inbytesleft,
                              char **outbuf, size_t *outbytesleft) {
  return cjk_generic_push(mac_korean_char_push, cd, inbuf, inbytesleft, outbuf,
                          outbytesleft);
}
...
static size_t mac_korean_char_pull(ucs2_t *out, const uint8_t *in,
  ...
  return cjk_char_pull(
      cjk_lookup(c, mac_korean_2uni_index, mac_korean_2uni_charset), out,
      mac_korean_compose);
}

static size_t mac_korean_pull(void *cd, char **inbuf, size_t *inbytesleft,
                              char **outbuf, size_t *outbytesleft) {
  return cjk_generic_pull(mac_korean_char_pull, cd, inbuf, inbytesleft, outbuf,
                          outbytesleft);
}
#endif
