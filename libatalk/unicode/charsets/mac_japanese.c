...
static size_t mac_japanese_char_push(uint8_t *out, const ucs2_t *in,
  ...
  return cjk_char_push(
      cjk_lookup(wc, mac_japanese_uni2_index, mac_japanese_uni2_charset), out);
}

static size_t mac_japanese_push(void *cd, char **inbuf, size_t *inbytesleft,
                                char **outbuf, size_t *outbytesleft) {
  return cjk_generic_push(mac_japanese_char_push, cd, inbuf, inbytesleft,
                          outbuf, outbytesleft);
}
...
static size_t mac_japanese_char_pull(ucs2_t *out, const uint8_t *in,
  ...
  return cjk_char_pull(
      cjk_lookup(c, mac_japanese_2uni_index, mac_japanese_2uni_charset), out,
      mac_japanese_compose);
}

static size_t mac_japanese_pull(void *cd, char **inbuf, size_t *inbytesleft,
                                char **outbuf, size_t *outbytesleft) {
  return cjk_generic_pull(mac_japanese_char_pull, cd, inbuf, inbytesleft,
                          outbuf, outbytesleft);
}
#endif
