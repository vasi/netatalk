    ...
    if ((entry = namecache[i]) != NULL) {
      do {
        ...
        LOG(log_debug, logtype_default,
            "namecache{%d}: name:%s, uuid:%s, type%s: %s, cached: %s", i,
            entry->name, uuid_bin2string(entry->uuid),
            (entry->type & UUID_ENOENT) == UUID_ENOENT ? "[negative]" : "",
            uuidtype[entry->type & UUIDTYPESTR_MASK], timestr);
    ...
    if ((entry = uuidcache[i]) != NULL) {
      do {
        ...
        if (strftime(timestr, 200, "%c", tmp) == 0)
          continue;
        LOG(log_debug, logtype_default,
            "uuidcache{%d}: uuid:%s, name:%s, type%s: %s, cached: %s", i,
            uuid_bin2string(entry->uuid), entry->name,
            (entry->type & UUID_ENOENT) == UUID_ENOENT ? "[negative]" : "",
            uuidtype[entry->type & UUIDTYPESTR_MASK], timestr);
      } while ((entry = entry->next) != NULL);
    }
 ...
 * @args type     (rw) type (user or group) of name, returns found type here
 * which might mark it as a negative entry
  ...
  while (entry) {
    ...
    if (ret == 0 && *type == (entry->type & UUIDTYPESTR_MASK)) {
      ...
      if ((tim - entry->creationtime) > CACHESECONDS) {
        LOG(log_debug, logtype_default,
            "search_cachebyname: expired: name:\"%s\"", entry->name);
        /* remove item */
        if (entry->prev) {
