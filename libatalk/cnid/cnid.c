...
void cnid_register(struct _cnid_module *module) {
  ...
  list_for_each(
      ptr,
      &modules) if (0 == strcmp(list_entry(ptr, cnid_module, db_list)->name,
                                module->name)) {
    LOG(log_error, logtype_afpd,
        "Module with name [%s] is already registered !", module->name);
    return;
  }
...
static int cnid_dir(const char *dir, mode_t mask) {
  ...
  if (stat(dir, &st) < 0) {
    ...
    LOG(log_info, logtype_cnid, "Setting uid/gid to %d/%d", st.st_uid,
        st.st_gid);
    ...
    if (stat(tmp, &st1) < 0) /* use .AppleDB owner, if folder already exists */
      st1 = st;
    LOG(log_info, logtype_cnid, "Setting uid/gid to %d/%d", st1.st_uid,
        st1.st_gid);
...
struct _cnid_db *cnid_open(struct vol *vol, char *type, int flags) {
  ...
  struct cnid_open_args args = {.cnid_args_flags = flags, .cnid_args_vol = vol};
