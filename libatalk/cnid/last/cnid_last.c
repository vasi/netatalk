...
/* ------------------------ */
cnid_t cnid_last_add(struct _cnid_db *cdb, const struct stat *st, cnid_t did,
                     const char *name, size_t len, cnid_t hint) {

  /* FIXME: it relies on fact, that this is never called twice for the same
   * file/dir. */
...
cnid_t cnid_last_get(struct _cnid_db *cdb, cnid_t did, const char *name,
                     size_t len) {
  /* FIXME: it relies on fact, that this is never called twice for the same
   * file/dir. */
  /* Propably we should look through DID tree. */
...
/* */
cnid_t cnid_last_lookup(struct _cnid_db *cdb, const struct stat *st, cnid_t did,
...
static struct _cnid_db *cnid_last_new(struct vol *vol) {
  ...
  if ((cdb->cnid_db_private = calloc(1, sizeof(struct _cnid_last_private))) ==
      NULL) {
    free(cdb);
    return NULL;
  }
...
struct _cnid_module cnid_last_module = {
    "last", {NULL, NULL}, cnid_last_open, 0};

/* Return the did/name pair corresponding to a CNID. */
char *cnid_last_resolve(struct _cnid_db *cdb, cnid_t *id, void *buffer,
                        size_t len) {
  /* FIXME: frankly, it does not work. As get, add and other functions. */
  return NULL;
}

int cnid_last_update(struct _cnid_db *cdb, cnid_t id, const struct stat *st,
