      ...
      if (errno == EINPROGRESS) {
        ...
        if ((err = select(sock + 1, NULL, &wfds, NULL, &tv)) == 0) {
          ...
          LOG(log_error, logtype_cnid,
              "getfd: select timed out for CNID server %s", host);
          close(sock);
          sock = -1;
          continue;
        }
        if (err == -1) {
          /* select failed */
          LOG(log_error, logtype_cnid,
              "getfd: select failed for CNID server %s", host);
          close(sock);
          sock = -1;
          continue;
        }
        ...
        if (!FD_ISSET(sock, &wfds)) {
          /* give up */
          LOG(log_error, logtype_cnid,
              "getfd: socket not ready connecting to %s", host);
          close(sock);
          sock = -1;
          continue;
        }

        if ((err = getsockopt(sock, SOL_SOCKET, SO_ERROR, &optval, &optlen)) !=
                0 ||
            optval != 0) {
          if (err != 0) {
            /* somethings very wrong */
            LOG(log_error, logtype_cnid,
                "getfd: getsockopt error with CNID server %s: %s", host,
                strerror(errno));
          } else {
            errno = optval;
        ...
        LOG(log_error, logtype_cnid, "getfd: connect CNID server %s: %s", host,
            strerror(errno));
        close(sock);
        sock = -1;
        continue;
...
static int send_packet(CNID_bdb_private *db, struct cnid_dbd_rqst *rqst) {
  ...
  if (write_vec(db->fd, iov, towrite, vecs) != towrite) {
    LOG(log_warning, logtype_cnid,
        "send_packet: Error writev rqst (volume %s): %s", db->vol->v_localname,
        strerror(errno));
    return -1;
  }
...
static int dbd_rpc(CNID_bdb_private *db, struct cnid_dbd_rqst *rqst,
  ...
  if (rply->namelen && rply->namelen > len) {
    LOG(log_error, logtype_cnid,
        "dbd_rpc: Error reading name (volume %s): %s name too long: %d. only "
        "wanted %d, garbage?",
        db->vol->v_localname, rply->name, rply->namelen, len);
    return -1;
  }
  if (rply->namelen && (ret = readt(db->fd, rply->name, rply->namelen, 0,
...
static int transmit(CNID_bdb_private *db, struct cnid_dbd_rqst *rqst,
  ...
  transmit_fail:
    ...
    if (!clean) { /* don't sleep if just got disconnected by cnid server */
      ...
      if (t - orig > MAX_DELAY) {
        LOG(log_error, logtype_cnid,
            "transmit: Request to dbd daemon (volume %s) timed out.",
            db->vol->v_localname);
        return -1;
      }
...
/* ---------------------- */
cnid_t cnid_dbd_add(struct _cnid_db *cdb, const struct stat *st, cnid_t did,
  ...
  LOG(log_debug, logtype_cnid,
      "cnid_dbd_add: CNID: %u, name: '%s', dev: 0x%llx, inode: 0x%llx, type: "
      "%s",
...
char *cnid_dbd_resolve(struct _cnid_db *cdb, cnid_t *id, void *buffer,
  ...
  case CNID_DBD_RES_OK:
    *id = rply.did;
    ...
    LOG(log_debug, logtype_cnid,
        "cnid_dbd_resolve: resolved did: %u, name: '%s'", ntohl(*id), name);
    break;
  case CNID_DBD_RES_NOTFOUND:
    *id = CNID_INVALID;
    name = NULL;
...
cnid_t cnid_dbd_lookup(struct _cnid_db *cdb, const struct stat *st, cnid_t did,
  ...
  LOG(log_debug, logtype_cnid,
      "cnid_dbd_lookup: CNID: %u, name: '%s', inode: 0x%llx, type: %d (0=file, "
      "1=dir)",
...
int cnid_dbd_update(struct _cnid_db *cdb, cnid_t id, const struct stat *st,
  ...
  LOG(log_debug, logtype_cnid,
      "cnid_dbd_update: CNID: %u, name: '%s', inode: 0x%llx, type: %d (0=file, "
      "1=dir)",
...
cnid_t cnid_dbd_rebuild_add(struct _cnid_db *cdb, const struct stat *st,
  ...
  LOG(log_debug, logtype_cnid,
      "cnid_dbd_rebuild_add: CNID: %u, name: '%s', inode: 0x%llx, type: %d "
      "(0=file, 1=dir), hint: %u",
  ...
  case CNID_DBD_RES_OK:
    id = rply.cnid;
    LOG(log_debug, logtype_cnid, "cnid_dbd_rebuild_add: got CNID: %u",
        ntohl(id));
    break;
  case CNID_DBD_RES_ERR_MAX:
    errno = CNID_ERR_MAX;
...
int cnid_dbd_delete(struct _cnid_db *cdb, const cnid_t id) {
  ...
  switch (rply.result) {
  case CNID_DBD_RES_OK:
    LOG(log_debug, logtype_cnid, "cnid_dbd_delete: deleted CNID: %u",
        ntohl(id));
  case CNID_DBD_RES_NOTFOUND:
    return 0;
  case CNID_DBD_RES_ERR_DB:
...
struct _cnid_module cnid_dbd_module = {"dbd", {NULL, NULL}, cnid_dbd_open, 0};
