...
static int init_prepared_stmt_put(CNID_mysql_private *db) {
  ...
  EC_NEG1(asprintf(
      &sql, "INSERT INTO `%s` (Id,Name,Did,DevNo,InodeNo) VALUES(?,?,?,?,?)",
      db->cnid_mysql_voluuid_str));
...
static int cnid_mysql_execute(MYSQL *con, char *fmt, ...) {
  ...
  if (rv) {
    LOG(log_info, logtype_cnid, "MySQL query \"%s\", error: %s", sql,
        mysql_error(con));
    errno = CNID_ERR_DB;
  }
  free(sql);
...
int cnid_mysql_delete(struct _cnid_db *cdb, const cnid_t id) {
  ...
  LOG(log_debug, logtype_cnid, "cnid_mysql_delete(%" PRIu32 "): END",
      ntohl(id));
...
int cnid_mysql_update(struct _cnid_db *cdb, cnid_t id, const struct stat *st,
...
cnid_t cnid_mysql_lookup(struct _cnid_db *cdb, const struct stat *st,
                         cnid_t did, const char *name, size_t len) {
  EC_INIT;
  CNID_mysql_private *db;
  ...
  LOG(log_maxdebug, logtype_cnid,
      "cnid_mysql_lookup(did: %" PRIu32 ", name: \"%s\", hint: %" PRIu32
      "): START",
...
exec_stmt:
  ...
  case 1:
    /* either both OR clauses matched the same id or only one matched, handled
     * below */
  ...
  if (retdid != did || STRCMP(retname, !=, name)) {
    ...
    LOG(log_debug, logtype_cnid,
        "cnid_mysql_lookup(DID:%u, name: \"%s\"): changed dev/ino", ntohl(did),
        name);
    if (cnid_mysql_delete(cdb, retid) != 0) {
      LOG(log_error, logtype_cnid, "MySQL query error: %s",
          mysql_error(db->cnid_mysql_con));
...
cnid_t cnid_mysql_add(struct _cnid_db *cdb, const struct stat *st, cnid_t did,
    ...
    if ((id = cnid_mysql_lookup(cdb, st, did, name, len)) == CNID_INVALID) {
      ...
      if (lastid > 0xffffffff) {
        ...
        EC_NEG1(cnid_mysql_execute(
            ...
            "COMMIT;",
            db->cnid_mysql_voluuid_str, db->cnid_mysql_voluuid_str,
            db->cnid_mysql_voluuid_str));
        db->cnid_mysql_flags |= CNID_MYSQL_FLAG_DEPLETED;
        hint = CNID_INVALID;
        do {
...
cnid_t cnid_mysql_get(struct _cnid_db *cdb, cnid_t did, const char *name,
  ...
  LOG(log_debug, logtype_cnid,
      "cnid_mysql_get(did: %" PRIu32 ", name: \"%s\"): START", ntohl(did),
      name);
...
struct _cnid_db *cnid_mysql_open(struct cnid_open_args *args) {
  ...
  EC_NULL(mysql_real_connect(
      db->cnid_mysql_con, obj->options.cnid_mysql_host,
      obj->options.cnid_mysql_user, obj->options.cnid_mysql_pw,
      obj->options.cnid_mysql_db, 0, NULL, CLIENT_MULTI_STATEMENTS));

  /* Add volume to volume table */
  if (cnid_mysql_execute(
          db->cnid_mysql_con,
          "CREATE TABLE IF NOT EXISTS volumes"
          "(VolUUID CHAR(32) PRIMARY KEY,VolPath TEXT(4096),Stamp "
          "BINARY(8),Depleted INT, INDEX(VolPath(64)))")) {
    LOG(log_error, logtype_cnid, "MySQL query error: %s",
        mysql_error(db->cnid_mysql_con));
    EC_FAIL;
  }
   ...
   * If that's the case, in cnid_mysql_add() we'll ignore the CNID "hint" taken
   * from the AppleDouble file.
   */
  if (cnid_mysql_execute(db->cnid_mysql_con,
  ...
  LOG(log_debug, logtype_cnid,
      "Finished initializing MySQL CNID module for volume '%s'", vol->v_path);
...
struct _cnid_module cnid_mysql_module = {
    "mysql", {NULL, NULL}, cnid_mysql_open, 0};
