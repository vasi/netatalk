...
/* ----------------------------- */
static int for_each_adouble(const char *from, const char *name, rf_loop fn,
  ...
  if (NULL == (dp = opendir(name))) {
    if (!flag) {
      LOG(log_error, logtype_afpd, "%s: opendir %s: %s", from,
...
/* ----------------- */
static int RF_renamedir_adouble(VFS_FUNC_ARGS_RENAMEDIR) { return 0; }

/* ----------------- */
static int deletecurdir_adouble_loop(const struct vol *vol, struct dirent *de,
...
static int RF_setdirunixmode_adouble(VFS_FUNC_ARGS_SETDIRUNIXMODE) {
  ...
  if (dir_rx_set(mode)) {
    if (ochmod(ad_dir(adouble), (DIRBITS | mode) & ~vol->v_umask, st,
               vol_syml_opt(vol) | vol_chmod_opt(vol)) < 0)
      return -1;
  }
...
/* ----------------- */
static int setdirmode_adouble_loop(const struct vol *vol, struct dirent *de,
                                   char *name, void *data, int flag) {
  ...
  if (ostat(name, &st, vol_syml_opt(vol)) < 0) {
    if (flag)
      return 0;
    LOG(log_error, logtype_afpd, "setdirmode: stat %s: %s", name,
        strerror(errno));
...
static int RF_setdirowner_adouble(VFS_FUNC_ARGS_SETDIROWNER) {
  if (lchown(".AppleDouble", uid, gid) < 0 && errno != EPERM) {
    LOG(log_debug, logtype_afpd, "setdirowner: chown %d/%d %s: %s", uid, gid,
  ...
  if (unix_rename(dirfd, adsrc, -1, vol->ad_path(dst, 0)) < 0) {
    ...
    if (errno == ENOENT) {
      ...
      if (ostatat(dirfd, adsrc, &st,
                  vol_syml_opt(vol))) /* source has no ressource fork, */
        return 0;
       ...
       */
      ad_init(&ad, vol);
      if (ad_open(&ad, dst, ADFLAGS_HF | ADFLAGS_RDWR | ADFLAGS_CREATE, 0666) ==
          0) {
...
/* ---------------- */
static int RF_renamedir_ea(VFS_FUNC_ARGS_RENAMEDIR) { return 0; }
...
static int deletecurdir_ea_osx_loop(const struct vol *vol, struct dirent *de,
                                    char *name, void *data, int flag) {
  int ret;
  struct stat sb;
...
static int RF_deletecurdir_ea(VFS_FUNC_ARGS_DELETECURDIR) {
  ...
  if ((err = for_each_adouble("deletecurdir_ea_osx", ".",
                              deletecurdir_ea_osx_loop, vol, NULL, 0)) != 0)
    return err;
...
/*
 * Up until we really start stacking many VFS modules on top of one another or
 * use dynamic module loading like we do for UAMs, up until then we just stack
 * VFS modules via an fixed size array. All VFS funcs must return AFP_ERR codes.
 * When a func in the chain returns an error this error code will be returned to
 * the caller, BUT the chain in followed and all following funcs are called in
