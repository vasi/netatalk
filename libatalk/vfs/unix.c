...
int setfilmode(const struct vol *vol, const char *name, mode_t mode,
  ...
  if (ochmod((char *)name, mode & ~vol->v_umask, st,
 ...
 *
 * Supports *at semantics (cf openat) if HAVE_ATFUNCS. Pass dirfd=-1 to ignore
 * this.
 */
 ...
 * Supports *at semantics if HAVE_ATFUNCS, pass dirfd=-1 to ignore this
 */
int copy_ea(const char *ea, int dirfd, const char *src, const char *dst,
            mode_t mode) {
  EC_INIT;
  int sfd = -1;
  int dfd = -1;
 ...
 *
 * unix_rename mulitplexes rename and renameat. If we don't HAVE_ATFUNCS, sfd
 * and dfd are ignored.
 *
 ...
 *
 * statat mulitplexes stat and fstatat. If we don't HAVE_ATFUNCS, dirfd is
 * ignored.
 *
