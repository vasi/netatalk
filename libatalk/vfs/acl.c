...
int remove_acl_vfs(const char *name) {
  ...
  if ((acl(name, ACE_SETACL, trivial_aces, new_aces)) == 0)
    ...
    LOG(log_error, logtype_afpd, "set_acl: error setting acl: %s",
        strerror(errno));
...
/*!
 * Remove any ACL_USER, ACL_GROUP, ACL_MASK or ACL_TYPE_DEFAULT ACEs from an
 * object
 *
...
int remove_acl_vfs(const char *name) {
  ...
  /* Now get ACL entries, then re-set the ACL again. */
