 ...
 *
 * - create header with with the format struct adouble_ea_ondisk, the file is
 * written to
 *   ".AppleDouble/fileWithEAs::EA"
 * - store EAs in files "fileWithEAs::EA::testEA1" and
 * "fileWithEAs::EA::testEA2"
 */
...
static inline mode_t ea_header_mode(mode_t mode) {
  ...
  /* Owner must be able to open, read and w-lock it, in order to chmod from eg
   * 0000 -> 0xxxx*/
...
static char *mtoupath(const struct vol *vol, const char *mpath) {
  static char
      upath[MAXPATHLEN + 2]; /* for convert_charset dest_len parameter +2 */
  const char *m;
  char *u;
  size_t inplen;
  size_t outlen;
  ...
  if ((size_t)-1 == (outlen = convert_charset(CH_UTF8_MAC, vol->v_volcharset,
                                              vol->v_maccharset, m, inplen, u,
                                              outlen, &flags))) {
    return NULL;
  }
 ...
 *
 * Purpose: unpack and verify header file data buffer at ea->ea_data into struct
 * ea
 *
 * Arguments:
 *
...
static int unpack_header(struct ea *restrict ea) {
  ...
  LOG(log_debug, logtype_afpd, "unpack_header: number of EAs: %u",
      ea->ea_count);
  buf += 2;
 ...
 * Effects:
 *
 * Grow array ea->ea_entries[]. If ea->ea_entries is still NULL, start
 * allocating. Otherwise realloc and put entry at the end. Increments
 * ea->ea_count.
 */
static int ea_addentry(struct ea *restrict ea, const char *restrict attruname,
                       size_t attrsize, int bitmap) {
  int ea_existed = 0;
  unsigned int count = 0;
  void *tmprealloc;
  ...
  if (ea->ea_count == 0) {
    ...
    tmprealloc =
        realloc(ea->ea_entries, sizeof(struct ea_entry) * (ea->ea_count + 1));
    if (!tmprealloc) {
      LOG(log_error, logtype_afpd, "ea_addentry: OOM");
      return -1;
    }
 ...
 * try to create a file with O_CREAT | O_EXCL, but the whole process in not
 * atomic. What do we do then? Someone else is in the process of creating the
 * header too, but it might not have finished it. That means we cant just open,
 * read and use it! We therefor currently just break with an error. On return
 * the header file is still r/w locked.
 */
static int create_ea_header(const char *restrict uname,
  ...
  if ((fd = open(uname, O_RDWR | O_CREAT | O_EXCL, 0666 & ~ea->vol->v_umask)) ==
      -1) {
...
static int write_ea(const struct ea *restrict ea,
                    const char *restrict attruname, const char *restrict ibuf,
                    size_t attrsize) {
  int fd = -1, ret = AFP_OK;
  struct stat st;
  char *eaname;
  ...
  if ((fd = open(eaname, O_RDWR | O_CREAT | O_EXCL,
...
static int ea_delentry(struct ea *restrict ea, const char *restrict attruname) {
  ...
  if (ea->ea_count == 0) {
    LOG(log_error, logtype_afpd,
        "ea_delentry('%s'): illegal ea_count of 0 on deletion", attruname);
    return -1;
  }
...
static int delete_ea_file(const struct ea *restrict ea, const char *eaname) {
  ...
  if ((stat(eafile, &st)) == 0) {
    if ((unlink(eafile)) != 0) {
      LOG(log_error, logtype_afpd, "delete_ea_file('%s'): unlink: %s", eafile,
          strerror(errno));
      ret = -1;
    } else
...
char *ea_path(const struct ea *restrict ea, const char *restrict eaname,
  ...
  adname =
      ea->vol->ad_path(ea->filename, (ea->ea_flags & EA_DIR) ? ADFLAGS_DIR : 0);
 ...
 * Function: ea_open
 *
 * Purpose: open EA header file, create if it doesnt exits and called with
 * O_CREATE
 *
 * Arguments:
 *
 ...
 *    flags       (r) EA_CREATE: create if it doesn't exist (without it won't be
 * created) EA_RDONLY: open read only EA_RDWR: open read/write Eiterh EA_RDONLY
 * or EA_RDWR MUST be requested ea          (w) pointer to a struct ea that we
 * fill
 *
 ...
 * Effects:
 *
 * opens header file and stores fd in ea->ea_fd. Size of file is put into
 * ea->ea_size. number of EAs is stored in ea->ea_count. flags are remembered in
 * ea->ea_flags. file is either read or write locked depending on the open
 * flags. When you're done with struct ea you must call ea_close on it.
 */
int ea_open(const struct vol *restrict vol, const char *restrict uname,
            eaflags_t eaflags, struct ea *restrict ea) {
  int ret = 0;
  char *eaname;
  struct stat st;
  ...
  if ((ea->ea_fd =
           open(eaname, (ea->ea_flags & EA_RDWR) ? O_RDWR : O_RDONLY)) == -1) {
  ...
  if (ea->ea_flags & EA_RDONLY) {
    ...
    if ((read_lock(ea->ea_fd, 0, SEEK_SET, 0)) != 0) {
      LOG(log_error, logtype_afpd, "ea_open: lock error on  header: %s",
          eaname);
      ret = -1;
      goto exit;
    }
  ...
  if ((unpack_header(ea)) != 0) {
    LOG(log_error, logtype_afpd, "ea_open: error unpacking header for: %s",
        eaname);
    ret = -1;
    goto exit;
  }
 ...
 * flags. When you're done with struct ea you must call ea_close on it.
 */
int ea_openat(const struct vol *restrict vol, int dirfd,
              const char *restrict uname, eaflags_t eaflags,
              struct ea *restrict ea) {
  int ret = 0;
  int cwdfd = -1;
      ...
      if (ea->ea_count == 0) {
        ...
        if ((statat(ea->dirfd, eaname, &st)) == 0) {
          if ((netatalk_unlinkat(ea->dirfd, eaname)) != 0) {
            LOG(log_error, logtype_afpd, "ea_close('%s'): unlink: %s", eaname,
                strerror(errno));
            ret = -1;
          } else
            LOG(log_debug, logtype_afpd, "ea_close(unlink '%s'): success",
                eaname);
        } else {
          /* stat error */
          if (errno != ENOENT) {
            LOG(log_error, logtype_afpd, "ea_close('%s'): stat: %s", eaname,
                strerror(errno));
            ret = -1;
          }
        }
        ...
        if ((ftruncate(ea->ea_fd, 0)) == -1) {
          LOG(log_error, logtype_afpd, "ea_close: ftruncate: %s",
              strerror(errno));
          ret = -1;
          goto exit;
        }

        if (write(ea->ea_fd, ea->ea_data, ea->ea_size) !=
            (ssize_t)ea->ea_size) {
...
int get_easize(VFS_FUNC_ARGS_EA_GETSIZE) {
  ...
  if ((ea_open(vol, uname, EA_RDONLY, &ea)) != 0) {
    if (errno != ENOENT)
      LOG(log_error, logtype_afpd,
          "get_easize: error calling ea_open for file: %s", uname);
  ...
  while (count < ea.ea_count) {
    if (strcmp(attruname, (*ea.ea_entries)[count].ea_name) == 0) {
      ...
      *rbuflen += 4;
      ...
      LOG(log_debug, logtype_afpd, "get_easize(\"%s\"): size: %u", attruname,
  ...
  if ((ea_close(&ea)) != 0) {
    LOG(log_error, logtype_afpd,
        "get_easize: error closing ea handle for file: %s", uname);
    return AFPERR_MISC;
  }
  ...
  while (count < ea.ea_count) {
    if (strcmp(attruname, (*ea.ea_entries)[count].ea_name) == 0) {
      ...
      if ((fd = open(eafile, O_RDONLY)) == -1) {
        LOG(log_error, logtype_afpd, "get_eacontent('%s'): open error: %s",
      ...
      toread = (maxreply < (*ea.ea_entries)[count].ea_size)
                   ? maxreply
                   : (*ea.ea_entries)[count].ea_size;
      LOG(log_debug, logtype_afpd, "get_eacontent('%s'): sending %u bytes",
          attruname, toread);
      ...
      *rbuflen += 4;
      ...
      if (read(fd, rbuf, toread) != (ssize_t)toread) {
        LOG(log_error, logtype_afpd, "get_eacontent('%s/%s'): short read",
            uname, attruname);
        close(fd);
        ret = AFPERR_MISC;
        break;
...
int list_eas(VFS_FUNC_ARGS_EA_LIST) {
  ...
  if ((ea_open(vol, uname, EA_RDONLY, &ea)) != 0) {
    if (errno != ENOENT) {
      LOG(log_error, logtype_afpd,
          "list_eas: error calling ea_open for file: %s", uname);
      return AFPERR_MISC;
    } else
      return AFP_OK;
  }
  ...
  while (count < ea.ea_count) {
    ...
    if ((len = convert_string(
             vol->v_volcharset, CH_UTF8_MAC, (*ea.ea_entries)[count].ea_name,
             (*ea.ea_entries)[count].ea_namelen, buf + attrbuflen, 255)) <= 0) {
      ret = AFPERR_MISC;
      goto exit;
    }
    if (len == 255)
    ...
    LOG(log_debug7, logtype_afpd, "list_eas(%s): EA: %s", uname,
...
exit:
  *buflen = attrbuflen;
  ...
  if ((ea_close(&ea)) != 0) {
    LOG(log_error, logtype_afpd,
        "list_eas: error closing ea handle for file: %s", uname);
    return AFPERR_MISC;
  }
...
int ea_renamefile(VFS_FUNC_ARGS_RENAMEFILE) {
  ...
  if ((ea_openat(vol, dirfd, src, EA_RDWR, &srcea)) != 0) {
    if (errno == ENOENT)
      ...
      LOG(log_error, logtype_afpd,
          "ea_renamefile('%s'/'%s'): ea_open error: '%s'", src, dst, src);
      return AFPERR_MISC;
    }
  }
  ...
  if ((ea_open(vol, dst, EA_RDWR | EA_CREATE, &dstea)) != 0) {
    if (errno == ENOENT) {
      /* Possibly the .AppleDouble folder didn't exist, we create it and try
       * again */
      ad_init(&ad, vol);
      if ((ad_open(&ad, dst, ADFLAGS_HF | ADFLAGS_RDWR | ADFLAGS_CREATE,
                   0666)) != 0) {
        LOG(log_error, logtype_afpd,
            "ea_renamefile('%s/%s'): ad_open error: '%s'", src, dst, dst);
        ret = AFPERR_MISC;
        goto exit;
      }
  ...
  while (count < srcea.ea_count) {
    ...
    LOG(log_maxdebug, logtype_afpd,
        "ea_renamefile('%s/%s'): moving EA '%s' to '%s'", src, dst, srceapath,
        eapath);
...
int ea_copyfile(VFS_FUNC_ARGS_COPYFILE) {
  ...
  if ((ea_openat(vol, sfd, src, EA_RDWR, &srcea)) != 0) {
    if (errno == ENOENT)
      ...
      LOG(log_error, logtype_afpd,
          "ea_copyfile('%s'/'%s'): ea_open error: '%s'", src, dst, src);
      return AFPERR_MISC;
    }
  }
  ...
  if ((ea_open(vol, dst, EA_RDWR | EA_CREATE, &dstea)) != 0) {
    if (errno == ENOENT) {
      ...
      if ((ad_open(&ad, dst, ADFLAGS_HF | ADFLAGS_RDWR | ADFLAGS_CREATE,
                   0666)) != 0) {
        LOG(log_error, logtype_afpd,
            "ea_copyfile('%s/%s'): ad_open error: '%s'", src, dst, dst);
        ret = AFPERR_MISC;
        goto exit;
      }
      ad_close(&ad, ADFLAGS_HF);
  ...
  while (count < srcea.ea_count) {
    ...
    LOG(log_maxdebug, logtype_afpd,
        "ea_copyfile('%s/%s'): copying EA '%s' to '%s'", src, dst, srceapath,
        eapath);

    /* Add EA to dstea */
    if ((ea_addentry(&dstea, eaname, easize, 0)) == -1) {
      LOG(log_error, logtype_afpd,
          "ea_copyfile('%s/%s'): ea_addentry('%s') error", src, dst, eaname);
      ret = AFPERR_MISC;
      goto exit;
    }
...
int ea_chown(VFS_FUNC_ARGS_CHOWN) {
  ...
  if ((ea_open(vol, path, EA_RDWR, &ea)) != 0) {
    if (errno == ENOENT)
      ...
      LOG(log_error, logtype_afpd, "ea_chown('%s'): error calling ea_open",
          path);
      return AFPERR_MISC;
    }
  }
...
exit:
  if ((ea_close(&ea)) != 0) {
    LOG(log_error, logtype_afpd, "ea_chown('%s'): error closing ea handle",
        path);
    return AFPERR_MISC;
  }
...
int ea_chmod_file(VFS_FUNC_ARGS_SETFILEMODE) {
  ...
  if ((setfilmode(vol, ea_path(&ea, NULL, 0), ea_header_mode(mode), NULL)) !=
      0) {
    LOG(log_error, logtype_afpd, "ea_chmod_file('%s'): %s",
        ea_path(&ea, NULL, 0), strerror(errno));
    switch (errno) {
    case EPERM:
    case EACCES:
      ret = AFPERR_ACCESS;
...
int ea_chmod_dir(VFS_FUNC_ARGS_SETDIRUNIXMODE) {
  ...
  if ((setfilmode(vol, ea_path(&ea, NULL, 0), ea_header_mode(mode), NULL)) !=
      0) {
    LOG(log_error, logtype_afpd, "ea_chmod_dir('%s'): %s",
        ea_path(&ea, NULL, 0), strerror(errno));
    switch (errno) {
    case EPERM:
    case EACCES:
  ...
  while (count < ea.ea_count) {
     ...
     * /.....
     * FIXME:
     * Until the EA code escapes / in EA name requests from the client, these
     * therefor wont work.
     */
    if ((eaname_safe = strrchr(eaname, '/'))) {
      LOG(log_warning, logtype_afpd, "ea_chmod_dir('%s'): contains a slash",
          eaname);
      eaname = eaname_safe;
    }
    ...
    if ((setfilmode(vol, eaname, ea_mode(mode), NULL)) != 0) {
      LOG(log_error, logtype_afpd, "ea_chmod_dir('%s'): %s", eaname,
          strerror(errno));
      switch (errno) {
      case EPERM:
      case EACCES:
