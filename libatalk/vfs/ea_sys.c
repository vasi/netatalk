...
int sys_get_easize(VFS_FUNC_ARGS_EA_GETSIZE) {
  ...
  if (ret == -1) {
    ...
    *rbuflen += 4;
    ...
    default:
      LOG(log_debug, logtype_afpd, "sys_getextattr_size: error: %s",
          strerror(errno));
      return AFPERR_MISC;
    }
  }
  ...
  LOG(log_debug7, logtype_afpd,
      "sys_getextattr_size(%s): attribute: \"%s\", size: %u", uname, attruname,
      ret);
...
int sys_get_eacontent(VFS_FUNC_ARGS_EA_GETCONTENT) {
  ...
  LOG(log_debug7, logtype_afpd,
      "sys_getextattr_content(%s): attribute: \"%s\", size: %u", uname,
      attruname, maxreply);
  if (vol->v_flags & AFPVOL_EA_SAMBA) {
    extra = 1;
  }
  ...
  if (fd != -1) {
    LOG(log_debug, logtype_afpd,
        "sys_get_eacontent(%s): file is already opened", uname);
  ...
  if (ret == -1) {
    ...
    *rbuflen += 4;
    ...
    default:
      LOG(log_debug, logtype_afpd, "sys_getextattr_content(%s): error: %s",
...
int sys_list_eas(VFS_FUNC_ARGS_EA_LIST) {
  ...
  /* PBaranski fix */
  if (fd != -1) {
    LOG(log_debug, logtype_afpd, "sys_list_eas(%s): file is already opened",
        uname);
  ...
  while (ret > 0) {
    ...
    if (NOT_NETATALK_EA(ptr)) {
      ...
      LOG(log_debug7, logtype_afpd, "sys_list_extattr(%s): attribute: %s",
          uname, ptr);
...
int sys_set_ea(VFS_FUNC_ARGS_EA_SET) {
  ...
  /* PBaranski fix */
  if (fd != -1) {
    LOG(log_debug, logtype_afpd, "sys_set_ea(%s): file is already opened",
        uname);
  ...
  if (ret == -1) {
    ...
    case OPEN_NOFOLLOW_ERRNO:
      /* its a symlink and client requested O_NOFOLLOW  */
      LOG(log_debug, logtype_afpd,
          "sys_set_ea(\"%s\", ea:'%s'): symlink with kXAttrNoFollow", uname,
          attruname);
      return AFP_OK;
    case EEXIST:
      LOG(log_debug, logtype_afpd,
          "sys_set_ea(\"%s/%s\", ea:'%s'): EA already exists", getcwdpath(),
          uname, attruname);
      return AFPERR_EXIST;
    case ENOATTR:
    case ENOENT:
    ...
    default:
      LOG(log_debug, logtype_afpd,
          ...
          oflag & O_NOFOLLOW ? "O_NOFOLLOW" : "-", strerror(errno));
      return AFPERR_MISC;
    }
  }
...
getout:
  ...
  if (ret == -1) {
    ...
    case ENOENT:
      ...
      break;
    case EACCES:
      LOG(log_debug, logtype_afpd, "sys_ea_copyfile(%s, %s): error: %s", src,
          dst, strerror(errno));
      return AFPERR_ACCESS;
    default:
      LOG(log_error, logtype_afpd, "sys_ea_copyfile(%s, %s): error: %s", src,
          dst, strerror(errno));
      return AFPERR_MISC;
    }
  }
