...
#define LOGLEVEL_STRING_IDENTIFIERS                                            \
  {                                                                            \
    "-", "severe", "error", "warn", "note", "info", "debug", "debug6",         \
        "debug7", "debug8", "debug9", "maxdebug"                               \
  }

/* these are the string identifiers corresponding to each logtype */
#define LOGTYPE_STRING_IDENTIFIERS                                             \
  {                                                                            \
    "Default", "Logger", "CNID", "AFPDaemon", "DSI", "UAMS", "FCE", "ad",      \
        "Spotlight", "end_of_list_marker"                                      \
  }
...
static void syslog_setup(int loglevel, enum logtypes logtype,
...
static int generate_message(char **message_details_buffer, char *user_message,
                            int display_options, enum loglevels loglevel,
  ...
  len = asprintf(&details, "%s%06u %s[%d] {%s:%d} (%s:%s): %s\n", buf,
                 (int)tv.tv_usec, log_config.processname, pid, basename,
                 log_src_linenumber, arr_loglevel_strings[loglevel],
                 arr_logtype_strings[logtype], user_message);
  if (len == -1) {
...
static void make_syslog_entry(enum loglevels loglevel, enum logtypes logtype,
                              char *message) {
...
static void log_init(void) {
  syslog_setup(log_info, logtype_default, logoption_ndelay | logoption_pid,
               logfacility_daemon);
}
...
static void log_setup(const char *filename, enum loglevels loglevel,
  ...
  if (strcmp(filename, "/dev/tty") == 0) {
    ...
    become_root();
    type_configs[logtype].fd = open(filename, O_CREAT | O_WRONLY | O_APPEND,
...
static void setuplog_internal(const char *loglevel, const char *logtype,
  ...
  if (filename == NULL) {
    /* must be syslog */
    syslog_setup(levelnum, typenum, logoption_ndelay | logoption_pid,
                 logfacility_daemon);
  } else {
...
void make_log_entry(enum loglevels loglevel, enum logtypes logtype,
  ...
  len = generate_message(&log_message, user_message,
