...
int server_child_remove(server_child_t *children, pid_t pid) {
  ...
  /* In main:child_handler() we need the fd in order to remove it from the
   * pollfd set */
 ...
 */
int server_child_transfer_session(server_child_t *children, pid_t pid,
                                  uid_t uid, int afp_socket,
  ...
  if ((child = server_child_resolve(children, pid)) == NULL) {
    ...
    if (kill(pid, 0) == 0) {
      LOG(log_note, logtype_default, "Reconnect: terminating old session[%u]",
          pid);
      kill(pid, SIGTERM);
      sleep(2);
      if (kill(pid, 0) == 0) {
        LOG(log_error, logtype_default, "Reconnect: killing old session[%u]",
            pid);
        kill(pid, SIGKILL);
        sleep(2);
      }
...
void server_child_kill_one_by_id(server_child_t *children, pid_t pid, uid_t uid,
                                 uint32_t idlen, char *id, uint32_t boottime) {
  afp_child_t *child, *tmp;
  int i;
  ...
  for (i = 0; i < CHILD_HASHSIZE; i++) {
    ...
    while (child) {
      ...
      if (child->afpch_pid != pid) {
        if (child->afpch_idlen == idlen &&
          ...
          if (child->afpch_boottime != boottime) {
            ...
            LOG(log_debug, logtype_default,
                "Found another session[%u] for client[%u]", child->afpch_pid,
                pid);
          }
        }
      } else {
        ...
        if (child->afpch_clientid)
          free(child->afpch_clientid);
        LOG(log_debug, logtype_default, "Setting client ID for %u",
            child->afpch_pid);
        child->afpch_uid = uid;
        child->afpch_valid = 1;
