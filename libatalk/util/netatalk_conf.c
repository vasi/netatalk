...
static int have_uservol = 0; /* whether there's generic user home share in
                                config ("~" or "~/path", but not "~user") */
...
static char *get_vol_uuid(const AFPObj *obj, const char *volname) {
  ...
  if ((fp = fopen(obj->options.uuidconf, "r")) != NULL) { /* read open? */
    ...
    while (fgets(buf, sizeof(buf), fp) != NULL) {
      ...
      if (sscanf(p, "%36s", uuid) == 1) {
        ...
        LOG(log_debug, logtype_afpd, "get_uuid('%s'): UUID: '%s'", volname,
            uuid);
        fclose(fp);
        return strdup(uuid);
      }
  ...
  } else if ((fp = fopen(obj->options.uuidconf, "a+")) ==
             NULL) { /* not found */
...
static int do_check_ea_support(const struct vol *vol) {
  ...
  if ((sys_setxattr(vol->v_path, eaname, eacontent, strlen(eacontent) + 1,
    ...
    LOG(log_warning, logtype_afpd,
        "volume \"%s\" does not support Extended Attributes or read-only "
        "volume",
        vol->v_localname);
    haseas = 0;
  }
 ...
 *
 * Using this information we can reject xlation of any variable depeninding on a
 * login context which is not given in the afp master, where we must evaluate
 * this whole stuff too for the Zeroconf announcements.
 */
static char *volxlate(const AFPObj *obj, char *dest, size_t destlen,
                      const char *src, const struct passwd *pwd,
...
static int hostaccessvol(const AFPObj *obj, const char *volname,
                         const char *args) {
  int mask_int;
 ...
 * @param opt     (r) option
 * @param defsec  (r) if "option" is not found in "vol", try to find it in
 * section "defsec"
 * @param defval  (r) if neither "vol" nor "defsec" contain "opt" return
 * "defval"
 *
 * @returns       const option string from "vol" or "defsec", or "defval" if not
 * found
 */
static const char *getoption(const dictionary *conf, const char *vol,
                             const char *opt, const char *defsec,
                             const char *defval) {
  const char *result;

  if ((!(result = atalk_iniparser_getstring(conf, vol, opt, NULL))) &&
      (defsec != NULL))
...
static int getoption_bool(const dictionary *conf, const char *vol,
  ...
  if (((result = atalk_iniparser_getboolean(conf, vol, opt, -1)) == -1) &&
      (defsec != NULL))
...
/*!
 * Get boolean option from volume, default section or global - use default value
 * if not set
 *
 ...
 *
 * @param obj      (r) handle
 * @param pwd      (r) struct passwd of logged in user, may be NULL in master
 * afpd
 * @param section  (r) volume name wo variables expanded (exactly as in
 * iniconfig)
 ...
 */
static struct vol *creatvol(AFPObj *obj, const struct passwd *pwd,
                            const char *section, const char *name,
                            const char *path_in, const char *preset) {
  EC_INIT;
  ...
  LOG(log_debug, logtype_afpd,
      "creatvol(volume: '%s', path: \"%s\", preset: '%s'): BEGIN", name, path,
      preset ? preset : "-");
  ...
  /* Once volumes are loaded, we never change options again, we just delete em
   * when they're removed from afp.conf */
  ...
  for (struct vol *vol = Volumes; vol; vol = vol->v_next) {
    ...
    if (STRCMP(path, ==, vol->v_path)) {
      LOG(log_note, logtype_afpd,
          "volume \"%s\" path \"%s\" is the same as volumes \"%s\" path", name,
          path, vol->v_configname);
      EC_EXIT_STATUS(0);
    }
    /*
  ...
  if (pwd) {
    if (accessvol(
            obj,
            getoption(obj->iniconfig, section, "invalid users", preset, NULL),
            pwd->pw_name) == 1)
      goto EC_CLEANUP;
    if (accessvol(
            obj,
            getoption(obj->iniconfig, section, "valid users", preset, NULL),
            pwd->pw_name) == 0)
      goto EC_CLEANUP;
    if (hostaccessvol(obj, section,
                      getoption(obj->iniconfig, section, "hosts deny", preset,
                                NULL)) == 1)
      goto EC_CLEANUP;
    if (hostaccessvol(obj, section,
                      getoption(obj->iniconfig, section, "hosts allow", preset,
                                NULL)) == 0)
      goto EC_CLEANUP;
  }
  ...
  if (atalk_iniparser_getboolean(obj->iniconfig, INISEC_GLOBAL, "vol dbnest",
                                 0)) {
    ...
    if (val == NULL) {
      /* check global option */
      global_path = (char *)atalk_iniparser_getstring(
          obj->iniconfig, INISEC_GLOBAL, "vol dbpath", NULL);
      if (global_path) {
  ...
  if ((val =
           getoption(obj->iniconfig, section, "directory perm", preset, NULL)))
...
static int volfile_changed(AFPObj *obj) {
  ...
  includefile =
      atalk_iniparser_getstring(obj->iniconfig, INISEC_GLOBAL, "include", NULL);
  if (includefile) {
...
static int readvolfile(AFPObj *obj, const struct passwd *pwent) {
  ...
  if ((default_preset = atalk_iniparser_getstring(obj->iniconfig, INISEC_GLOBAL,
                                                  "vol preset", NULL))) {
    LOG(log_debug, logtype_afpd, "readvolfile: default_preset: %s",
        default_preset);
  }
  ...
  for (i = 0; i < secnum; i++) {
    ...
    if (STRCMP(secname, ==, INISEC_HOMES)) {
      ...
      LOG(log_debug, logtype_afpd, "readvolfile: pwent->pw_dir: '%s'",
          pwent->pw_dir);

      if ((realpath(pwent->pw_dir, tmp)) == NULL) {
        LOG(log_debug, logtype_afpd,
            "readvolfile: Cannot get realpath '%s' (%s).", pwent->pw_dir,
            strerror(errno));
        continue;
      }
      LOG(log_debug, logtype_afpd, "readvolfile: realpath pwent->pw_dir: '%s'",
          tmp);

      /* check if user home matches our "basedir regex" */
      if ((basedir = atalk_iniparser_getstring(
               obj->iniconfig, INISEC_HOMES, "basedir regex", NULL)) == NULL) {
      ...
      if (regexerr != 0 &&
          (regexerr = regcomp(&reg, basedir, REG_EXTENDED)) != 0) {
        char errbuf[1024];
        regerror(regexerr, &reg, errbuf, sizeof(errbuf));
        LOG(log_debug, logtype_default, "readvolfile: bad basedir regex: %s",
            errbuf);
        continue;
      }
      ...
      if ((p = atalk_iniparser_getstring(obj->iniconfig, INISEC_HOMES, "path",
                                         NULL))) {
      ...
      /* Get path */
      if ((p = atalk_iniparser_getstring(obj->iniconfig, secname, "path",
                                         NULL)) == NULL)
        continue;
    ...
    if (STRCMP(secname, ==, INISEC_HOMES)) {
      p = atalk_iniparser_getstring(obj->iniconfig, INISEC_HOMES, "home name",
                                    "$u's home");
      if (strstr(p, "$u") == NULL) {
    ...
    if (volxlate(obj, volname, sizeof(volname) - 1, tmp, pwent, path, NULL) ==
        NULL)
      continue;

    preset =
        atalk_iniparser_getstring(obj->iniconfig, secname, "vol preset", NULL);
...
static int readextmap(const char *file) {
  ...
  EC_NULL_LOGSTR(fp = fopen(file, "r"),
                 "Couldn't open extension maping file %s", file);
  ...
  while (fgets(buf, sizeof(buf), fp) != NULL) {
    ...
    case '.':
      ...
      LOG(log_debug, logtype_afpd, "readextmap: mapping: '%s' -> %s/%s", ext,
          type, creator);
      break;
    }
  }
 ...
 *                     LV_DEFAULT: load shares in a user/session context, this
 * honors authorisation LV_ALL: load shares that are available in the config
 * file LV_FORCE: reload file even though the timestamp wasn't changed
 */
int load_volumes(AFPObj *obj, lv_flags_t flags) {
  ...
  if (!(flags & LV_ALL)) {
    ...
    if (pwresult == NULL) {
      LOG(log_error, logtype_afpd, "load_volumes: getpwuid_r: %s",
          strerror(errno));
      EC_FAIL;
    }
    pwresult = &pwent;
  ...
  if (Volumes) {
    ...
    if (obj->uid && pwresult) {
      ...
      unbecome_root();
      if (ret != 0) {
        LOG(log_error, logtype_afpd, "load_volumes: set_groups: %s",
            strerror(errno));
        EC_FAIL;
      }
    }
    ...
    includefile = (char *)atalk_iniparser_getstring(
        obj->iniconfig, INISEC_GLOBAL, "include", NULL);
    if (includefile) {
  ...
  /* try putting a read lock on the volume file twice, sleep 1 second if first
   * attempt fails */
  ...
  while (retries < 2) {
    if ((read_lock(fd, 0, SEEK_SET, 0)) != 0) {
      ...
      if (!retries) {
        LOG(log_error, logtype_afpd,
            "readvolfile: can't lock configfile \"%s\"",
  ...
  while (vol) {
    if (vol->v_deleted && !(vol->v_flags & AFPVOL_OPEN)) {
      LOG(log_debug, logtype_afpd, "load_volumes: deleted: %s",
          vol->v_localname);
      nextvol = vol->v_next;
      if (prevvol) {
 ...
 *
 * getvolbypath() assumes that the user home directory has the same name as the
 * username. If that is not true, getuserbypath() is called and tries to
 * retrieve the username from the directory owner, checking its validity.
 *
...
static char *getuserbypath(const char *path) {
  ...
  LOG(log_debug, logtype_afpd,
      "getuserbypath: match user: %s, home: %s, realhome: %s", pwd->pw_name,
      pwd->pw_dir, hdir);
 ...
 * (3) If there is, match "path" with "basedir regex" to get the user home
 * parent dir (4) Built user home path by appending the basedir matched in (3)
 * and appending the username (5) The next path element then is the username
 * (5b) getvolbypath() assumes that the user home directory has the same name as
 * the username. If that is not true, getuserbypath() is called and tries to
 * retrieve the username from the directory owner, checking its validity (6)
 * Append [Homes]->path subdirectory if defined (7) Create volume
 *
 * @param obj  (rw) handle
...
struct vol *getvolbypath(AFPObj *obj, const char *path) {
  ...
  for (tmp = Volumes; tmp; tmp = tmp->v_next) { /* (1) */
    ...
    if (strncmp(path, tmp->v_path, v_path_len) == 0) {
      if (v_path_len < strlen(path) && path[v_path_len] != '/') {
        LOG(log_debug, logtype_afpd,
            "getvolbypath: path(\"%s\") != volume(\"%s\")", path, tmp->v_path);
      } else {
        LOG(log_debug, logtype_afpd,
            "getvolbypath: path(\"%s\") == volume(\"%s\")", path, tmp->v_path);
        vol = tmp;
  ...
  /* (3) */
  EC_NULL_LOG(basedir = atalk_iniparser_getstring(obj->iniconfig, INISEC_HOMES,
                                                  "basedir regex", NULL));
  LOG(log_debug, logtype_afpd,
  ...
  LOG(log_debug, logtype_afpd,
      "getvolbypath: basedir regex: '%s', basedir match: \"%s\"", basedir,
      tmpbuf);
  ...
  /* (6) */
  if ((subpathconfig = atalk_iniparser_getstring(obj->iniconfig, INISEC_HOMES,
                                                 "path", NULL))) {
    /*
  ...
  /* (7) */
  if (volxlate(obj, volpath, sizeof(volpath) - 1, tmpbuf, pw, NULL, NULL) ==
      NULL)
    EC_FAIL;
  ...
  EC_NULL_LOG(volxlate(obj, volname, sizeof(volname) - 1, tmpbuf, pw,
                       realvolpath, NULL));

  const char *preset, *default_preset;
  default_preset = atalk_iniparser_getstring(obj->iniconfig, INISEC_GLOBAL,
                                             "vol preset", NULL);
  preset = atalk_iniparser_getstring(obj->iniconfig, INISEC_HOMES, "vol preset",
                                     NULL);
...
int afp_config_parse(AFPObj *AFPObj, char *processname) {
  ...
  options->configfile = AFPObj->cmdlineconfigfile
  ...
  /* [Global] */
  options->logconfig = atalk_iniparser_getstrdup(config, INISEC_GLOBAL,
                                                 "log level", "default:note");
  options->logfile =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "log file", NULL);
  ...
  if (atalk_iniparser_getboolean(config, INISEC_GLOBAL,
                                 "solaris share reservations", 1))
  ...
  options->loginmesg =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "login message", NULL);
  options->guest = atalk_iniparser_getstrdup(config, INISEC_GLOBAL,
                                             "guest account", "nobody");
  options->extmapfile = atalk_iniparser_getstrdup(
      config, INISEC_GLOBAL, "extmap file", _PATH_CONFDIR "extmap.conf");
  options->passwdfile = atalk_iniparser_getstrdup(
      config, INISEC_GLOBAL, "passwd file", _PATH_AFPDPWFILE);
  options->uampath = atalk_iniparser_getstrdup(config, INISEC_GLOBAL,
                                               "uam path", _PATH_AFPDUAMPATH);
  options->uamlist = atalk_iniparser_getstrdup(
      config, INISEC_GLOBAL, "uam list", "uams_dhx.so uams_dhx2.so");
  options->port =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "afp port", "548");
  options->signatureopt =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "signature", "");
  options->k5service =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "k5 service", NULL);
  options->k5realm =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "k5 realm", NULL);
  options->listen =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "afp listen", NULL);
  options->interfaces =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "afp interfaces", NULL);
  options->ntdomain =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "nt domain", NULL);
  options->addomain =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "ad domain", NULL);
  options->ntseparator =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "nt separator", NULL);
  options->mimicmodel =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "mimic model", NULL);
  options->zeroconfname =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "zeroconf name", NULL);
  options->adminauthuser =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "admin auth user", NULL);
  options->ignored_attr = atalk_iniparser_getstrdup(config, INISEC_GLOBAL,
                                                    "ignored attributes", NULL);
  options->cnid_mysql_host =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "cnid mysql host", NULL);
  options->cnid_mysql_user =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "cnid mysql user", NULL);
  options->cnid_mysql_pw =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "cnid mysql pw", NULL);
  options->cnid_mysql_db =
      atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "cnid mysql db", NULL);
  options->connections =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "max connections", 200);
  options->passwdminlen =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "passwd minlen", 0);
  options->tickleval =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "tickleval", 30);
  options->timeout =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "timeout", 4);
  options->dsireadbuf =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "dsireadbuf", 12);
  options->server_quantum = atalk_iniparser_getint(
      config, INISEC_GLOBAL, "server quantum", DSI_SERVQUANT_DEF);
  options->volnamelen =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "volnamelen", 80);
  options->dircachesize = atalk_iniparser_getint(
      config, INISEC_GLOBAL, "dircachesize", DEFAULT_MAX_DIRCACHE_SIZE);
  options->tcp_sndbuf =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "tcpsndbuf", 0);
  options->tcp_rcvbuf =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "tcprcvbuf", 0);
  options->fce_fmodwait =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "fce holdfmod", 60);
  options->sleep =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "sleep time", 10);
  options->disconnected =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "disconnect time", 24);
  options->splice_size =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "splice size", 64 * 1024);
  options->sparql_limit =
      atalk_iniparser_getint(config, INISEC_GLOBAL, "sparql results limit", 0);
  ...
  if ((p = atalk_iniparser_getstring(config, INISEC_GLOBAL, "hostname",
                                     NULL))) {
  ...
  if ((p = atalk_iniparser_getstring(config, INISEC_GLOBAL, "k5 keytab",
                                     NULL))) {
  ...
  if ((p = atalk_iniparser_getstring(config, INISEC_GLOBAL, "admin group",
                                     NULL))) {
  ...
  if ((p = atalk_iniparser_getstring(config, INISEC_GLOBAL, "force user",
                                     NULL))) {
  ...
  if ((p = atalk_iniparser_getstring(config, INISEC_GLOBAL, "force group",
                                     NULL))) {
    struct group *gr = getgrnam(p);
    if (gr != NULL) {
  ...
  q = atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "cnid server",
                                "localhost:4700");
  r = strrchr(q, ':');
  if (r)
    *r = 0;
  ...
  if ((q = atalk_iniparser_getstrdup(config, INISEC_GLOBAL, "fqdn", NULL))) {
    ...
    if (gethostbyname(q)) {
      ...
      LOG(log_error, logtype_afpd,
          "error parsing -fqdn, gethostbyname failed for: %s", q);
    }
    free(q);
  }
  ...
  if (!(p = atalk_iniparser_getstring(config, INISEC_GLOBAL, "unix charset",
                                      NULL))) {
  ...
  /* vol charset is in [G] and [V] */
  if (!(p = atalk_iniparser_getstring(config, INISEC_GLOBAL, "vol charset",
                                      NULL))) {
  ...
  LOG(log_debug, logtype_afpd, "Global vol charset is %s",
      options->volcodepage);

  /* mac charset is in [G] and [V] */
  if (!(p = atalk_iniparser_getstring(config, INISEC_GLOBAL, "mac charset",
                                      NULL))) {
  ...
  LOG(log_debug, logtype_afpd, "Global mac charset is %s",
      options->maccodepage);
