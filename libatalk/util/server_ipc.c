...
static char *ipc_cmd_str[] = {"IPC_DISCOLDSESSION", "IPC_GETSESSION",
                              "IPC_STATE", "IPC_VOLUMES"};

/*
 * Pass afp_socket to old disconnected session if one has a matching token
 * (token = pid)
 * @returns -1 on error, 0 if no matching session was found, 1 if session was
 * found and socket passed
 */
static int ipc_kill_token(struct ipc_header *ipc, server_child_t *children) {
  ...
  return server_child_transfer_session(children, pid, ipc->uid, ipc->afp_socket,
                                       ipc->DSI_requestID);
}
...
static int ipc_get_session(struct ipc_header *ipc, server_child_t *children) {
  ...
  LOG(log_debug, logtype_afpd,
      "ipc_get_session(pid: %u, uid: %u, time: 0x%08x)", ipc->child_pid,
      ipc->uid, boottime);

  server_child_kill_one_by_id(children, ipc->child_pid, ipc->uid, idlen,
                              clientid, boottime);
...
int ipc_server_read(server_child_t *children, int fd) {
  ...
  if ((ret = read(fd, buf, IPC_HEADERLEN)) != IPC_HEADERLEN) {
    if (ret != 0) {
      ...
      LOG(log_error, logtype_afpd,
          "Reading IPC header failed (%i of %u bytes read): %s", ret,
          IPC_HEADERLEN, strerror(errno));
    }
    return -1;
  }
  ...
  if (ipc.len > (IPC_MAXMSGSIZE - IPC_HEADERLEN)) {
    LOG(log_info, logtype_afpd, "IPC message exceeds allowed size (%u)",
        ipc.len);
    return -1;
  }
  ...
  if (ipc.len != 0) {
    if ((ret = read(fd, buf, ipc.len)) != (int)ipc.len) {
      LOG(log_info, logtype_afpd,
          "Reading IPC message failed (%u of %u  bytes read): %s", ret, ipc.len,
          strerror(errno));
      return -1;
    }
  }
  ...
  case IPC_DISCOLDSESSION:
    ...
    if (ipc_kill_token(&ipc, children) == 1) {
      /* Transfered session (ie afp_socket) to old disconnected child, now kill
       * the new one */
...
int ipc_child_write(int fd, uint16_t command, int len, void *msg) {
  ...
  if ((ret = writet(fd, block, len + IPC_HEADERLEN, 0, 1)) !=
      len + IPC_HEADERLEN) {
    return -1;
  }
