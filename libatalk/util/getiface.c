...
static int getifaces(const int sockfd, char ***list) {
  ...
  new = (char **)malloc((sizeof(ifs) / sizeof(struct if_nameindex) + 1) *
                        sizeof(char *));
  ...
  *list = new;
  ...
  new = (char **)malloc((ifc.ifc_len / sizeof(struct ifreq) + 1) *
                        sizeof(char *));
  for (ifr = ifc.ifc_req; ifc.ifc_len >= (int)sizeof(struct ifreq);
       ifc.ifc_len -= ifrsize, ifr = nextifr) {
#ifdef BSD4_4
    ifrsize =
        sizeof(ifr->ifr_name) + (ifr->ifr_addr.sa_len > sizeof(struct sockaddr)
                                     ? ifr->ifr_addr.sa_len
