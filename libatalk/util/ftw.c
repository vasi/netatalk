...
static inline int open_dir_stream(int *dfdp, struct ftw_data *data,
  ...
  *((char *)__mempcpy(data->dirbuf + data->ftw.base, name, namlen)) = '\0';
  ...
  if (statres < 0) {
    if (errno != EACCES && errno != ENOENT)
      ...
      if (dir->streamfd != -1)
        statres =
            FXSTATAT(_STAT_VER, dir->streamfd, name, &st, AT_SYMLINK_NOFOLLOW);
      else
  ...
  if (result == 0 && (flag == FTW_NS || !(data->flags & FTW_MOUNT) ||
    ...
    if (flag == FTW_D) {
      ...
      result =
          (*data->func)(data->dirbuf, &st, data->cvt_arr[flag], &data->ftw);
  }
...
static int ftw_dir(struct ftw_data *data, struct STAT *st,
  ...
  result =
      open_dir_stream(old_dir == NULL ? NULL : &old_dir->streamfd, data, &dir);
  if (result != 0) {
    if (errno == EACCES)
...
static int ftw_startup(const char *dir, int is_nftw, void *func,
  ...
  if (result == 0) {
    ...
    if (((flags & FTW_PHYS) ? LXSTAT(_STAT_VER, name, &st)
      ...
      if (!(flags & FTW_PHYS) && errno == ENOENT &&
        ...
        result =
            (*data.func)(data.dirbuf, &st, data.cvt_arr[FTW_SLN], &data.ftw);
      else
      ...
      if (S_ISDIR(st.st_mode)) {
        ...
        result = (*data.func)(data.dirbuf, &st, data.cvt_arr[flag], &data.ftw);
      }
    }
...
/* Entry points.  */
int NFTW_NAME(const char *path, NFTW_FUNC_T func, dir_notification_func_t up,
