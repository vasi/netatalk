...
int run_cmd(const char *cmd, char **cmd_argv) {
  ...
  if (wpid != pid) {
    LOG(log_error, logtype_default, "waitpid(%d): %s", (int)pid,
        strerror(errno));
    EC_FAIL;
  }
 ...
 * Currently the only used options are O_NOFOLLOW, used to switch between
 *symlink behaviour, and O_NETATALK_ACL for ochmod() indicating chmod_acl()
 *shall be called which does special ACL handling depending on the filesytem
 *********************************************************************************/
 ...
 *
 * ostatat mulitplexes ostat and fstatat. If we don't HAVE_ATFUNCS, dirfd is
 * ignored.
 *
