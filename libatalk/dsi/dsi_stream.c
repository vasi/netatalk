...
static void dsi_header_pack_reply(const DSI *dsi, char *buf) {
  ...
  memcpy(buf + 2, &dsi->header.dsi_requestID,
...
static int dsi_peek(DSI *dsi) {
  ...
  while (1) {
    ...
    if (dsi->eof < dsi->end) {
      ...
      if (!warned) {
        ...
        LOG(log_note, logtype_dsi,
            "dsi_peek: readahead buffer is full, possibly increase -dsireadbuf "
            "option");
        LOG(log_note, logtype_dsi,
            "dsi_peek: dsireadbuf: %d, DSI quantum: %d, effective buffer size: "
            "%d",
            dsi->dsireadbuf,
    ...
    /* Check if there's sth to read, hopefully reading that will unblock the
     * client */
...
static size_t from_buf(DSI *dsi, uint8_t *buf, size_t count) {
  ...
  LOG(log_debug, logtype_dsi,
      ...
      dsi->start - dsi->buffer, dsi->eof - dsi->start, dsi->end - dsi->eof,
      nbe);
...
/*
 * Get "length" bytes from buffer and/or socket. In order to avoid frequent
 * small reads this tries to read larger chunks (8192 bytes) into a buffer.
 */
...
/* ---------------------------------------
 */
static void block_sig(DSI *dsi) { dsi->in_write++; }

/* ---------------------------------------
 */
static void unblock_sig(DSI *dsi) { dsi->in_write--; }
 ...
 *             or euid == 0 ie where still running as root (unauthenticated
 * session)
 */
...
ssize_t dsi_stream_write(DSI *dsi, void *data, const size_t length, int mode) {
  ...
  while (written < length) {
    ...
    if (errno == EAGAIN || errno == EWOULDBLOCK) {
      LOG(log_debug, logtype_dsi, "dsi_stream_write: send: %s",
          strerror(errno));
  ...
  while (written < total) {
...
#elif defined(FREEBSD)
    len =
        sendfile(fromfd, dsi->socket, pos, total - written, hdrp, &nwritten, 0);
    if (len == 0)
      len = nwritten;
#else
    ...
    if (len < 0) {
      ...
      case EAGAIN:
        ...
        break;
      default:
        LOG(log_error, logtype_dsi, "dsi_stream_read_file: %s",
            strerror(errno));
        ret = -1;
        goto exit;
      }
    ...
    if (hdrp) {
      if (len >= iovec.iov_len) {
        hdrp = NULL;
        len -= iovec.iov_len; /* len now contains how much sendfile() actually
                                 sent from the file */
      } else {
        iovec.iov_len -= len;
        iovec.iov_base += len;
  ...
  while (stored < length) {
    ...
    if (len == -1 && (errno == EINTR || errno == EAGAIN)) {
      ...
      if (len || stored || dsi->read_count) {
        if (!(dsi->flags & DSI_DISCONNECTED)) {
          LOG(log_error, logtype_dsi, "dsi_stream_read: len:%d, %s", len,
              (len < 0) ? strerror(errno) : "unexpected EOF");
        }
        return 0;
      }
      break;
...
int dsi_stream_send(DSI *dsi, void *buf, size_t length) {
  ...
  if (!length) { /* just write the header */
    LOG(log_maxdebug, logtype_dsi,
        "dsi_stream_send(%u bytes): DSI header, no data", sizeof(block));
...
int dsi_stream_receive(DSI *dsi) {
  ...
  /* read in the header */
  if (dsi_buffered_stream_read(dsi, (uint8_t *)block, sizeof(block)) !=
      sizeof(block))
    return 0;
