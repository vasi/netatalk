 ...
 *
 * Calling without <filename> configures basic logging to syslog. Specifying
 * <filename> configures extended logging to <filename>.
 *
 ...
 *
 * Calling without <any_string> disables syslog logging, calling with
 * <any_string> disables file logging.
 *
 * <logtype>:
 * you can setup a default with "Default". Any other logtype used in LOG will
 * then use the default if not setup itself. This is probabyl the only thing you
 * may want to use.
 *
 * Example:
 ...
 *
 *    syslog_setup(int loglevel, enum logtypes logtype, int display_options, int
 * facility);
 *
...
/* This stores the config and options for one filelog type (e.g. logger, afpd
 * etc.) */
typedef struct {
...
UAM_MODULE_EXPORT void make_log_entry(enum loglevels loglevel,
                                      enum logtypes logtype, const char *file,
