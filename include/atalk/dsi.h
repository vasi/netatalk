...
typedef enum { DSI_MIN = 1, DSI_TCPIP = 1, DSI_MAX = 1 } dsi_proto;
...
#define DSIOPT_REPLCSIZE                                                       \
  0x02 /* AFP replaycache size supported by the server (that's us) */
...
#define DSI_DISCONNECTED                                                       \
  (1 << 4)               /* we're in diconnected state after a socket error */
#define DSI_DIE (1 << 5) /* SIGUSR1, going down in 5 minutes */
#define DSI_NOREPLY (1 << 6) /* in dsi_write we generate our own replies */
#define DSI_RECONSOCKET                                                        \
  (1 << 7) /* we have a new socket from primary reconnect */
#define DSI_RECONINPROG (1 << 8) /* used in the new session in reconnect */
#define DSI_AFP_LOGGED_OUT                                                     \
  (1 << 9) /* client called afp_logout, quit on next EOF from socket */
