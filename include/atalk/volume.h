...
/* load_volumes() flags */
typedef enum { LV_DEFAULT = 0, LV_ALL = 1, LV_FORCE = 2 } lv_flags_t;
...
#define AFPVOL_NOV2TOEACONV                                                    \
  (1 << 5)                        /* no adouble:v2 to adouble:ea conversion */
#define AFPVOL_SPOTLIGHT (1 << 6) /* Index volume for Spotlight searches */
#define AFPVOL_EA_SAMBA                                                        \
  (1 << 7)                 /* Store Samba compatible xattrs (append 0 byte) */
#define AFPVOL_RO (1 << 8) /* read-only volume */
#define AFPVOL_CHMOD_PRESERVE_ACL (1 << 9) /* try to preserve ACLs */
#define AFPVOL_CHMOD_IGNORE (1 << 10)      /* try to preserve ACLs */
#define AFPVOL_FORCE_STICKY_XATTR                                              \
  (1 << 11) /* write metadata xattr as root on sticky dirs */
#define AFPVOL_NOSTAT                                                          \
  (1 << 16) /* advertise the volume even if we can't stat() it                 \
             * maybe because it will be mounted later in preexec */
#define AFPVOL_UNIX_PRIV (1 << 17) /* support unix privileges */
#define AFPVOL_NODEV                                                           \
  (1 << 18) /* always use 0 for device number in cnid calls                    \
             * help if device number is notconsistent across reboot            \
             * NOTE symlink to a different device will return an ACCESS error  \
             */
#define AFPVOL_EILSEQ                                                          \
  (1 << 20) /* encode illegal sequence 'asis' UCS2, ex "\217-", which is not   \
               a valid SHIFT-JIS char, is encoded  as U\217 -*/
#define AFPVOL_INV_DOTS (1 << 22) /* dots files are invisible */
#define AFPVOL_TM (1 << 23)       /* Supports TimeMachine */
#define AFPVOL_ACLS (1 << 24)     /* Volume supports ACLS */
#define AFPVOL_SEARCHDB                                                        \
  (1 << 25) /* Use fast CNID db search instead of filesystem */
#define AFPVOL_NONETIDS                                                        \
  (1 << 26) /* signal the client it shall do privelege mapping */
#define AFPVOL_FOLLOWSYM                                                       \
  (1 << 27) /* follow symlinks on the server, default is not to */
...
#define vol_chmod_opt(vol)                                                     \
  (((vol)->v_flags & AFPVOL_CHMOD_PRESERVE_ACL) ? O_NETATALK_ACL               \
   : ((vol)->v_flags & AFPVOL_CHMOD_IGNORE)     ? O_IGNORE                     \
                                                : 0)
