...
/* special key/data pair we use to store current cnid and database stamp in
 * cnid2.db */
...
/*
   Rootinfo data, fields as they are used by normal entries for CNIDs (for
   reference): 4 bytes: CNID 8 bytes: dev 8 bytes: inode 4 bytes: is a
   file/directory (type) 4 bytes: DID x bytes: name
