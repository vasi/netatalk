...
#define AD_HEADER_LEN                                                          \
  (ADEDLEN_MAGIC + ADEDLEN_VERSION + ADEDLEN_FILLER + ADEDLEN_NENTRIES) /* 26  \
                                                                         */
...
#define AD_DATASZ2                                                             \
  (AD_HEADER_LEN + ADEDLEN_NAME + ADEDLEN_COMMENT + ADEDLEN_FILEI +            \
   ADEDLEN_FINDERI + ADEDLEN_DID + ADEDLEN_AFPFILEI + ADEDLEN_SHORTNAME +      \
   ADEDLEN_PRODOSFILEI + ADEDLEN_PRIVDEV + ADEDLEN_PRIVINO + ADEDLEN_PRIVSYN + \
   ADEDLEN_PRIVID + (ADEID_NUM_V2 * AD_ENTRY_LEN))
#if AD_DATASZ2 != 741
#error bad size for AD_DATASZ2
#endif

#define AD_DATASZ_EA                                                           \
  (AD_HEADER_LEN + (ADEID_NUM_EA * AD_ENTRY_LEN) + ADEDLEN_FINDERI +           \
   ADEDLEN_COMMENT + ADEDLEN_FILEDATESI + ADEDLEN_AFPFILEI + ADEDLEN_PRIVDEV + \
   ADEDLEN_PRIVINO + ADEDLEN_PRIVSYN + ADEDLEN_PRIVID)

#if AD_DATASZ_EA != 402
#error bad size for AD_DATASZ_EA
#endif

#define AD_DATASZ_OSX                                                          \
  (AD_HEADER_LEN + (ADEID_NUM_OSX * AD_ENTRY_LEN) + ADEDLEN_FINDERI)
...
#define ADFLAGS_CHECK_OF                                                       \
  (1 << 6) /* check for open forks from us and other afpd's */
...
#define AD_DATE_MASK                                                           \
  (AD_DATE_CREATE | AD_DATE_MODIFY | AD_DATE_BACKUP | AD_DATE_ACCESS)
...
#define ATTRBIT_DOPEN                                                          \
  (1 << 3) /* data fork already open. Not stored, computed on the fly */
#define ATTRBIT_ROPEN                                                          \
  (1 << 4) /* resource fork already open. Not stored, computed on the fly */
#define ATTRBIT_NOWRITE                                                        \
  (1 << 5) /* "w", f, write inhibit(v2)/read-only(v1) bit */
...
extern int ad_setid(struct adouble *, dev_t dev, ino_t ino, uint32_t, uint32_t,
                    const void *);
